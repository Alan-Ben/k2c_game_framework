...
namespace ALPackage
{
    public class ALMonoTaskTimingNode
    {
        /** 当前回合标记 */
        private int _m_iCurRound;
        /** 当前回合的本节点定时任务队列 */
        private List<_IALBaseMonoTask> _m_lCurRoundTimingTaskList;

        /** 下一回合标记 */
        private int _m_iNextRound;
        /** 固定记录下一回合的延迟任务队列集合 */
        private List<_IALBaseMonoTask> _m_lNextRoundTimingTaskList;
    
        /** 非本回合与下一回合的较长时间延迟任务信息集合队列 */
        private LinkedList<ALMonoTaskTimingNodeFarDelayTaskInfo> _m_lFarDelayTaskList;

        public ALMonoTaskTimingNode(int _round)
        {
            _m_iCurRound = _round;
            _m_lCurRoundTimingTaskList = new List<_IALBaseMonoTask>();
        
            _m_iNextRound = _round + 1;
            _m_lNextRoundTimingTaskList = new List<_IALBaseMonoTask>();

            _m_lFarDelayTaskList = new LinkedList<ALMonoTaskTimingNodeFarDelayTaskInfo>();
        }
         ...
         */
        public bool addTimingTask(int _round, _IALBaseMonoTask _task)
        {
            lock (_m_lCurRoundTimingTaskList)
            {
                ...
                else if (_round > _m_iNextRound)
                {
                    ...
                    LinkedListNode<ALMonoTaskTimingNodeFarDelayTaskInfo> iterator = _m_lFarDelayTaskList.First;
                    while (null != iterator)
                    {
                        ALMonoTaskTimingNodeFarDelayTaskInfo taskInfo = iterator.Value;
                        ...
                        else if (taskInfo.getRound() > _round)
                        {
                            //当查询的节点序号比插入序号还要靠后时
                            //创建新节点，并插入当前查询的节点之前
                            ALMonoTaskTimingNodeFarDelayTaskInfo newInfo = new ALMonoTaskTimingNodeFarDelayTaskInfo(_round);
         ...
         */
        public void popAllRoundTaskAndMoveNextRound(int _round, List<_IALBaseMonoTask> _recList)
        {
            if(null == _recList)
                return ;
        ...
        protected void _moveNextRound()
        {
            ...
            {
                ALMonoTaskTimingNodeFarDelayTaskInfo farDelayInfo = _m_lFarDelayTaskList.First.Value;
