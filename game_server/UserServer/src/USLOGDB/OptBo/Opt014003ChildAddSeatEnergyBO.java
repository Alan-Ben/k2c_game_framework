...
public class Opt014003ChildAddSeatEnergyBO extends BaseOptLogBo {
    ...
    @DataBaseField(type = "bigint(20)", fieldname = "seatId", comment = "席位ID")
    private long seatId;

    public static final int FIELD_addCount =8;
    @DataBaseField(type = "int(11)", fieldname = "addCount", comment = "增加数量")
    private int addCount;

    public Opt014003ChildAddSeatEnergyBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt014003ChildAddSeatEnergyBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        seatId = rs.getLong(9);
        addCount = rs.getInt(10);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt014003ChildAddSeatEnergyBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `seatId`, `addCount`";
    }

    @Override
    public String getTableName() {
        return "`opt_014_003_child_add_seat_energy`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(addCount).append("', ");
   ...
   @Override
    ...
    public void setAddCount(BM _bm, int addCount) {
        if(addCount==this.addCount) 
            return;
        this.addCount = addCount; 
        markField(_bm, FIELD_addCount); 
    }
    public void saveAddCount(BM _bm, int addCount) {
        if(addCount==this.addCount) 
            return;
        this.addCount = addCount;
        saveField(_bm, "addCount", addCount);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `addCount` = '").append(addCount).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_addCount)) sBuilder.append(" `addCount` = '").append(addCount).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_014_003_child_add_seat_energy` ("
                ...
                + "`seatId` bigint(20) NOT NULL DEFAULT '0' COMMENT '席位ID',"
                + "`addCount` int(11) NOT NULL DEFAULT '0' COMMENT '增加数量',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='子嗣-增加训练席位脑力值' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
