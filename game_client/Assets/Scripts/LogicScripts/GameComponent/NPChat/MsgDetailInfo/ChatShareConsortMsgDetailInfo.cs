
using NPEnum;
using ChatPackage;
using Common;
using Common.ConsortObj;
using Common.NpChatObj;
using CommonEnum;

namespace GOE
{
    /// <summary>
    /// 聊天妃子分享信息
    /// </summary>
    public class ChatShareConsortMsgDetailInfo : _AMsgDetailInfo<NPCommon_ChatContent_ConsortShare, NPCommon_ChatPlayerContent>, _INPChatMiniShowInfo
    {
        public ChatShareConsortMsgDetailInfo() : base((int) ENPChatMsgType.SHARE_CONSORT)
        {
        }
        
        /// <summary>
        /// 是否是我自己的消息
        /// </summary>
        public override bool isMyMsg { get { return sender.getCid() == NPPlayer.instance.playerInfo.CID; } }
        
        /// <summary>
        /// 设置消息发送者为我自己
        /// </summary>
        public void setSenderIsMyself()
        {
            sender.setCName(NPPlayer.instance.playerInfo.PlayerName);
            sender.setCid(NPPlayer.instance.playerInfo.CID);
            sender.setIconId(NPPlayer.instance.playerInfo.curIcon.baseData.sub_id);
            sender.setIconBgkId(NPPlayer.instance.playerInfo.curIconBgk.baseData.sub_id);
            sender.setBubbleId(NPPlayer.instance.playerInfo.getCurrentBubbleId());
            sender.setVipLvl((int)NPPlayer.instance.playerInfo.getCurrentVIPLvl());
            sender.setPlayerSkinId(NPPlayer.instance.playerInfo.getCurrentSkinId());
            sender.setCurTitle(NPPlayer.instance.titleComp.curTitle);
            sender.setIsShow(NPPlayer.instance.titleComp.isShowOthers);
        }

        /// <summary>
        /// 设置消息内容...
        /// </summary>
        public void setContent(GGottenConsortInfo gottenConsortInfo)
        {
            if(gottenConsortInfo == null)
                return;
            
            content.setConsortId(gottenConsortInfo.consortId);
            content.setSkinId(gottenConsortInfo.consortSkinShowInfo?.skinId ?? 0);
            content.setCharm(gottenConsortInfo.charm);
            content.setIntimacy(gottenConsortInfo.intimacy);

            Consort_Fetters consortFetters = new Consort_Fetters(gottenConsortInfo.fetterInfo?.fetterLevel ?? 0);
            content.setConsortFetters(consortFetters);
        }
        /// <inheritdoc/>
        public string getMiniSender()
        {
            return sender.getCName();
        }
        /// <inheritdoc/>
        public string getMiniContent()
        {
            return TextTranslate.instance.getLanguage(TransKeyConst.chat_share_consort_str,GCommon.getItemName(ENPItemType.CONSORT,content.getConsortId()));
        }
    }
}