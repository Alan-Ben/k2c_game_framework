...
namespace UnityEngine.Rendering.Universal
{
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        static void RenderCameraStack(ScriptableRenderContext context, Camera baseCamera)
        {
            ...
            int secondToLastActiveOverlayCameraIndex = -1;
            bool hasUICamera = false;
            if (baseCameraAdditionalData != null && baseCameraAdditionalData.scriptableRenderer is UniversalRenderer baseURender)
            {
                if (baseURender.isUICamera)
                    hasUICamera = true;
            }
            if (cameraStack != null)
            {
                ...
                for (int i = 0; i < cameraStack.Count; ++i)
                {
                    ...
                    if (currCamera.isActiveAndEnabled)
                    {
                        ...
                        if (data.scriptableRenderer is UniversalRenderer universalRenderer)
                        {
                            if (universalRenderer.isUICamera)
                                hasUICamera = true;
                        }

                        anyPostProcessingEnabled |= data.renderPostProcessing;
                        secondToLastActiveOverlayCameraIndex = lastActiveOverlayCameraIndex;
            ...
            bool isOnlyUICamera = hasUICamera && !isStackedRendering;
...
#pragma warning restore 0618
            ...
            // ����б���ֻ��һ���������ô˵��������ǵ����ڶ������
            baseCameraData.secondToLastCamera = secondToLastActiveOverlayCameraIndex == -1;
            baseCameraData.hasUICameraInStack = hasUICamera;
            bool directRenderToCameraTarget = false;
            if (hasUICamera)
            {
                //  TODO ģ��������£�����Ⱦ�ķֱ��ʲ����У����Կ��ǽ��ͷֱ��ʴ���
                {
                    // �������Ĭ���ӿڣ�����UIģ��״̬�Ǿ�̬ģ������ʵʱģ��������ҪUI RT
                    baseCameraData.needUITarget = !baseCameraData.isDefaultViewport || MJUniversalRenderAPI.uiBlurState == UIBlurState.StaticBlur || MJUniversalRenderAPI.uiBlurState == UIBlurState.RealTimeBlur ;
#if UNITY_ANDROID
                    // ������������UI��������Ҳ���ҪUI RT,�����ҪresolveFinalTarget���������Ϊ�����ڶ���
                    if (!baseCameraData.needUITarget)
                    {
                        if (baseCameraData.imageScalingMode == ImageScalingMode.None)
                        {
                            baseCameraData.resolveFinalTarget = true;
                        }
                        else
                        {
                            baseCameraData.resolveFinalTarget = baseCameraData.secondToLastCamera;
                        }
                    }
#endif
                }
            ...
            {
                baseCameraData.hasUICameraInStack = false;
                baseCameraData.needUITarget = false;
            }
            ...
            if (isStackedRendering)
            {
                for (int i = 0; i < cameraStack.Count; ++i)
                {
                    ...
                    if (currCameraData != null)
                    {
                        ...
                        bool secondToLastCamera = i == secondToLastActiveOverlayCameraIndex;
...
#if UNITY_ANDROID
                        // ��������Ƿ��ǵ����� �Ƿ���UI���
                        overlayCameraData.secondToLastCamera = secondToLastCamera;
                        // ������������UI�����������ҪUI RT,�����ҪresolveFinalTarget���������Ϊ�����ڶ���
                        if (hasUICamera && !baseCameraData.needUITarget)
                        {
                            if (baseCameraData.imageScalingMode == ImageScalingMode.None)
                            {
                                overlayCameraData.resolveFinalTarget = true;
                            }
                            else
                            {
                                overlayCameraData.resolveFinalTarget = secondToLastCamera;
                            }                            
                        }
#endif
#if ENABLE_VR && ENABLE_XR_MODULE
