...
public class Opt013004HeroUpgradeStepBO extends BaseOptLogBo {
    ...
    public static final int FIELD_hero_id =7;
    @DataBaseField(type = "bigint(20)", fieldname = "hero_id", comment = "大臣ID")
    private long hero_id;

    public static final int FIELD_step =8;
    @DataBaseField(type = "int(11)", fieldname = "step", comment = "当前阶位")
    private int step;

    public static final int FIELD_talent_exp =9;
    @DataBaseField(type = "bigint(20)", fieldname = "talent_exp", comment = "当前天赋经验")
    private long talent_exp;

    public Opt013004HeroUpgradeStepBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt013004HeroUpgradeStepBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        hero_id = rs.getLong(9);
        step = rs.getInt(10);
        talent_exp = rs.getLong(11);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt013004HeroUpgradeStepBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `hero_id`, `step`, `talent_exp`";
    }

    @Override
    public String getTableName() {
        return "`opt_013_004_hero_upgrade_step`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(talent_exp).append("', ");
   ...
   @Override
    ...
    // 当前天赋经验
    public long getTalentExp() { return this.talent_exp; }
    public void setTalentExp(BM _bm, long talent_exp) {
        if(talent_exp==this.talent_exp) 
            return;
        this.talent_exp = talent_exp; 
        markField(_bm, FIELD_talent_exp); 
    }
    public void saveTalentExp(BM _bm, long talent_exp) {
        if(talent_exp==this.talent_exp) 
            return;
        this.talent_exp = talent_exp;
        saveField(_bm, "talent_exp", talent_exp);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `talent_exp` = '").append(talent_exp).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_talent_exp)) sBuilder.append(" `talent_exp` = '").append(talent_exp).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_013_004_hero_upgrade_step` ("
                ...
                + "`step` int(11) NOT NULL DEFAULT '0' COMMENT '当前阶位',"
                + "`talent_exp` bigint(20) NOT NULL DEFAULT '0' COMMENT '当前天赋经验',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='大臣-大臣升级阶位' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
