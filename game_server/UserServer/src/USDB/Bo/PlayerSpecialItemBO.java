...
@RefBo(isIdAuto= true)
public class PlayerSpecialItemBO extends BaseBO {
    ...
    @DataBaseField(type = "int(11)", fieldname = "type", comment = "特殊物品类型")
    private int type;
    ...
    public PlayerSpecialItemBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        type = rs.getInt(3);
        data = rs.getBytes(4);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new PlayerSpecialItemBO(rs));
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_special_item` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家CID',"
                + "`type` int(11) NOT NULL DEFAULT '0' COMMENT '特殊物品类型',"
                + "`data` blob NULL COMMENT '数据',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家特殊物品数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
