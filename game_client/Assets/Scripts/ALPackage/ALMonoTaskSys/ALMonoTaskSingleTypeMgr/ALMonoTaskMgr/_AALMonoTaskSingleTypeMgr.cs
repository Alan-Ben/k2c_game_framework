...
namespace ALPackage
{
    public abstract class _AALMonoTaskSingleTypeMgr
    {
        /** 当前任务队列由于此队列只从队列头抽取对象，并且仅在队列尾插入对象 */
        private List<_IALBaseMonoTask> _m_lCurrentTaskList;
        ...
        private ALMonoTaskTimingNode[] _m_arrTimingTaskNodeList;

        /** 临时用于检测时间标记所使用的临时任务队列，只在一处调用，因此不加锁 */
        private List<_IALBaseMonoTask> _m_lTmpTimingTaskList;

        public _AALMonoTaskSingleTypeMgr(int _checkTimePerSec, int _checkAreaNodeSize)
        {
            _m_lCurrentTaskList = new List<_IALBaseMonoTask>();
            ...
            //创建固定区间长度的精度队列
            _m_arrTimingTaskNodeList = new ALMonoTaskTimingNode[_m_iTimingTaskCheckAreaSize];
            for (int i = 0; i < _m_iTimingTaskCheckAreaSize; i++)
            {
                _m_arrTimingTaskNodeList[i] = new ALMonoTaskTimingNode(0);
            }

            _m_lTmpTimingTaskList = new List<_IALBaseMonoTask>();
        }
         ...
         */
        public void regTask(_IALBaseMonoTask _task)
        {
        ...
        public void regTask(List<_IALBaseMonoTask> _taskList)
        {
            lock (_m_lCurrentTaskList)
            {
                //向链表中添加执行任务
         ...
         */
        public void regTask(_IALBaseMonoTask _task, float _time)
        {
         ...
         */
        public _IALBaseMonoTask popCurrentTask()
        {
            lock (_m_lCurrentTaskList)
            {
                //判断任务队列是否为空
                ...
                //取出并移除任务队列第一个任务
                _IALBaseMonoTask task = _m_lCurrentTaskList[_m_iCurTaskIndex];
                _m_iCurTaskIndex++;
         ...
         */
        protected void _registerTaskList(List<_IALBaseMonoTask> _taskList)
        {
         ...
         */
        protected bool _regTimingTask(float _dealTime, _IALBaseMonoTask _task)
        {
            lock(_m_arrTimingTaskNodeList)
            {
         ...
         */
        protected void _popTimerTask(List<_IALBaseMonoTask> _recList)
        {
            if (null == _recList)
                return;
