...
namespace Hotfix
{
    ...
    public class HotfixCommonErrCodeRequestCallbackProtocolDealer<T> : _AHotfixRequestCallbackProtocolDealer<T>
        where T : _IALProtocolStructure, new()
    {
        private Action<T> _m_sucAction;

        public HotfixCommonErrCodeRequestCallbackProtocolDealer(Action<T> _sucAction, bool _preDealGCProtocol = false, bool _needPrintProtocol = true)
