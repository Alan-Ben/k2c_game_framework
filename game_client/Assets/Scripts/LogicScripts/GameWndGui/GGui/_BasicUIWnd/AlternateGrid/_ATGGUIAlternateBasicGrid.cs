...
namespace GOE
{
    ...
    public abstract class _ATGGUIAlternateBasicGrid<_T_ITEM_MONO, _T_CONTAINER_MONO, _T_ITEM_WND> : _ATALBasicUISubWnd<_T_CONTAINER_MONO>, _IALBasicRefreshUIWndInterface
        where _T_ITEM_MONO : _TALUGUIMonoGridItem
        where _T_CONTAINER_MONO : _ATGGUIMonoAlternateBasicGrid<_T_ITEM_MONO>
        where _T_ITEM_WND : _ATALUGUIBasicGridItemWnd<_T_ITEM_MONO>
    {
        ...
        /** 根据当前窗口尺寸创建出来的所有可能需要使用到的间隔点对象队列 */
        private List<AlternateGridDotItem> _m_lDotList;
        /** 需要刷新位置的间隔点列表 */
        private List<AlternateGridDotItem> _m_lMoveDotList;
        /** 需要展示间隔点的下标列表 */
        private HashSet<int> _m_iNeedShowDotIndexList;
        ...
        protected _ATGGUIAlternateBasicGrid(_T_CONTAINER_MONO _wnd)
            : base(_wnd)
        {
            ...
            _m_lDotList = new List<AlternateGridDotItem>();
            _m_lMoveDotList = new List<AlternateGridDotItem>();
            _m_iNeedShowDotIndexList = new HashSet<int>();
        ...
        public Vector2 getItemPos(int _idx)
        {
            ...
            {
                //横向位置设置，注意加上预留位移
                bool isOffsetLine = _checkIsOffsetLine(_idx, out int curItemIndexInThisLine, out int curLineIndex);
                float halfItemHeight = _m_wTmpItem.wnd.width / 2.0f + _m_vCalSpace.y / 2.0f;
                pos.x = (curLineIndex * (wnd.itemTemplate.width + _m_vCalSpace.x)) + _m_vCalSpace.x + wnd.paddingForSide.x;
                pos.y = -((curItemIndexInThisLine) * (wnd.itemTemplate.height + _m_vCalSpace.y)) + _m_vCalSpace.y - (isOffsetLine ? halfItemHeight : 0);
            }
            else
            {
                //纵向位置设置，注意加上预留位移
                bool isOffsetLine = _checkIsOffsetLine(_idx, out int curItemIndexInThisLine, out int curLineIndex);
                float halfItemWidth = _m_wTmpItem.wnd.width / 2.0f + _m_vCalSpace.x / 2.0f;
                pos.x = ((curItemIndexInThisLine) * (wnd.itemTemplate.width + _m_vCalSpace.x)) + _m_vCalSpace.x + (isOffsetLine ? halfItemWidth : 0);
                pos.y = -((curLineIndex * (wnd.itemTemplate.height + _m_vCalSpace.y)) + _m_vCalSpace.y + wnd.paddingForSide.x);
            }
        ...
        public void setItemCount(int _count)
        {
            ...
            // 空物品提示
            if (null != wnd && null != wnd.noneItemsTips)
                ALUGUICommon.setUIObjScale(wnd.noneItemsTips, _m_iTotalCount <= 0 ? 1 : 0);
        ...
        public void clearAll()
        {
            ...
            //释放间隔点对象
            foreach (AlternateGridDotItem dotItem in _m_lDotList)
            {
                ALUnityCommon.releaseGameObj(dotItem.dotGO);
            }
            ...
            _m_lDotList.Clear();
            _m_lMoveDotList.Clear();
            _m_iNeedShowDotIndexList.Clear();
        }
        ...
        public void checkWndSize()
        {
                ...
                || _m_iShowTotlaCount != _m_iTotalCount || _m_bNeedForceRefreshSize)
            {
                ...
                {
                    //由于行偏移少一个item的行数量
                    int skewingCount = _getOffsetCount(_m_iShowTotlaCount, _m_iPerUnitItemCount);
                    int totalLineCount = (_m_iShowTotlaCount + skewingCount + _m_iPerUnitItemCount - 1) / _m_iPerUnitItemCount;
                    //设置区域对象的尺寸
                    if (wnd.layoutStyle == ALGUIListLayoutStyle.HORIZONTAL)
                    {
                        //设置总宽度的时候需要附带padding
                ...
                //计算需要的总数量
                //每行数量 * 需要展示的行数 - 由于行偏移少一个item的数量
                int totalCacheItemCount = _m_iPerUnitItemCount * _m_iShowLineCount - (_m_iShowLineCount / 2);

                //计算需要的间隔点的总数量
                int dotItemCount = totalCacheItemCount - _m_iShowLineCount;
                ...
                //=====================处理间隔点=====================
                //根据需要的总数量从数据集合中删除多余，增加缺少的
                if (wnd != null && wnd.spaceTemplate != null)
                {
                    while (_m_lDotList.Count < dotItemCount)
                    {
                        _addDotItem(-1);
                    }
                    //删除多余的
                    while (_m_lDotList.Count > dotItemCount)
                    {
                        _removeDotItem(_m_lDotList[_m_lDotList.Count - 1]);
                    }
                }
        ...
        private void _recalculateCurRectNoBar()
        {
            ...
            if (wnd.layoutStyle == ALGUIListLayoutStyle.HORIZONTAL)
            {
                ...
                {
                    //展示范围内有几行偏移
                    int alternateCount = (int)((_m_vGridCurShowRect.x - wnd.paddingForSide.x) / (wnd.itemTemplate.width + _m_vCalSpace.x)) / 2;
                    _m_iStartItemIdx = (int)((_m_vGridCurShowRect.x - wnd.paddingForSide.x) / (wnd.itemTemplate.width + _m_vCalSpace.x)) * _m_iPerUnitItemCount - alternateCount;
                }

                //由于行偏移少一个item的行数量
                int skewingCount = _m_iShowLineCount/2;
                _m_iFinalItemIdx = _m_iStartItemIdx + (_m_iPerUnitItemCount * _m_iShowLineCount) - 1 - skewingCount;
                //判断最后的索引是否有效
            ...
            else
            {
                ...
                {
                    //展示范围内有几行偏移
                    int alternateCount = (int)((_m_vGridCurShowRect.y - wnd.paddingForSide.x) / (wnd.itemTemplate.height + _m_vCalSpace.y)) / 2;
                    _m_iStartItemIdx = (int)((_m_vGridCurShowRect.y - wnd.paddingForSide.x) / (wnd.itemTemplate.height + _m_vCalSpace.y)) * _m_iPerUnitItemCount - alternateCount;
                }
            ...
            for (_m_iTmpi = 0; _m_iTmpi < _m_lShowItemList.Count; _m_iTmpi++)
            {
                ...
                //设置显示对象位置
                Vector2 itemPostV2 = getItemPos(_m_iTmpi2);
                ALUGUICommon.setUIPos(_m_wTmpItem.wnd.gameObject, itemPostV2.x, itemPostV2.y);
            ...
            if (wnd != null && wnd.spaceTemplate != null)
            {
                ...
                //查找需要展示间隔点的下标列表
                for (int i = _m_iStartItemIdx; i <= _m_iFinalItemIdx; i++)
                {
                    bool isOffsetLine = _checkIsOffsetLine(i, out int curItemIndexInThisLine, out int curLineIndex);
                    bool needShowDot = false;
                    if (i + 1 != _m_iTotalCount && isOffsetLine && curItemIndexInThisLine + 1 != _m_iPerUnitItemCount - 1)
                        needShowDot = true;
                    else if (i + 1 != _m_iTotalCount && !isOffsetLine && curItemIndexInThisLine + 1 != _m_iPerUnitItemCount)
                        needShowDot = true;

                    if (needShowDot)
                    {
                        _m_iNeedShowDotIndexList.Add(i);
                    }
                }

                //将在展示范围内的间隔点剔除出需要展示的下标列表，记录需要移动的间隔点列表_m_lMoveDotList
                for (int i = 0; i < _m_lDotList.Count; i++)
                {
                    if (_m_iNeedShowDotIndexList.Contains(_m_lDotList[i].index))
                    {
                        _m_iNeedShowDotIndexList.Remove(_m_lDotList[i].index);
                        Vector2 itemPos = getItemPos(_m_lDotList[i].index);
                        if (wnd.layoutStyle == ALGUIListLayoutStyle.HORIZONTAL)
                            ALUGUICommon.setUIPos(_m_lDotList[i].dotGO, itemPos.x, itemPos.y - (_m_wTmpItem.wnd.width / 2.0f + _m_vCalSpace.y / 2.0f));
                        else
                            ALUGUICommon.setUIPos(_m_lDotList[i].dotGO, itemPos.x + _m_wTmpItem.wnd.width / 2.0f + _m_vCalSpace.x / 2.0f, itemPos.y);
                        ALUGUICommon.setUIObjScale(_m_lDotList[i].dotGO, 1f);
                    }
                    else
                    {
                        _m_lDotList[i].index = -1;
                        ALUGUICommon.setUIObjScale(_m_lDotList[i].dotGO, 0f);
                        _m_lMoveDotList.Add(_m_lDotList[i]);
                    }
                }

                //根据剩余需要展示的间隔点下标，设置剩下的间隔点位置
                int moveDotIndex = 0;
                foreach (int needShowIndex in _m_iNeedShowDotIndexList)
                {
                    if (moveDotIndex < _m_lMoveDotList.Count)
                    {
                        _m_lMoveDotList[moveDotIndex].index = needShowIndex;
                        Vector2 itemPos = getItemPos(needShowIndex);
                        if (wnd.layoutStyle == ALGUIListLayoutStyle.HORIZONTAL)
                            ALUGUICommon.setUIPos(_m_lMoveDotList[moveDotIndex].dotGO, itemPos.x, itemPos.y - (_m_wTmpItem.wnd.width / 2.0f + _m_vCalSpace.y / 2.0f));
                        else
                            ALUGUICommon.setUIPos(_m_lMoveDotList[moveDotIndex].dotGO, itemPos.x + _m_wTmpItem.wnd.width / 2.0f + _m_vCalSpace.x / 2.0f, itemPos.y);
                        ALUGUICommon.setUIObjScale(_m_lMoveDotList[moveDotIndex].dotGO, 1f);
                        moveDotIndex++;
                    }
                }
                _m_iNeedShowDotIndexList.Clear();
        ...
        //                 , startPos + ((inControlIdx / _m_iPerUnitItemCount) * (wnd.itemTemplate.width + _m_vCalSpace.x)) + _m_vCalSpace.x + wnd.paddingForSide.x
        //                 , -(((inControlIdx % _m_iPerUnitItemCount) * (wnd.itemTemplate.height + _m_vCalSpace.y)) + _m_vCalSpace.y));
        //         }
        //         else
        //         {
        //             //纵向位置设置
        //             ALUGUICommon.setUIPos(
        //                 _m_wTmpItem.wnd.gameObject
        //                 , ((inControlIdx % _m_iPerUnitItemCount) * (wnd.itemTemplate.width + _m_vCalSpace.x)) + _m_vCalSpace.x
        //                 , -startPos - (((inControlIdx / _m_iPerUnitItemCount) * (wnd.itemTemplate.height + _m_vCalSpace.y)) + _m_vCalSpace.y + wnd.paddingForSide.x));
        //         }
        //
        //         //判断序号是否一致，并刷新显示
        //         if (_m_wTmpItem.itemIdx == _m_iTmpi2)
        ...
        * */
        ...
        private void _addDotItem(int _index)
        {
            //判断对应数据是否有效
            if (null == wnd || wnd.spaceTemplate == null)
                return;

            //实例化一个子窗口对象
            GameObject itemWndObj = GameObject.Instantiate(wnd.spaceTemplate);
            if (null == itemWndObj)
                return;
            ...
            AlternateGridDotItem itemInfo = new AlternateGridDotItem();
            itemInfo.dotGO = itemWndObj;
            itemInfo.index = _index;
            _m_lDotList.Add(itemInfo);
        }

        private void _removeDotItem(AlternateGridDotItem _info)
        {
            if (_info == null)
                return;

            //删除对应的对象
            ALUnityCommon.releaseGameObj(_info.dotGO);
            _m_lDotList.Remove(_info);
        }

        /// <summary>
        /// 检查该下标是否在偏移的行
        /// </summary>
        /// <param name="_index">item下标</param>
        /// <param name="curItemIndexInThisLine">item在当前行位置的下标</param>
        /// <param name="curLineIndex">item在总第几行的下标</param>
        /// <returns></returns>
        private bool _checkIsOffsetLine(int _index, out int curItemIndexInThisLine, out int curLineIndex)
        {
            int roundLeft = (_index + 1) % (_m_iPerUnitItemCount * 2 - 1);
            curLineIndex = (_index + 1) / (_m_iPerUnitItemCount * 2 - 1) * 2;
            curItemIndexInThisLine = 0;
            if (roundLeft == 0)
            {
                curLineIndex--;
                curItemIndexInThisLine = _m_iPerUnitItemCount - 1 - 1;
                return true;
            }
            else if (roundLeft > _m_iPerUnitItemCount)
            {
                curLineIndex++;
                curItemIndexInThisLine = roundLeft - _m_iPerUnitItemCount - 1;
                return true;
            }
            else
            {
                curItemIndexInThisLine = roundLeft - 1;
                return false;
            }
        }
        ...
        /// <param name="_perLineCount"></param>
        /// <returns></returns>
        private int _getOffsetCount(int _totalCount,int _perLineCount)
        {
            int temp1 = _totalCount / (_perLineCount + _perLineCount - 1);
            int temp2 = _totalCount % (_perLineCount + _perLineCount - 1);
            int skewingCount = temp1 + (temp2 > _perLineCount ? 1 : 0);
            return skewingCount;
        }
    ...
    public class AlternateGridDotItem
    {
        private int _m_iIndex;
        private GameObject _m_dotGO;

        public int index
        {
            get { return _m_iIndex; }
            set { _m_iIndex = value; }
        }
