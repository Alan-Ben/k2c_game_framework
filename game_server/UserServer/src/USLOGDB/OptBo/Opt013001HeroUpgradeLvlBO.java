...
public class Opt013001HeroUpgradeLvlBO extends BaseOptLogBo {
    ...
    public static final int FIELD_is_ten =8;
    @DataBaseField(type = "tinyint(1)", fieldname = "is_ten", comment = "十连升级")
    ...
    public Opt013001HeroUpgradeLvlBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        is_ten = rs.getBoolean(10);
        cur_lvl = rs.getInt(11);
        cur_exp = rs.getLong(12);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt013001HeroUpgradeLvlBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `hero_id`, `is_ten`, `cur_lvl`, `cur_exp`";
    }

    @Override
    public String getTableName() {
        return "`opt_013_001_hero_upgrade_lvl`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(is_ten ? 1 : 0).append("', ");
   ...
   @Override
    ...
    // 十连升级
    public boolean getIsTen() { return this.is_ten; }
    public void setIsTen(BM _bm, boolean is_ten) {
        if(is_ten==this.is_ten) 
            return;
        this.is_ten = is_ten; 
        markField(_bm, FIELD_is_ten); 
    }
    public void saveIsTen(BM _bm, boolean is_ten) {
        if(is_ten==this.is_ten) 
            return;
        this.is_ten = is_ten;
        saveField(_bm, "is_ten", is_ten ? 1 : 0);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `is_ten` = '").append(is_ten ? 1 : 0).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_is_ten)) sBuilder.append(" `is_ten` = '").append(is_ten ? 1 : 0).append("',");
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_013_001_hero_upgrade_lvl` ("
                ...
                + "`is_ten` tinyint(1) NOT NULL DEFAULT '0' COMMENT '十连升级',"
                + "`cur_lvl` int(11) NOT NULL DEFAULT '0' COMMENT '现等级',"
                + "`cur_exp` bigint(20) NOT NULL DEFAULT '0' COMMENT '现经验',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='大臣-大臣升级' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
