...
public class MjTravelLogBO extends MJEventLogBo {
    ...
    public static final int FIELD_travel_count =7;
    @DataBaseField(type = "int(11)", fieldname = "travel_count", comment = "游历次数")
    private int travel_count;

    public static final int FIELD_event_id =8;
    @DataBaseField(type = "int(11)", fieldname = "event_id", comment = "事件ID")
    ...
    public MjTravelLogBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        uid = rs.getString(3);
        vip_lv = rs.getInt(4);
        ...
        travel_count = rs.getInt(9);
        event_id = rs.getInt(10);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new MjTravelLogBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `uid`, `vip_lv`, `server_id`, `platform`, `region`, `create_time`, `travel_count`, `event_id`, `timestamp`";
    }

    @Override
    public String getTableName() {
        return "`mj_travel_log`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(travel_count).append("', ");
   ...
   @Override
    ...
    // 游历次数
    public int getTravelCount() { return this.travel_count; }
    public void setTravelCount(BM _bm, int travel_count) {
        if(travel_count==this.travel_count) 
            return;
        this.travel_count = travel_count; 
        markField(_bm, FIELD_travel_count); 
    }
    public void saveTravelCount(BM _bm, int travel_count) {
        if(travel_count==this.travel_count) 
            return;
        this.travel_count = travel_count;
        saveField(_bm, "travel_count", travel_count);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `travel_count` = '").append(travel_count).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_travel_count)) sBuilder.append(" `travel_count` = '").append(travel_count).append("',");
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `mj_travel_log` ("
                ...
                + "`travel_count` int(11) NOT NULL DEFAULT '0' COMMENT '游历次数',"
                + "`event_id` int(11) NOT NULL DEFAULT '0' COMMENT '事件ID',"
                + "`timestamp` int(11) NOT NULL DEFAULT '0' COMMENT '事件发生时间戳(10位)',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='梦加日志-游历次数记录' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
