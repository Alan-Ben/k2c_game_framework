 ...
 * 基础数据调整窗口
 **/
public class EditorWndLong : _IBasicEditorItem
{
    //名称
    ...
    //设置处理
    private Action<long> _m_dOnSetting;

    public EditorWndLong(string _name, long _value, Func<string> _addHead, Action<long> _doneAction)
    {
        _m_sName = _name;
    ...
    public EditorWndLong(string _name, long _value, Func<string> _addHead, int _showWidth, Action<long> _doneAction)
    {
        _m_sName = _name;
        _m_sValue = _value.ToString();
        _m_fAddHeadFunc = _addHead;
    ...
    public void dealFunc()
    {
        //转化值
        long value = 0;
        if (long.TryParse(_m_sValue, out value))
        {
            _m_dOnSetting(value);
