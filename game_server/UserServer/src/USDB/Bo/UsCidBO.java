...
@RefBo(isIdAuto= true)
public class UsCidBO extends BaseBO {
    ...
    public UsCidBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        uid = rs.getString(2);
        cid = rs.getLong(3);
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `us_cid` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`uid` varchar(32) NOT NULL DEFAULT '' COMMENT '玩家uid',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家cid',"
                + "UNIQUE INDEX `uid` (`uid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='us_cid 记录User服器处理cid映射' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
