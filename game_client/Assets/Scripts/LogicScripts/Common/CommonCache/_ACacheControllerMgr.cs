...
namespace GOE
{
    ...
    public abstract class _ACacheControllerMgr<T, TEMP, T_Cache>
        where T_Cache : _AALCacheController<T, TEMP>
    {
        ...
        public _ACacheControllerMgr(string _rootFolderName)
        {            
        ...
        public _ACacheControllerMgr(GameObject _cacheParent)
        {
...
#if UNITY_EDITOR
                Debug.LogWarning($"_ACacheControllerMgr 释放后还有往里push，建议确认一下:{_index}");
#endif
                return;
            }
