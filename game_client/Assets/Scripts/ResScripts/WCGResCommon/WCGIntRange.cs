...
public class WCGIntRange
{
    ...
    public static WCGIntRange readFromStr(string _str)
    {
        //拆分字符串后进行读取
        string[] strs = _str.Split(new string[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        //需要支持物品类型没有配置的情况
        WCGIntRange ret = new WCGIntRange();
        try
        {
            if (strs.Length > 0)
        ...
        {
            Debug.LogError($"解析WCGIntRange时发生错误：{_str}\n{e}");
            return null;
        }
    ...
    /// <returns></returns>
    public static List<WCGIntRange> readList(string _str)
    {
        List<WCGIntRange> list = new List<WCGIntRange>();
        ...
        {
            WCGIntRange newItem = WCGIntRange.readFromStr(strs[i]);
            if (null == newItem)
                continue;
    ...
    /// 为自动导出写的
    /// </summary>
    public static List<WCGIntRange> MakeListFromString(string _str)
    {
    ...
    /// 为自动导出写的
    /// </summary>
    public static WCGIntRange[] MakeArrayFromString(string _str)
    {
