...
namespace Hotfix
{
    ...
    /// 炸弹格子窗口
    /// </summary>
    public class GGUIWndTileMatchCheckerSubBoom : _ATGGUIWndTileMatchCheckerSubBase<GGUIMonoTileMatchCheckerSubBoom>
    {
        public GGUIWndTileMatchCheckerSubBoom([NotNull] GGUIWndTileMatchChecker _blockShow, TileMatchBlockRefObj _blockRefObj, TileMatchBlockShowRefObj _blockShowRefObj, MonoSkin _wnd) : base(_blockShow, _blockRefObj, _blockShowRefObj, _wnd)
        {
            initWnd();
        }
        ...
        public override void proactiveUniteBoomTriggerShow(Vector2Int _uniteBlockLogicPos, Action _complete)
        {
            if (_m_gameShow != null && hotfixWnd != null && _m_blockShow.rectTransform != null)
            {
                _m_gameShow.playCheckerboardSfx(hotfixWnd.proactiveUniteBoomTriggerCheckerboardClearSfxId, _m_blockShow.rectTransform.localPosition, 0, null);
                _complete?.Invoke();
            }
            else
            {
