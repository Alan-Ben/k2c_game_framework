 ...
 * 已触发的邀约剧情ID推送
 **/
public class GS2GC_015_054_OnTriggeredCallDlgIdAdd implements ALBasicProtocolPack._IALProtocolStructure {
/** 空 */
...
public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	...
	if(_buf.remaining() > 0) consortId = _buf.getLong();
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
	if(_buf.remaining() > 0) triggeredCallStroryId = _buf.getLong();
}
