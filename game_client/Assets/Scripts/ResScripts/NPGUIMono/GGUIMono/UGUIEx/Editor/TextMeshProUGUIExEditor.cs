...
namespace GOE
{
    ...
    public class TextMeshProUGUIExEditor : TMP_EditorPanelUI
    {
        ...
        protected TextMeshProUGUIEx _m_TextMeshProUGUIEx;
        ...
        protected override void OnEnable()
        {
            ...
            _m_TextMeshProUGUIEx = target as TextMeshProUGUIEx;
            _m_initFontAsset = _m_TextMeshProUGUIEx == null ? null : _m_TextMeshProUGUIEx.font;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                // 检查字体是否被修改, 若被修改则恢复
                if (_m_TextMeshProUGUIEx != null && _m_TextMeshProUGUIEx.font != _m_initFontAsset)
                {
                    Debug.LogError_EditorOnly("TextMeshProUGUIEx 不允许手动修改FontAsset, 请通过变换FontType或使用语言修改需要使用的FontAsset", _m_TextMeshProUGUIEx);
                    _setFontAssetWithChgInitFont(_m_initFontAsset);//恢复原始字体
                    EditorUtility.SetDirty(target);
                }
                
                // 检查材质是否被修改, 若被修改同步改动InitMaterialTag
                if (m_InitMaterialTagProp != null && _m_TextMeshProUGUIEx != null && _m_TextMeshProUGUIEx.fontSharedMaterial != null)
                {
                    string initMaterialTag = m_InitMaterialTagProp.stringValue;
                    if (string.IsNullOrEmpty(initMaterialTag) || !_m_TextMeshProUGUIEx.fontSharedMaterial.name.EndsWith(initMaterialTag))
                    {
#if NP_GAME
                        m_InitMaterialTagProp.stringValue = TMP_FontAssetMaterialObj.getTMPMaterialTag(_m_TextMeshProUGUIEx.fontSharedMaterial);
#else
                        _m_TextMeshProUGUIEx.fontSharedMaterial.name.Split(new char[] {' '}).LastOrDefault()?.Replace("(Clone)", "");
#endif
        ...
        private void _chgFont()
        {
            ...
            if (m_FontTypeProp == null || _m_TextMeshProUGUIEx == null)
            {
                Debug.LogError_EditorOnly("_m_TextMeshProUGUIEx or m_FontTypeProp is null", _m_TextMeshProUGUIEx);
                return;
            }
            ...
            {
                Debug.LogError_EditorOnly($"没有找到{NPGSOGameCommonInfo.objName}或者配置的TMPFontTypeAssetPathList为空, 将使用TMP_Settings中配置的默认字体", _m_TextMeshProUGUIEx);
            ...
            {
                Debug.LogError_EditorOnly($"没有找到语言:{textUseLanguage}对应的LanguageFontTypeAssetPath, 请检查{NPGSOGameCommonInfo.objName}配置, 这里将会默认使用{ENPLanguage.NONE}配置", _m_TextMeshProUGUIEx);
            ...
            {
                Debug.LogError_EditorOnly($"没有找到:{ENPLanguage.NONE}配置的默认字体, 请检查{NPGSOGameCommonInfo.objName}配置, 这里将会使用{ENPLanguage.EN_US}的字体配置", _m_TextMeshProUGUIEx);
            ...
            {
                Debug.LogError_EditorOnly($"没有找到语言:{ENPLanguage.EN_US}对应的LanguageFontTypeAssetPath, 请检查{NPGSOGameCommonInfo.objName}配置, 这里将会默认使用TMPFontTypeAssetPathList列表第一个元素数据", _m_TextMeshProUGUIEx);
                fontTypeAssetPath = commonInfo.TMPFontTypeAssetPathList.GetFirst();
            }

            if (fontTypeAssetPath == null)
            {
                Debug.LogError_EditorOnly($"没有可用的字体配置, 请检查{NPGSOGameCommonInfo.objName}配置", _m_TextMeshProUGUIEx);
            ...
            {
                Debug.LogError_EditorOnly($"获取Font资源路径失败, 请检查{NPGSOGameCommonInfo.objName}配置", _m_TextMeshProUGUIEx);
            ...
            if (fontAsset == null)
            {
                Debug.LogError_EditorOnly($"加载TMP_FontAsset资源:{fontAssetPath}失败", _m_TextMeshProUGUIEx);
        ...
        {
            if(_m_TextMeshProUGUIEx == null)
                return;

            _m_TextMeshProUGUIEx.font = _fontAsset;
            _m_initFontAsset = _m_TextMeshProUGUIEx.font;//因为set_font方法中当font为null时有赋默认值操作, 所以这里初始值要使用get_font获取
        }
        ...
        private void _resetMaterialToInit()
        {
            ...
            if (_m_TextMeshProUGUIEx == null || m_InitMaterialTagProp == null)
            {
                Debug.LogError_EditorOnly($"_m_TextMeshProUGUIEx == null || m_InitMaterialTagProp == null", _m_TextMeshProUGUIEx);
                return;
            }
            ...
            {
                Debug.LogError_EditorOnly($"string.IsNullOrEmpty(initMaterialTag)", _m_TextMeshProUGUIEx);
                return;
            }

            // 若当前材质已经是InitMaterial同类型材质, 则不需要重置, 通过fontSharedMaterial获取材质, 不会创建新的材质实例
            if (_m_TextMeshProUGUIEx.fontSharedMaterial != null && _m_TextMeshProUGUIEx.fontSharedMaterial.name.EndsWith(initMaterialTag))
                return;
            
            Material[] materials = TMP_EditorUtility.FindMaterialReferences(_m_TextMeshProUGUIEx.font);//获取所有可用材质
            if (materials == null)
            {
                Debug.LogError_EditorOnly($"[TextMeshProUGUIExEditor _resetMaterialToInit] 想要重置材质为初始类型:{initMaterialTag}, 但是FontAsset:{_m_TextMeshProUGUIEx.font} 中找不到对应类型的材质", _m_TextMeshProUGUIEx);
                return;
            }
            ...
            foreach (Material material in materials)
            {
                ...
                {
                    _m_TextMeshProUGUIEx.fontMaterial = material;//通过fontMaterial属性设置材质, 不会实例化新的材质
                    EditorUtility.SetDirty(target);
                    return;
                }
            }
