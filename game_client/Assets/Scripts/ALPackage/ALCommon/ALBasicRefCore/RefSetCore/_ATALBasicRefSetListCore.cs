...
namespace ALPackage
{
     ...
     * 数据集合中心管理对象
     **/
    public abstract class _ATALBasicRefSetListCore<T> : _TALBasicRefMapListCore<T>, _IALInitRefObj where T : _IALBasicRefObj
    {
        /** 初始化状态位 */
        ...
        protected _ATALBasicRefSetListCore() 
        {
            _m_bInit = false;
        }
        ...
        public void _onLocalObjLoaded(UnityEngine.Object _obj)
        {
            ...
            //获取对象
            _TALSOBasicRefSet<T> refSetObj = (_TALSOBasicRefSet<T>)_obj;
            if(null == refSetObj)
            {
#if UNITY_EDITOR
                Debug.LogWarning(ToString() + " Basic Ref Load err!");
