...
@RefBo(isIdAuto= true)
public class PlayerStoreReviewsBO extends BaseBO {
    ...
    @DataBaseField(type = "text", fieldname = "content", comment = "评价内容")
    private String content;

    public PlayerStoreReviewsBO() {
        id = 0;
        cid = 0L;
        content = "";
    }

    public PlayerStoreReviewsBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new PlayerStoreReviewsBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `content`";
    }

    @Override
    public String getTableName() {
        return "`player_store_reviews`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_store_reviews` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家CID',"
                + "`content` text NULL COMMENT '评价内容',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家商店评价数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
