...
import RCSDB.Bo.NpRsParamBO;

import java.util.Hashtable;
import java.util.List;

public class NPRCSParams
{
    static NPRCSParams _g_instance = new NPRCSParams();
    //参数保存表
    private final Hashtable<Integer, NpRsParamBO> _m_htParamTable;
    //锁对象
    private final MutexAtom _m_mutex;

    protected NPRCSParams()
    {
        _m_htParamTable = new Hashtable<Integer, NpRsParamBO>();
        _m_mutex = new MutexAtom();
    }

    public static NPRCSParams getInstance()
    {
        if (null == _g_instance)
            _g_instance = new NPRCSParams();
    ...
    public boolean initFromDB()
    {
        try
        {
            List<NpRsParamBO> bos = NPRecordServer.getInstance().getBM().getBM(NpRsParamBO.class).s_findAll();
            if (null == bos)
            {
                return false;
            }
            ...
            for (int i = 0; i < bos.size(); i++)
            {
                ...
                if (_m_htParamTable.containsKey(bos.get(i).getTag()))
                {
                    ...
                    //删除数据
                    bos.get(i).del(NPRecordServer.getInstance().getBM());
    ...
    public void setParam(int _tag, long _value)
    {
        ...
        try
        {
            NpRsParamBO valueBo = _m_htParamTable.get(_tag);

            BM bmObj = NPRecordServer.getInstance().getBM();
            if (null == valueBo)
            {
                //创建新数据
                NpRsParamBO newBo = new NpRsParamBO();
