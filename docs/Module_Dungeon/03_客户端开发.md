...
/// 副本UI延迟加载管理
/// </summary>
public class DungeonUILazyLoader : MonoBehaviour
{
    [SerializeField] private GameObject[] _lazyPrefabs;
    [SerializeField] private Transform[] _lazyContainers;
    [SerializeField] private float _delayTime = 0.1f;

    private Dictionary<int, GameObject> _loadedUI = new Dictionary<int, GameObject>();

    public void PreloadAsync(int index, Action<GameObject> onComplete = null)
    {
        if (index < 0 || index >= _lazyPrefabs.Length) return;

        if (_loadedUI.ContainsKey(index))
        {
            onComplete?.Invoke(_loadedUI[index]);
            return;
        }

        StartCoroutine(LoadCoroutine(index, onComplete));
    }

    private IEnumerator LoadCoroutine(int index, Action<GameObject> onComplete)
    {
        yield return new WaitForSeconds(_delayTime);

        var prefab = _lazyPrefabs[index];
        var container = _lazyContainers[index];

        var instance = Instantiate(prefab, container);
        instance.SetActive(false);
        _loadedUI[index] = instance;

        onComplete?.Invoke(instance);
    }

    public T GetUI<T>(int index) where T : Component
    {
        if (_loadedUI.TryGetValue(index, out var obj))
        {
...
/// 副本配置数据缓存
/// </summary>
public class DungeonConfigCache : MonoBehaviour
{
    private static DungeonConfigCache _instance;
    public static DungeonConfigCache Instance => _instance;

    // 配置缓存
    private Dictionary<int, MiddayDungeonRefObj> _middayCache = new Dictionary<int, MiddayDungeonRefObj>();
    private Dictionary<int, EveningDungeonBossRefObj> _bossCache = new Dictionary<int, EveningDungeonBossRefObj>();
    private Dictionary<int, List<MiddayDungeonBoxRefObj>> _boxCache = new Dictionary<int, List<MiddayDungeonBoxRefObj>>();

    // 缓存过期时间
    private const float CACHE_EXPIRE_TIME = 300f; // 5分钟
    ...
    private void Update()
    {
        if (Time.time - _lastCacheTime > CACHE_EXPIRE_TIME)
        {
            RefreshCache();
        }
    }
    ...
    private void RefreshCache()
    {
        // 刷新午间副本缓存
        _middayCache.Clear();
        var middayList = GRefdataCoreMgr.instance.middayDungeonRefCore.getAll();
        foreach (var item in middayList)
        {
            _middayCache[item.dungeon_id] = item;
        }

        // 刷新Boss缓存
        _bossCache.Clear();
        var bossList = GRefdataCoreMgr.instance.eveningDungeonBossRefCore.getAll();
        foreach (var item in bossList)
        {
            _bossCache[item.boss_id] = item;
        }

        _lastCacheTime = Time.time;
    }

    public MiddayDungeonRefObj GetMiddayDungeon(int dungeonId)
    {
        return _middayCache.TryGetValue(dungeonId, out var dungeon) ? dungeon : null;
    }

    public EveningDungeonBossRefObj GetBoss(int bossId)
    {
        return _bossCache.TryGetValue(bossId, out var boss) ? boss : null;
    }

    public List<MiddayDungeonBoxRefObj> GetBoxes(int dungeonId)
    {
        if (!_boxCache.TryGetValue(dungeonId, out var boxes))
        {
            boxes = GRefdataCoreMgr.instance.middayDungeonBoxRefCore.getListByDungeonId(dungeonId);
            _boxCache[dungeonId] = boxes;
        }
        return boxes;
    }
}
...
/// 批量伤害更新管理器
/// </summary>
public class BatchDamageUpdateManager : MonoBehaviour
{
    private List<DamageUpdateTask> _pendingTasks = new List<DamageUpdateTask>();
    private const float UPDATE_INTERVAL = 0.1f; // 100ms批量更新一次

    private float _timer;

    public void AddDamageUpdate(long bossId, long damage, long playerId)
    {
        _pendingTasks.Add(new DamageUpdateTask
        {
            BossId = bossId,
            Damage = damage,
            PlayerId = playerId,
    ...
    private void Update()
    {
        ...
        if (_timer >= UPDATE_INTERVAL && _pendingTasks.Count > 0)
        {
            ProcessBatchUpdate();
            _timer = 0;
        }
    ...
    private void ProcessBatchUpdate()
    {
        ...
        // 按Boss分组
        var groupedByBoss = _pendingTasks.GroupBy(t => t.BossId);

        foreach (var group in groupedByBoss)
        {
            long totalDamage = group.Sum(t => t.Damage);
            UpdateBossHp(group.Key, totalDamage);
        }

        _pendingTasks.Clear();
    }

    private void UpdateBossHp(long bossId, long totalDamage)
    {
        // 更新UI显示
        WinMsg.SendMsg(WinMsgType.ON_BATCH_DAMAGE_UPDATE, bossId, totalDamage);
    }

    private struct DamageUpdateTask
    {
        public long BossId;
        public long Damage;
