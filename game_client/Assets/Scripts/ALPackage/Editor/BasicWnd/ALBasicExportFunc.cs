...
namespace ALPackage
{
    public class ALBasicExportFunc
    {
        #region 类定义
        ...
        public static Type C_T_Color32 = typeof(Color32);
        ...
        public static TTemp AutoRead<TTemp>(Dictionary<string, string> _lineData, string _errMsg = "")
        {
            ...
            //开始读取
            System.Reflection.FieldInfo tmpField = null;
            string tmpValue = string.Empty;
            object[] param = new object[1];
            TTemp readObj = System.Activator.CreateInstance<TTemp>();
            for(int i = 0; i < fields.Length; i++)
            {
                ...
                {
                    UnityEngine.Debug.LogError("类： " + TempCls.Name + " 的变量： " + tmpField.Name + " 无法读取到数据: " + _errMsg);
                    continue;
                }
            }
        ...
        /// <returns></returns>
        public static bool AutoReadField(System.Reflection.FieldInfo _fieldInfo, object _obj, Dictionary<string, string> _lineData, string _errMsg = "")
        {
            if(null == _fieldInfo || null == _obj)
                return false;

            //无数据则获取属性判断，如果可为空则不做处理
            object[] objs = _fieldInfo.GetCustomAttributes(typeof(ALAutoExportVariableAttr), false);
            ALAutoExportVariableAttr fieldAtt = null;
            if(objs.Length > 0)
                fieldAtt = (ALAutoExportVariableAttr)objs[0];
            ...
            if(!_lineData.TryGetValue(_fieldInfo.Name.ToLowerInvariant(), out tmpValue) || tmpValue.Trim().Length <= 0)
            {
                ...
                if(null != fieldAtt)
                {
                    ...
                    {
                        UnityEngine.Debug.LogError("类： " + _obj.GetType().Name + " 的变量： " + _fieldInfo.Name + " 无法查询到数据: " + _errMsg);
                        return false;
                    }
            ...
            if(_fieldInfo.FieldType.IsArray || HasImplementedRawGeneric(_fieldInfo.FieldType, C_T_List))
            {
                ...
                if(!tmpValue.Trim().Equals(""))
                {
                    ...
                    for(int i = 0; i < strs.Length; i++)
                    {
                        ...
                        {
                            value = System.Activator.CreateInstance(singleType, true);//没有传true的话，没有public构造函数的对象无法反射创建
                        ...
                        if(!readRes)
                        {
                            UnityEngine.Debug.LogError("类： " + _obj.GetType().Name + " 的变量： " + _fieldInfo.Name + " 读取失败: " + _errMsg);
                            return false;
                        }
                    ...
                    {
                        //创建队列
                        object listObj = System.Activator.CreateInstance(_fieldInfo.FieldType, true);//没有传true的话，没有public构造函数的对象无法反射创建
                        System.Reflection.MethodInfo method = _fieldInfo.FieldType.GetMethod("AddRange");
                        //调用函数
                        object[] param = new object[1];
                        param[0] = arrObj;
            ...
            else
            {
                ...
                //使用自定义读取方式
                object readObj = System.Activator.CreateInstance(_fieldInfo.FieldType, true);//没有传true的话，没有public构造函数的对象无法反射创建
                if(!readCustomSingleFieldValue(_fieldInfo.FieldType, readObj, tmpValue, _errMsg))
                {
                    UnityEngine.Debug.LogError("类： " + _obj.GetType().Name + " 的变量： " + _fieldInfo.Name + " 对应结构体没有实现ParseFromString函数: " + _errMsg);
                    return false;
                }
        ...
        public static bool readCustomFieldList(Type _singleType, string _value, out object _returnList, string _errMsg = "")
        {
            ...
            System.Reflection.MethodInfo readFuncInfo = _singleType.GetMethod("MakeListFromString", 
                System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            if(null != readFuncInfo)
            {
                //调用函数读取
                invokeObj = System.Activator.CreateInstance(_singleType, true);//没有传true的话，没有public构造函数的对象无法反射创建
            }
            else
            {
        ...
        {
            object invokeObj = null;
            System.Reflection.MethodInfo readFuncInfo = _singleType.GetMethod("MakeArrayFromString", 
                System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            if(null != readFuncInfo)
            {
                //调用函数读取
        ...
        {
            System.Reflection.MethodInfo readFuncInfo = _singleType.GetMethod("ParseFromString", _g_arrParseArg);
            if(null == readFuncInfo)
            {
                UnityEngine.Debug.LogError("类： " + _singleType.Name + " 没有实现ParseFromString函数");
                return false;
            }
        ...
        public static bool ParseValue(string _value, Type _type, ref object _obj, string _errMsg = "")
        {
            try
            {
                ...
                if(_value.Equals(string.Empty))
                {
                    ...
                    else if (_type == C_T_Color32)
                    {
                        _obj = (Color32) Color.black;
                        return true;
                    }
                ...
                else
                {
                    ...
                    else if(_type == C_T_Float)
                    {
                        if(_value == "0" || _value == "" || _value == string.Empty)
                            ...
                            _obj = ALCommon.ParseFloat(_value);
                    ...
                    {
                        _obj = ALCommon.GetRect(_value, _errMsg);

                        return true;
                    }
                    else if(_type == C_T_Vector2)
                    {
                        _obj = ALCommon.GetVector2(_value, _errMsg);

                        return true;
                    }
                    else if(_type == C_T_Vector3)
                    {
                        _obj = ALCommon.GetVector3(_value, _errMsg);

                        return true;
                    }
                    else if(_type == C_T_Vector2Int)
                    {
                        _obj = ALCommon.GetVector2Int(_value, _errMsg);

                        return true;
                    }
                    else if(_type == C_T_Vector3Int)
                    {
                        _obj = ALCommon.GetVector3Int(_value, _errMsg);

                        return true;
                    }
                    else if(_type == C_T_Color)
                    {
                        _obj = ALCommon.GetColor(_value);

                        return true;
                    }
                    else if(_type == C_T_Color32)
                    {
                        _obj = ALCommon.GetColor32(_value);

                        return true;
                    }
                    else if (_type == C_T_Vector4)
                    {
                        _obj = ALCommon.GetVector4(_value, _errMsg);
                        return true;
                    }
                    else
                    {
