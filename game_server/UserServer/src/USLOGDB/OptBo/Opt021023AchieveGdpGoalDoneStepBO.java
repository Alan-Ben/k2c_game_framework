...
public class Opt021023AchieveGdpGoalDoneStepBO extends BaseOptLogBo {
    ...
    public static final int FIELD_achieve_id =7;
    ...
    @DataBaseField(type = "int(11)", fieldname = "step", comment = "成就步骤")
    private int step;

    public Opt021023AchieveGdpGoalDoneStepBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt021023AchieveGdpGoalDoneStepBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt021023AchieveGdpGoalDoneStepBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `achieve_id`, `step`";
    }

    @Override
    public String getTableName() {
        return "`opt_021_023_achieve_gdp_goal_done_step`";
    }
   ...
   @Override
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_021_023_achieve_gdp_goal_done_step` ("
                ...
                + "`step` int(11) NOT NULL DEFAULT '0' COMMENT '成就步骤',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家完成成就（国力目标）阶段' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
