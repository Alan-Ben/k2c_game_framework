...
@RefBo(isIdAuto= true)
public class ActivityPlayerCrystalGiftPackInfoBO extends BaseBO {
    ...
    public ActivityPlayerCrystalGiftPackInfoBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        instance_id = rs.getLong(2);
        group_id = rs.getLong(3);
        cid = rs.getLong(4);
        next_refresh_time_ms = rs.getLong(5);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new ActivityPlayerCrystalGiftPackInfoBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `instance_id`, `group_id`, `cid`, `next_refresh_time_ms`";
    }

    @Override
    public String getTableName() {
        return "`activity_player_crystal_gift_pack_info`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `activity_player_crystal_gift_pack_info` ("
                ...
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='活动玩家礼包信息' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
