...
public class Opt041007MarsExploreTeamStartRepairBO extends BaseOptLogBo {
    ...
    public static final int FIELD_repairNum =8;
    @DataBaseField(type = "bigint(20)", fieldname = "repairNum", comment = "修复伤兵数量 0-全部")
    private long repairNum;

    public Opt041007MarsExploreTeamStartRepairBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt041007MarsExploreTeamStartRepairBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        teamId = rs.getLong(9);
        repairNum = rs.getLong(10);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt041007MarsExploreTeamStartRepairBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `teamId`, `repairNum`";
    }

    @Override
    public String getTableName() {
        return "`opt_041_007_mars_explore_team_start_repair`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(repairNum).append("', ");
   ...
   @Override
    ...
    // 修复伤兵数量 0-全部
    public long getRepairNum() { return this.repairNum; }
    public void setRepairNum(BM _bm, long repairNum) {
        if(repairNum==this.repairNum) 
            return;
        this.repairNum = repairNum; 
        markField(_bm, FIELD_repairNum); 
    }
    public void saveRepairNum(BM _bm, long repairNum) {
        if(repairNum==this.repairNum) 
            return;
        this.repairNum = repairNum;
        saveField(_bm, "repairNum", repairNum);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `repairNum` = '").append(repairNum).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_repairNum)) sBuilder.append(" `repairNum` = '").append(repairNum).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_041_007_mars_explore_team_start_repair` ("
                ...
                + "`repairNum` bigint(20) NOT NULL DEFAULT '0' COMMENT '修复伤兵数量 0-全部',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='火星-火星探险队伍开启维修' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
