...
{
    /// <summary>
    /// UserMessagingPlatform(google官方cmp用户信息平台)
    /// </summary>
    public class MJSDK_UMPLib
    {
        //组件标识符  --- 库名
        private static string _g_MJSDK_LibName = "MJSDK_UMP";
        //组件版本信息
        ...
        /// <summary>
        /// ump-show：展示用户信息平台cmp
        /// </summary>
        /// <param name="_sucDelegate"></param>
        /// <param name="_failDelegate"></param>
        [UnityEngine.Scripting.Preserve]
        public static void ump_show(Action<string> _sucDelegate, Action<int, string> _failDelegate)
        {
            ...
            MJSDK.sendMsgToPhonePlatform("ump", "show", "", new MJSDK_CallbackDealer_Action(_sucDelegate, _failDelegate));
        }

        /// <summary>
        /// ump-isPrivacy：是否再次向用户展示（cmp）
        /// </summary>
        /// <param name="_sucDelegate"></param>
        /// <param name="_failDelegate"></param>
        [UnityEngine.Scripting.Preserve]
        public static void ump_isPrivacy(Action<string> _sucDelegate, Action<int, string> _failDelegate)
        {
            if (!MJSDK_Tool_Lib.checkInitState(_g_MJSDK_LibName, _g_isInit, _failDelegate))
            {
                return;
            ...
            MJSDK.sendMsgToPhonePlatform("ump", "isPrivacy", "", new MJSDK_CallbackDealer_Action(_sucDelegate, _failDelegate));
        }
    }
}
