...
namespace GOE
{
	...
	/// 保存信息在本地
	/// </summary>
	public class NPExportSettingMgr 
	{
	    private static string exportSettingKey = "cache_export_setting_";
	    private static NPExportSettingMgr _g_instance;
	    public static NPExportSettingMgr instance 
	    {
	        get
	        {
	            if (null == _g_instance)
	                _g_instance = new NPExportSettingMgr();
	            return _g_instance;
	        }
	    }
	    ...
	    public Action<ENPExportSettingEnum, bool> onSubExportMenuSelectChg;//单项选择改变

	    //没有意义的临时 变量
	    private NPExportSettingData tmpSettingData = null;

	    public NPExportSettingMgr()
	    {
	    ...
	    public void regSubExportSetting (ENPExportSettingEnum _type, Action _action, bool _isOn, string _tag, Func<string, string, bool> _judgeCanShowFunc) {
	    ...
	    public void loadLocalSave()
	    {
	        ...
	        for (int i = 0; i < strs.Length; ++i)
	        {
	            ...
	            //根据枚举获取配置的数据，设置为当前值
	            setSubExportIsSelect((ENPExportSettingEnum)enumValue, true);
	        }
	    }

	    //根据枚举获取对应的导出配置数据
	    public NPExportSettingData getExportSettingData (ENPExportSettingEnum _type) {
	        for (int i = 0; i < _m_lExportSettingList.Count; ++i) {
	    ...
	    //设置单项导出是否被选中
	    public void setSubExportIsSelect (ENPExportSettingEnum _type, bool _isOn) {
	        tmpSettingData = getExportSettingData(_type);
	        if (tmpSettingData == null)
	            return;
	...
	public class NPExportSettingData {
	    private ENPExportSettingEnum _m_eType;
	    ...
	    public ENPExportSettingEnum type { get { return _m_eType; } }
	    ...
	    public NPExportSettingData(ENPExportSettingEnum _enumType, Action _action, bool _isSelect, string _tag, Func<string, string, bool> _judgeCanShowFunc)
	    {
	        setInfo(_enumType, _action, _isSelect, _tag, _judgeCanShowFunc);
	    }

	    public void setInfo(ENPExportSettingEnum _enumType, Action _action, bool _isSelect, string _tag, Func<string, string, bool> _judgeCanShowFunc)
	    {
	        _m_eType = _enumType;
	    ...
	    {
	        string path = ALExportDataCore.instance.getValue(_m_eType.ToString());
