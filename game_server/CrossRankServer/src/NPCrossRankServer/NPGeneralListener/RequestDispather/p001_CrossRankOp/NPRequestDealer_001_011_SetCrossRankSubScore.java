...
import NP2CRS_R.p001_CrossRankOp.NP2CRS_R_001_011_SetCrossRankSubScore;
...
public class NPRequestDealer_001_011_SetCrossRankSubScore extends NPRequestDealer<NP2CRS_R_001_011_SetCrossRankSubScore>
{
    @Override
    protected void _dealMessage(_IWCGBasicRequestCommiter _commiter, NP2CRS_R_001_011_SetCrossRankSubScore _msg)
    {
        //查询跨服排行集合信息
        ...
        {
            CommLog.error("set rank sub score:[{}][{}]-[{}] to cross rank rankid:[{}] of cross instance:[{}] fail! can not find cross instance"
                    , _msg.getObjId(), _msg.getSubObjId(), _msg.getScore(), _msg.getRankId(), _msg.getCrossInstanceId());
        ...
        if(null == rankList)
        {
            CommLog.error("set rank sub score:[{}][{}]-[{}] to cross rank rankid:[{}] of cross instance:[{}] fail! can not find rank list for:[{}]"
                    , _msg.getObjId(), _msg.getSubObjId(), _msg.getScore(), _msg.getRankId(), _msg.getCrossInstanceId(), rankInstanceId);
        ...
        boolean res = rankList.setSubScoreCompareUpdateTime(_msg.getObjId(), _msg.getSubObjId(), _msg.getScoreSourceId(), _msg.getScore(), _msg.getUpdateTimeMs(), context);
        if(!res)
        {
            CommLog.error("set rank sub score:[{}][{}]-[{}] to cross rank rankid:[{}] of cross instance:[{}] fail! set score op fail for rank list:[{}]"
                    , _msg.getObjId(), _msg.getSubObjId(), _msg.getScore(), _msg.getRankId(), _msg.getCrossInstanceId(), rankInstanceId);
            _commiter.commitFailRes(RankErr.RANK_OP_FAIL.getCode());
            return;
        }
        
        _commiter.commitSucRes(NPWriter_001_CrossRankOp.make_011_SetCrossRankSubScore());
    }
}
