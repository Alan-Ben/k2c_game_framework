...
namespace GOE
{
    ...
    /// 一个可以被 cache 回收的类
    /// </summary>
    public abstract class _ACacheClass
    {
        /// <summary>
        /// 被 cache 回收时调用
        /// </summary>
    ...
    /// 适用于某个类需要频繁的 new
    /// </remarks>
    public abstract class _ACacheClass<T> : _ACacheClass
        where T : _ACacheClass, new()
    {
        // 内部的静态缓存
        ...
        public static T pop()
        {
            if (_g_cache == null)
            {
                ALLog.Error($"{typeof(T).FullName}.pop 时，没有 cache 存在哦，先调用 {typeof(T).FullName}.createCache 方法，再使用这个方法");
                return new T();
            }
        ...
        public static void createCache(int _initCount, int _maxCount)
        {
            if (_g_cache != null)
            {
                ALLog.Error($"{typeof(T).FullName}.createCache 时，已经存在 cache 了，需要先调用 {typeof(T).FullName}.discardCache 再调用这个方法");
                return;
            }
        ...
        /// 内部的缓存类
        /// </summary>
        private class Cache : _AALUnsafeCacheController<T, T>
        {
