...
@RefBo(isIdAuto= true)
public class PlayerClockRewardBO extends BaseBO {
    ...
    @DataBaseField(type = "bigint(20)", fieldname = "ref_id", comment = "定时奖励配置id")
    private long ref_id;

    public static final int FIELD_next_refresh_time_ms =2;
    @DataBaseField(type = "bigint(20)", fieldname = "next_refresh_time_ms", comment = "下一次发放奖励的时间")
    private long next_refresh_time_ms;

    public static final int FIELD_num =3;
    @DataBaseField(type = "int(11)", fieldname = "num", comment = "已发放次数")
    private int num;

    public PlayerClockRewardBO() {
        id = 0;
        cid = 0L;
        ref_id = 0L;
    ...
    public PlayerClockRewardBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        ref_id = rs.getLong(3);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new PlayerClockRewardBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `ref_id`, `next_refresh_time_ms`, `num`";
    }

    @Override
    public String getTableName() {
        return "`player_clock_reward`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_clock_reward` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家CID',"
                + "`ref_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '定时奖励配置id',"
                + "`next_refresh_time_ms` bigint(20) NOT NULL DEFAULT '0' COMMENT '下一次发放奖励的时间',"
                + "`num` int(11) NOT NULL DEFAULT '0' COMMENT '已发放次数',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家定时奖励数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
