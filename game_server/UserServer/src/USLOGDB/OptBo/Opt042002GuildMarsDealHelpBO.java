...
public class Opt042002GuildMarsDealHelpBO extends BaseOptLogBo {
    ...
    @DataBaseField(type = "bigint(20)", fieldname = "guildId", comment = "玩家所在联盟ID")
    private long guildId;

    public static final int FIELD_dealCount =8;
    @DataBaseField(type = "int(11)", fieldname = "dealCount", comment = "处理数量")
    private int dealCount;

    public Opt042002GuildMarsDealHelpBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt042002GuildMarsDealHelpBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        dealCount = rs.getInt(10);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt042002GuildMarsDealHelpBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `guildId`, `dealCount`";
    }

    @Override
    public String getTableName() {
        return "`opt_042_002_guild_mars_deal_help`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(dealCount).append("', ");
   ...
   @Override
    ...
    // 处理数量
    public int getDealCount() { return this.dealCount; }
    public void setDealCount(BM _bm, int dealCount) {
        if(dealCount==this.dealCount) 
            return;
        this.dealCount = dealCount; 
        markField(_bm, FIELD_dealCount); 
    }
    public void saveDealCount(BM _bm, int dealCount) {
        if(dealCount==this.dealCount) 
            return;
        this.dealCount = dealCount;
        saveField(_bm, "dealCount", dealCount);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `dealCount` = '").append(dealCount).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_dealCount)) sBuilder.append(" `dealCount` = '").append(dealCount).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_042_002_guild_mars_deal_help` ("
                ...
                + "`guildId` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家所在联盟ID',"
                + "`dealCount` int(11) NOT NULL DEFAULT '0' COMMENT '处理数量',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='处理联盟互助' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
