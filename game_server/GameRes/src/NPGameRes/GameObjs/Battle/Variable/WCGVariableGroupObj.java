 ...
 * 参数对象序列号结构体
 **/
public class WCGVariableGroupObj
{
    public enum EWCGVariableCalType
    {
        NONE,
        ADD,        //加
        SUB,        //减
     ...
     * 本对象的条件类型
     */
    public WCGVariableObj variableObj;
    public List<WCGVariableGroupObj> childGroupList;
    public EWCGVariableCalType calType = EWCGVariableCalType.NONE;

    protected WCGVariableGroupObj()
    {
        variableObj = null;
        childGroupList = new ArrayList<WCGVariableGroupObj>();
    }

    public boolean hasVariable()
    {
        return (variableObj != null && variableObj.variableType != EWCGVariableType.NONE)
    ...
    //将字符串转化为本对象
    public static WCGVariableGroupObj readVariableGroup(String _str, String _errHeader)
    {
        return readVariableGroup(_str, 0, 1, _errHeader);
    }

    public static WCGVariableGroupObj readVariableGroup(String _str, int _startIdx, int _depth, String _err)
    {
        if (_depth > 5)
        {
            CommLog.error(_err);
        ...
        WCGVariableGroupObj obj = new WCGVariableGroupObj();
        //下一个读取对象
        WCGVariableGroupObj newGroupObj = new WCGVariableGroupObj();
        ...
        do
        {
            ...
            if (index == startIdx)
            {
                ...
                if (tmpChar == '(')
                {
                    ...
                    WCGVariableGroupObj newObj = readVariableGroup(infoStr.toString(), 0, _depth + 1, _err);
                    if (null != newObj)
                    {
            ...
            } else if (index != -1)//此时取出字符串进行条件读取
            {
                ...
                newGroupObj.variableObj = WCGVariableObj.readVariable(infoStr.toString());
                //加入队列
                obj.childGroupList.add(newGroupObj);
                //创建新的对象
                newGroupObj = new WCGVariableGroupObj();
