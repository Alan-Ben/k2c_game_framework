...
namespace ALPackage
{
    ...
    /// </remarks>
    public abstract class _ATALStateFactory<STATE_T, T> where T : Enum where STATE_T : _ATALStateBase<T>
    {
        //缓存不同类型以及对应类型的状态构造对象
        private Dictionary<Type, _TALBasicStateCacheController<STATE_T, T>> _m_dicStateCacheDic;

        public _ATALStateFactory()
        {
            _m_dicStateCacheDic = new Dictionary<Type, _TALBasicStateCacheController<STATE_T, T>>();
        }
        ...
        /// <param name=""></param>
        /// <param name=""></param>
        public void regCacheController(Type _type, _TALBasicStateCacheController<STATE_T, T> _cacheController)
        {
        ...
        {
            _TALBasicStateCacheController<STATE_T, T> cacheController;
