...
@RefBo(isIdAuto= true)
public class PlayerGuildBoxFreeBO extends BaseBO {
    ...
    public PlayerGuildBoxFreeBO() {
        id = 0;
        cid = 0L;
        box_data = null;
    }

    public PlayerGuildBoxFreeBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new PlayerGuildBoxFreeBO(rs));
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_guild_box_free` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家CID',"
                + "`box_data` blob NULL COMMENT '联盟宝箱数据',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家联盟宝箱（免费宝箱）数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
