...
{
    /// <summary>
    /// 默认构造的一个带入Model(json模板)可以快速定义处理函数的处理类
    /// 方便各组件快速调用
    public class MJSDK_CallbackDealer_Model<T> : _IMJSDK_CallbackDealer where T : new()
    {
        //成功回调
        private Action<T> _m_dSucDelegate;
        //失败回调
        private Action<int, string> _m_dFailDelegate;

        public MJSDK_CallbackDealer_Model(Action<T> _sucDelegate, Action<int, string> _failDelegate)
        {
        ...
        void _IMJSDK_CallbackDealer.onSDKDealDone(string _content)
        {
            ...
            try
            {
                info = JsonUtility.FromJson<T>(_content);
            }
            catch (Exception _e)
            {
                UnityEngine.Debug.LogError($"Read Version Error json error! errorMessage[{_e.Message}] json[{_content}]");
            }

            if (null != _m_dSucDelegate)
                _m_dSucDelegate(info);
            else
