...
namespace ALPackage
{
    ...
    public partial class ALCommon
    {
        ...
        public static float GetFloat(string _str, string _errMsg = "")
        {
            ...
            float tempValue = 0;
            if(!ALCommon.TryParseFloat(_str, out tempValue))
            {
        ...
        public static List<float> ParseFloatList(string _str, string _errMsg = "")
        {
            if (!string.IsNullOrWhiteSpace(_str))
            {
                ...
                for (int i = 0, count = strs.Length; i < count; i++)
                {
                    ...
                    if (ALCommon.TryParseFloat(tempStr, out value))
                    {
                        _resultList.Add(value);
                    }
                    else
        ...
        public static List<TEnum> ParseEnumList<TEnum>(string _str, bool ignoreCase = true, string _errMsg = "") where TEnum : struct
        {
            if (!string.IsNullOrWhiteSpace(_str))
            {
                ...
                for (int i = 0, count = strs.Length; i < count; i++)
                {
                    ...
                    if (ALCommon.TryEnumParse(enumType, tempStr, out value, ignoreCase))
                    {
                        _resultList.Add(value);
                    }
                    else
        ...
        public static Vector4 GetVector4(string _str, string _errMsg = "")
        {
            ...
            if(!_str.Trim().Equals(""))
            {
                ...
                v4.Set(ALCommon.ParseFloat(strs[0]), ALCommon.ParseFloat(strs[1]), ALCommon.ParseFloat(strs[2]), ALCommon.ParseFloat(strs[3]));
            }
            return v4;
        }
        ...
        public static Color32 GetColor32(string _str)
        {
            byte r = byte.Parse(_str.Substring(0, 2), System.Globalization.NumberStyles.HexNumber);
            byte g = byte.Parse(_str.Substring(2, 2), System.Globalization.NumberStyles.HexNumber);
            byte b = byte.Parse(_str.Substring(4, 2), System.Globalization.NumberStyles.HexNumber);
            byte a = byte.Parse(_str.Substring(6, 2), System.Globalization.NumberStyles.HexNumber);
            return new Color32(r, g, b, a);
        }
        
        public static Color32 GetColor32_Str(string _str, string _errMsg = "")
        {
            if (string.IsNullOrEmpty(_str))
                return Color.clear;
