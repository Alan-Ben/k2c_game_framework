 ...
 * 基础数据调整窗口
 **/
public class EditorWndInt : _IBasicEditorItem
{
    //名称
    ...
    public EditorWndInt(string _name, int _value, Func<string> _addHead, Action<int> _doneAction)
    {
        _m_sName = _name;
    ...
    public EditorWndInt(string _name, int _value, Func<string> _addHead, int _showWidth, Action<int> _doneAction)
    {
        _m_sName = _name;
        _m_sValue = _value.ToString();
        _m_fAddHeadFunc = _addHead;
    ...
    public void dealFunc()
    {
        //转化值
        int value = 0;
        if (int.TryParse(_m_sValue, out value))
        {
            _m_dOnSetting(value);
