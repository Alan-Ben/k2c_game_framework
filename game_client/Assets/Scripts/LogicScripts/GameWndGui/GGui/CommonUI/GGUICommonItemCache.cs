...
namespace GOE
{
    ...
    /// 通用item的对象池
    /// </summary>
    public class GGUICommonItemCache<T, TEMP> : _AALUICacheController<T, TEMP> 
        where T : _ATALBasicUISubWnd<TEMP> 
        where TEMP : _AALBasicUIWndMono
    {
        private Transform _m_tParentTrans;//父节点
        public GGUICommonItemCache(Transform _trans, int _minCount, int _maxCount) : base(_minCount, _maxCount)
        {
            if (null != _trans)
        ...
        // 警告信息文字
        protected override string _warningTxt { get { return "GGUICommonItemCache"; } }
