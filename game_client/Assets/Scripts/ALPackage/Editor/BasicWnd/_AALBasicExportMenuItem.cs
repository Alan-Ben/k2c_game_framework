...
namespace ALPackage
{
    public abstract class _AALBasicExportMenuItem : _IALExportMenuInterface
    {
        /** 显示的子对象 */
        private List<_IALExportMenuInterface> _m_lChildShowItems;

        protected ALSelectItem _m_siSelectItem;
        protected ALSelectItem _m_siForceItem;
        //是否展开状态
        ...
        public _AALBasicExportMenuItem ()
        {
            _m_lChildShowItems = new List<_IALExportMenuInterface>();
            _m_bIsExpend = false;

            _m_siSelectItem = new ALSelectItem("select");
            _m_siForceItem = new ALSelectItem("force");
        }
         ...
         * 注册资源导出菜单项
         **/
        protected void _regSubItem(_IALExportMenuInterface _item)
        {
            if (null == _item)
                return;
