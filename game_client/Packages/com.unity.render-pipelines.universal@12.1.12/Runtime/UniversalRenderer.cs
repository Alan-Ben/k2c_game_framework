...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed class UniversalRenderer : ScriptableRenderer
    {
...
#endif
        private bool m_IsUICamera = false;
        ...
        internal bool isUICamera { get => m_IsUICamera; }
...
#endif
            m_IsUICamera = data.name == "Render_UI_Camera";
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            createColorTexture |= cameraData.needUITarget;
            createColorTexture |= RequiresIntermediateColorTexture(ref cameraData, isUICamera);
            ...
            else
            {
                ...
                // �����UI��������Զ����߼�
                if (cameraData.hasUICameraInStack && !cameraData.needUITarget)
                {
                    if (m_IsUICamera)
                    {
                        RenderTargetHandle cameraTargetHandle = RenderTargetHandle.GetCameraTarget(cameraData.xr);
                        ...
                        setFirstTimeCameraColorTargetIsBound();
                    }
                    else
                    {
                        // 不是UI相机，并且也没有创建ColorTexture，也直接使用CameraTarget
                        if (!createColorTexture)
                        {
                            RenderTargetHandle cameraTargetHandle = RenderTargetHandle.GetCameraTarget(cameraData.xr);
                            m_ActiveCameraColorAttachment = cameraTargetHandle;
            ...
            {
                {
                    if (m_IsUICamera)
                    {
                        ...
                        if (lastCameraInTheStack && m_ActiveCameraColorAttachment !=
                            RenderTargetHandle.GetCameraTarget(cameraData.xr))
                        {
                    ...
                    {
                        //�������UI ���,���Ҳ���ֱ�Ӹ���FrameBuffer������Ҫ�ֶ���3D�����Ⱦ���������FrameBuffer��
                        if (createColorTexture)
                        {
                            if (cameraData.secondToLastCamera)
                            {
                                if (applyPostProcessing)
                                {
                                    // ֱ�Ӹ�FrameBuffer
                                    postProcessPass.Setup(cameraTargetDescriptor, m_ActiveCameraColorAttachment, RenderTargetHandle.GetCameraTarget(cameraData.xr), false,
                                        m_ActiveCameraDepthAttachment, colorGradingLut, false, false);
        ...
        bool RequiresIntermediateColorTexture(ref CameraData cameraData, bool isUICamera)
        {
...
#if UNITY_IOS
            if (isUICamera) isScaledRender = false; // �����UI�������������
#endif
