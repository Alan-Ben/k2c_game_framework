...
@RefBo(isIdAuto= true)
public class BaseAdmobLogBO extends BaseBO {
    ...
    @DataBaseField(type = "varchar(64)", fieldname = "adfrom2", comment = "二级渠道名称：无渠道时，默认使用default")
    private String adfrom2;

    public static final int FIELD_nation =4;
    @DataBaseField(type = "varchar(64)", fieldname = "nation", comment = "国家")
    private String nation;

    public static final int FIELD_ads_type =5;
    @DataBaseField(type = "varchar(16)", fieldname = "ads_type", comment = "广告类型")
    private String ads_type;

    public static final int FIELD_game_postion =6;
    @DataBaseField(type = "varchar(16)", fieldname = "game_postion", comment = "广告位置,项目组自定义")
    private String game_postion;
    ...
    public BaseAdmobLogBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        ...
        ads_type = rs.getString(7);
        game_postion = rs.getString(8);
        timestamp = rs.getInt(9);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new BaseAdmobLogBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `sole_id`, `uid`, `cid`, `adfrom2`, `nation`, `ads_type`, `game_postion`, `timestamp`, `server_id`, `platform`, `region`, `ext`, `date_time`";
    }

    @Override
    public String getTableName() {
        return "`base_admob_log`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(ads_type == null ? null : ads_type.replace("'","''").replace("\\","\\\\")).append("', ");
        strBuf.append("'").append(game_postion == null ? null : game_postion.replace("'","''").replace("\\","\\\\")).append("', ");
    ...
    // 广告类型
    public String getAdsType() { return this.ads_type; }
    public void setAdsType(BM _bm, String ads_type) {
        if(ads_type.equals(this.ads_type)) 
            return;
        this.ads_type = ads_type; 
        markField(_bm, FIELD_ads_type); 
    }
    public void saveAdsType(BM _bm, String ads_type) {
        if(ads_type.equals(this.ads_type)) 
            return;
        this.ads_type = ads_type;
        saveField(_bm, "ads_type", ads_type);
    }

    // 广告位置,项目组自定义
    public String getGamePostion() { return this.game_postion; }
    public void setGamePostion(BM _bm, String game_postion) {
        if(game_postion.equals(this.game_postion)) 
            return;
        this.game_postion = game_postion; 
        markField(_bm, FIELD_game_postion); 
    }
    public void saveGamePostion(BM _bm, String game_postion) {
        if(game_postion.equals(this.game_postion)) 
            return;
        this.game_postion = game_postion;
        saveField(_bm, "game_postion", game_postion);
    }
    ...
    public String getUpdateKeyValue() {
        ...
        sBuilder.append(" `ads_type` = '").append(ads_type == null ? null : ads_type.replace("'","''").replace("\\","\\\\")).append("',");
        sBuilder.append(" `game_postion` = '").append(game_postion == null ? null : game_postion.replace("'","''").replace("\\","\\\\")).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_ads_type)) sBuilder.append(" `ads_type` = '").append(ads_type == null ? null : ads_type.replace("'","''").replace("\\","\\\\")).append("',");
        if(isFieldMarked(FIELD_game_postion)) sBuilder.append(" `game_postion` = '").append(game_postion == null ? null : game_postion.replace("'","''").replace("\\","\\\\")).append("',");
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `base_admob_log` ("
                ...
                + "`adfrom2` varchar(64) NOT NULL DEFAULT '' COMMENT '二级渠道名称：无渠道时，默认使用default',"
                + "`nation` varchar(64) NOT NULL DEFAULT '' COMMENT '国家',"
                + "`ads_type` varchar(16) NOT NULL DEFAULT '' COMMENT '广告类型',"
                + "`game_postion` varchar(16) NOT NULL DEFAULT '' COMMENT '广告位置,项目组自定义',"
                ...
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='梦加日志-变现广告表' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
    ...
    private int getBufferSize()
    {
        ...
        _size += ALBasicProtocolPack.ALProtocolCommon.GetStringBufSize(ads_type);//ads_type
        _size += ALBasicProtocolPack.ALProtocolCommon.GetStringBufSize(game_postion);//game_postion
        _size+=4;//timestamp
        _size+=4;//server_id
        _size+=4;//platform
    ...
    public ByteBuffer toByteBuffer()
    {
        ...
        ALBasicProtocolPack.ALProtocolCommon.PutStringIntoBuf(buff, ads_type);
        ALBasicProtocolPack.ALProtocolCommon.PutStringIntoBuf(buff, game_postion);
    ...
    protected void readFromByteBuffer(ByteBuffer buff)
    {
        ...
        ads_type=ALBasicProtocolPack.ALProtocolCommon.GetStringFromBuf(buff);
        game_postion=ALBasicProtocolPack.ALProtocolCommon.GetStringFromBuf(buff);
