...
/***
 * 用户在向Guild转发消息时，如果针对返回的消息需要做二次处理的，通过本对象进行处理
 */
public abstract class _ATGuildUserMsgRedirectCommiter<T extends  _IALProtocolStructure> extends _ANPUSUserBasicMsgItem
{
    //基础转发的消息对象
    private _ANPUSUserBasicMsgItem _m_miBasicMsgItem;

    public _ATGuildUserMsgRedirectCommiter(_ANPUSUserBasicMsgItem _basicMsgItem)
    {
    ...
    public void commitSucRes(_IALProtocolStructure _msg) {
        if(null == _msg)
        {
            CommLog.error("GuildMsgRedirectCommiter commitSucRes error, msg type is not correct, msg class:" + _msg.getClass().getName());
