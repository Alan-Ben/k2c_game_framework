...
import GC2GS.p023_ArenaOp.GC2GS_023_006_ReqFightBackSelectHero;
...
import NPUSServer.NPUSUserMgr.UserComp.ArenaComp.FightReport.ArenaFightReportInfo;
...
public class MsgDealer_GC2GS_023_006_ReqFightBackSelectHero extends NPUserMsgDealer<GC2GS_023_006_ReqFightBackSelectHero>
{
    @Override
    protected void _dealMessage(_ANPUSUserBasicMsgItem _commiter, GC2GS_023_006_ReqFightBackSelectHero _msg)
    {
        ...
        //查找战报
        ArenaFightReportInfo fightReport = userData.getArenaComponent().getFightReportMgr().findFightReport(_msg.getDbId());
        if (fightReport == null)
        {
            _commiter.commitFailRes(ArenaErr.REPORT_NOT_FOUND.getCode());
            return;
        }
        ...
        //先选择对手
        promise.then(p->{
            userData.getArenaComponent().selectOpponent(fightReport.getOpponentCid(), EArenaAttackType.CELEBRITY_RANK, _result ->
            {
                if (!_result.isSucc())
                {
        ...
        promise.then(p->{
            ...
            //标记战报为已读
            userData.getArenaComponent().getFightReportMgr().markFightBackRead(fightReport);

            _commiter.commitSucRes(US2GCWriter_023_ArenaOp.make_006_RetFightBackSelectHero());
        });
    }
}
