...
namespace ALPackage
{
     ...
     * 缓存池模板对象
     **/
    public abstract class _AALCacheHashSetController<T, TEMP>
    {
        ...
        /** 总的缓存队列 */
        private HashSet<T> _m_lTotalCacheList;
        /** 还可使用的缓存队列 */
        private HashSet<T> _m_lEnableCacheList;
        /** 已经被使用的缓存队列 */
        private HashSet<T> _m_lUsedItemList;

        protected _AALCacheHashSetController(int _minCount, int _maxCount)
        {
            ...
            _m_lTotalCacheList = new HashSet<T>();
            _m_lEnableCacheList = new HashSet<T>();
            _m_lUsedItemList = new HashSet<T>();

            _m_iAddUnit = 1;
        }
        
        protected _AALCacheHashSetController(int _minCount, int _maxCount, int _addUnit)
        {
            _m_tTemplateObj = default(TEMP);
        ...
        public HashSet<T> usedItemList { get { return _m_lUsedItemList; } }
        ...
        public void discard()
        {
            foreach (var item in _m_lTotalCacheList)
            {
                //释放资源
                _discardItem(item);
            }
        ...
        public T popItem()
        {
            if (_m_lEnableCacheList.Count <= 0)
            {
                ...
                if (_m_lTotalCacheList.Count > _m_iIsWarningCount)
                {
                    ...
                    //判断是否超过需要报错的数量1000
                    if (_m_lTotalCacheList.Count > ALConst.CACHE_ERROR_COUNT)
                    {
...
//                foreach (T item in _m_lEnableCacheList)
//                {
//                    tempItem = item;
//                    break;
//                }

                var enumerator = _m_lEnableCacheList.GetEnumerator();
                enumerator.MoveNext();
                T tempItem = enumerator.Current;
                _m_lEnableCacheList.Remove(tempItem);
                _m_lUsedItemList.Add(tempItem);
                return tempItem;
            }
