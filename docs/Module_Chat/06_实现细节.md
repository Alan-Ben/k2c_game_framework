...
/// 消息排序策略
/// </summary>
public class MessageSortStrategy
{
    // 按时间戳排序（默认）
    public static int SortByTimestamp(ChatMessage a, ChatMessage b)
    {
        return a.sendTime.CompareTo(b.sendTime);
    }

    // 按优先级排序（置顶消息优先）
    public static int SortByPriority(ChatMessage a, ChatMessage b)
    {
    ...
    // 按发送者等级排序
    public static int SortBySenderLevel(ChatMessage a, ChatMessage b)
    {
        if (a.senderVipLevel != b.senderVipLevel)
            return b.senderVipLevel.CompareTo(a.senderVipLevel);
        return a.sendTime.CompareTo(b.sendTime);
    }
}
```
...
classDiagram
    class IMessageHandler {
        <<interface>>
        +int msgType
        +Handle(MessageContext ctx) void
    }
    ...
    class MessageHandlerFactory {
        -Dictionary handlers
        +Register(IMessageHandler handler) void
        +GetHandler(int msgType) IMessageHandler
    }

    IMessageHandler <|.. TextMessageHandler
    IMessageHandler <|.. EmoteMessageHandler
    IMessageHandler <|.. ShareMessageHandler
...
public class MessageHandlerRegistry {
    ...
    public static void init() {
        // 注册内置处理器
        register(new TextMessageHandler());
        register(new EmoteMessageHandler());
        register(new ShareHeroHandler());
        register(new ShareConsortHandler());

        // 扫描并注册自定义处理器
        ServiceLoader.load(IMessageHandler.class).forEach(handler -> {
            register(handler);
            logger.info("注册消息处理器: type={}, class={}",
                handler.getMsgType(), handler.getClass().getSimpleName());
        });
    }
}
```
...
| 消息丢失 | 网络波动 | 消息确认+重传机制 |
| 消息乱序 | 多线程处理 | 消息序号+排序 |
| 内存泄漏 | 消息缓存过大 | LRU淘汰+定时清理 |
| 延迟高 | 消息队列积压 | 增加消费者+扩容 |
| 敏感词漏判 | Trie树未更新 | 定时热更新词库 |
