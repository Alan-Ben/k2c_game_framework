...
@RefBo(isIdAuto= true)
public class NpCrossInstanceJoinerBO extends BaseBO {
    
    @DataBaseField(type = "bigint(20)", fieldname = "id", comment = "数据库表唯一键值")
    private long id;

    public static final int FIELD_cross_instance_id =0;
    @DataBaseField(type = "bigint(20)", fieldname = "cross_instance_id", comment = "跨服排行分组Id")
    ...
    public NpCrossInstanceJoinerBO() {
        id = 0;
        cross_instance_id = 0L;
        joiner_id = 0L;
    }

    public NpCrossInstanceJoinerBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new NpCrossInstanceJoinerBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cross_instance_id`, `joiner_id`";
    }

    @Override
    public String getTableName() {
        return "`np_cross_instance_joiner`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `np_cross_instance_joiner` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cross_instance_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '跨服排行分组Id',"
