...
| 服务端错误码 | 客户端处理 | 用户提示 |
|-------------|-----------|----------|
| 22001 | 触发重连 | "连接断开,正在重连..." |
| 22002 | 刷新房间列表 | "未加入该频道" |
| 22003 | 显示倒计时 | "冷却中,请等待X秒" |
| 22005 | 显示禁言弹窗 | "您已被禁言" |
...
{
    // 正常长度
    Assert.IsTrue(ChatValidator.ValidateLength("测试消息"));

    // 边界长度
    Assert.IsTrue(ChatValidator.ValidateLength(new string('a', 200)));

    // 超长消息
    Assert.IsFalse(ChatValidator.ValidateLength(new string('a', 201)));

    // 空消息
    Assert.IsFalse(ChatValidator.ValidateLength(""));
}

[Test]
public void TestCooldownCalculation()
{
    var manager = new ChatCooldownManager();
    manager.StartCooldown(1);

    Assert.IsTrue(manager.IsInCooldown(1));
    Assert.Greater(manager.GetRemainingCooldown(1), 0);

    // 等待冷却结束
    System.Threading.Thread.Sleep(6000);
    Assert.IsFalse(manager.IsInCooldown(1));
}
```
