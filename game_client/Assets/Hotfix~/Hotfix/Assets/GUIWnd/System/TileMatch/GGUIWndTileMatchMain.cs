...
namespace Hotfix
{
    ...
    /// 三消活动页面
    /// </summary>
    public class GGUIWndTileMatchMain : _AHotfixBaseSubPrefabWnd<GGUIMonoTileMatchMain>
    {
        //活动id
        private long _m_lActivityId;
        
        public GGUIWndTileMatchMain(long _activityId, Transform _parent) : base(_parent)
        {
            _m_lActivityId = _activityId;
        }

        protected override string _monoAssetPath { get { return UIResPathAssistant.getAssetPath(6100); } }
        protected override string _monoObjName { get { return UIResPathAssistant.getObjName(6100); } }
        ...
        private void _checkActivity()
        {
            ...
            {
                // 判断是否需要显示第一次进入游戏对话
                if (hotfixWnd != null && HotfixAccountSettingMgr.instance.hotfixAccountSetting.checkNeedShowFirstEnterTileMatchActivityDialog(activityInfo.startTimeMs))
                {
