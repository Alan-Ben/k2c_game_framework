...
{
	/// <summary>
	/// 可以支持返回两个TTmapd的自动导出基类
	/// 第一个类型是模板类,  第一个类型是模板类2, 第三个类型是实际游戏中运用的类,  第四个类型是表的集合类
	/// </summary>
	/// <typeparam name="TTemp"></typeparam>
	/// <typeparam name="Tobj"></typeparam>
	/// <typeparam name="TMap"></typeparam>
	public abstract class NPBasicAutoExportMenuItemTwoTemp<TTemp, TTempTwo, Tobj, TMap> : NPBasicExportMenuItemEX 
	    where TTemp : new() 
	    where TTempTwo : new()
	    ...
	    where TMap : _TALSOBasicRefSet<Tobj>, new()
	{
	    ...
	    public NPBasicAutoExportMenuItemTwoTemp(ENPExportSettingEnum _exportEnum
	        , string _assetPath
	    ...
	    private void exportGeneralRefSet()
	    {
	        ...
	        List<TTempTwo> tempTwoList = new List<TTempTwo>();

	        NPExportWnd.autoReadXlsTwoTemp<TTemp, TTempTwo>(excelPath, tabName, exportEnum, tempList, tempTwoList);
	        if(tempList.Count == 0 || tempTwoList.Count == 0 || tempList.Count != tempTwoList.Count)
	        {
	            Debug.LogError(exportEnum + "数据有问题,请注意.");
            }
	        ...
	        refSet.refList = new List<Tobj>(_exchangeTemplate(tempList, tempTwoList));
        ...
        //将模板数据转化为实例数据的函数
        public abstract List<Tobj> _exchangeTemplate(List<TTemp> _tempList, List<TTempTwo> _tempTwoList);
