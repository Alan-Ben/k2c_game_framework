...
public class Opt014014AdultApplyGroupBO extends BaseOptLogBo {
    ...
    public static final int FIELD_bonus =8;
    @DataBaseField(type = "bigint(20)", fieldname = "bonus", comment = "子嗣收益")
    private long bonus;

    public static final int FIELD_minBonus =9;
    @DataBaseField(type = "bigint(20)", fieldname = "minBonus", comment = "对方子嗣允许最小收益")
    private long minBonus;

    public Opt014014AdultApplyGroupBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt014014AdultApplyGroupBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        bonus = rs.getLong(10);
        minBonus = rs.getLong(11);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt014014AdultApplyGroupBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `adultId`, `bonus`, `minBonus`";
    }

    @Override
    public String getTableName() {
        return "`opt_014_014_adult_apply_group`";
    }
   ...
   @Override
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_014_014_adult_apply_group` ("
                ...
                + "`minBonus` bigint(20) NOT NULL DEFAULT '0' COMMENT '对方子嗣允许最小收益',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='成年子嗣-进入全服联姻池' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
