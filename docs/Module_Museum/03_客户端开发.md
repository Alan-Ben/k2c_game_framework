...
namespace UI.Museum
{
    ...
    /// 藏品卡片UI组件
    /// </summary>
    public class MuseumItemCardUI : MonoBehaviour
    {
        [Header("UI组件")]
        ...
        {
            new Color(0.6f, 0.6f, 0.6f),  // 普通 - 灰色
            new Color(0.2f, 0.6f, 1f),    // 稀有 - 蓝色
            new Color(0.6f, 0.2f, 0.8f),  // 史诗 - 紫色
            new Color(1f, 0.5f, 0f)       // 传说 - 橙色
        };

        // 稀有度边框资源路径
        private static readonly string[] RarityFramePaths = new string[]
        {
            "UI/Museum/frame_common",
            "UI/Museum/frame_rare",
            "UI/Museum/frame_epic",
            "UI/Museum/frame_legend"
        };

        private MuseumItemInfo _itemData;
        private System.Action<MuseumItemInfo> _onClick;

        /// <summary>
        /// 设置数据
        /// </summary>
        public void SetData(MuseumItemInfo item)
        {
            _itemData = item;
            ...
            // 设置等级
            _levelText.text = $"Lv.{item.level}/{config.maxLevel}";

            // 设置稀有度
            int rarityIndex = config.rarity - 1;
            _rarityFrame.color = RarityColors[rarityIndex];

            // 设置图标
            var sprite = MuseumResourceManager.Instance.GetIcon(item.itemId);
            if (sprite != null)
            {
            ...
            // 设置属性预览
            var attrs = item.CalculateAttr();
            _attrText.text = FormatAttrText(attrs);

            // 设置激活状态
            _activeIcon.gameObject.SetActive(item.isActive);
        ...
        /// 设置点击回调
        /// </summary>
        public void SetClickCallback(System.Action<MuseumItemInfo> onClick)
        {
            _onClick = onClick;
            _cardButton.onClick.RemoveAllListeners();
            _cardButton.onClick.AddListener(() => _onClick?.Invoke(_itemData));
        }
        ...
        private string FormatAttrText(Dictionary<int, long> attrs)
        {
            ...
            foreach (var attr in attrs)
            {
                string attrName = GetAttrName(attr.Key);
                texts.Add($"{attrName}+{attr.Value}");
            }
            return string.Join(" ", texts);
        }
...
### 11.2 MuseumUpgradePopupUI - 升级弹窗组件
...
namespace UI.Museum
{
    ...
    /// 升级弹窗UI组件
    /// </summary>
    public class MuseumUpgradePopupUI : PopupUI
    {
        [Header("信息展示")]
        [SerializeField] private Image _iconImage;
        [SerializeField] private TextMeshProUGUI _nameText;
        ...
        [Header("属性对比")]
        [SerializeField] private Transform _currentAttrContainer;
        [SerializeField] private Transform _targetAttrContainer;
        [SerializeField] private GameObject _attrItemPrefab;
        ...
        [Header("按钮")]
        [SerializeField] private Button _upgradeOnceBtn;
        [SerializeField] private Button _upgradeMaxBtn;
        [SerializeField] private Toggle _protectToggle;
        ...
        protected override void OnOpen()
        {
            ...
            _upgradeOnceBtn.onClick.AddListener(OnUpgradeOnce);
            _upgradeMaxBtn.onClick.AddListener(OnUpgradeMax);
        }

        protected override void OnClose()
        {
            _closeBtn.onClick.RemoveListener(Close);
            _upgradeOnceBtn.onClick.RemoveListener(OnUpgradeOnce);
            _upgradeMaxBtn.onClick.RemoveListener(OnUpgradeMax);
            base.OnClose();
        }
        ...
        private void UpdateUI()
        {
            var config = MuseumItemConfig.Get(_itemData.itemId);
            if (config == null) return;

            // 基础信息
            _nameText.text = config.itemName;
            _currentLevelText.text = $"Lv.{_itemData.level}";
            _targetLevelText.text = $"Lv.{_itemData.level + 1}";

            // 图标
            _iconImage.sprite = MuseumResourceManager.Instance.GetIcon(_itemData.itemId);

            // 属性对比
            UpdateAttrCompare(_itemData.level, _itemData.level + 1);

            // 升级消耗
            UpdateCost(_itemData.level + 1);

            // 成功率
            UpdateSuccessRate(_itemData.level + 1);

            // 计算最大可升级次数
            _maxUpgradeCount = CalculateMaxUpgradeCount();

            // 最大升级按钮
            _upgradeMaxBtn.interactable = _maxUpgradeCount > 1;
            _upgradeMaxBtn.GetComponentInChildren<TextMeshProUGUI>().text =
                _maxUpgradeCount > 1 ? $"一键升×{_maxUpgradeCount}" : "一键升级";
        }

        /// <summary>
        /// 更新属性对比显示
        /// </summary>
        private void UpdateAttrCompare(int currentLevel, int targetLevel)
        {
            // 清空现有属性项
            ClearContainer(_currentAttrContainer);
            ClearContainer(_targetAttrContainer);

            // 计算当前属性
            var currentAttrs = _itemData.CalculateAttr();
            foreach (var attr in currentAttrs)
            {
                CreateAttrItem(_currentAttrContainer, attr.Key, attr.Value);
            }

            // 计算目标属性
            var targetAttrs = _itemData.GetNextAttr();
            foreach (var attr in targetAttrs)
            {
                var current = currentAttrs.ContainsKey(attr.Key) ? currentAttrs[attr.Key] : 0;
                long diff = attr.Value - current;
                CreateAttrItem(_targetAttrContainer, attr.Key, attr.Value, diff);
            }
        }
        ...
        /// 更新消耗显示
        /// </summary>
        private void UpdateCost(int targetLevel)
        {
            ClearContainer(_costContainer);

            var cost = _itemData.GetUpgradeCost(targetLevel);
            bool canAfford = true;

            foreach (var c in cost)
            {
                var item = CreateCostItem(_costContainer, c.Key, c.Value);
                if (!ResourceComponent.Instance.HasEnough(c.Key, c.Value))
                {
                    canAfford = false;
            ...
            _upgradeOnceBtn.interactable = canAfford;
        }

        /// <summary>
        /// 更新成功率显示
        /// </summary>
        private void UpdateSuccessRate(int targetLevel)
        {
            var upgradeConfig = MuseumItemUpgradeConfig.Get(_itemData.itemId, targetLevel);
            if (upgradeConfig != null)
            {
                float rate = upgradeConfig.successRate * 100;
                _successRateText.text = $"成功率: {rate:F1}%";
                _successRateSlider.value = upgradeConfig.successRate;

                // 低成功率显示警告色
                if (rate < 80)
                {
                    _successRateText.color = Color.red;
                }
                else if (rate < 100)
                {
                    _successRateText.color = Color.yellow;
                }
                else
                {
                    _successRateText.color = Color.green;
                }
            }
        }
        ...
        /// 计算最大可升级次数
        /// </summary>
        private int CalculateMaxUpgradeCount()
        {
            var config = MuseumItemConfig.Get(_itemData.itemId);
            if (config == null) return 0;

            int count = 0;
            int currentLevel = _itemData.level;

            while (currentLevel < config.maxLevel)
            {
                var upgradeCost = _itemData.GetUpgradeCost(currentLevel + 1);
                if (!CheckResourceEnough(upgradeCost)) break;
        ...
        /// 单次升级
        /// </summary>
        private void OnUpgradeOnce()
        {
            bool useProtect = _protectToggle.isOn;
            MuseumComponent.Instance.ReqUpgrade(_itemData.itemId, 1, useProtect, (success) =>
            {
                if (success)
                {
                    // 播放升级成功动画
        ...
        private void OnUpgradeMax()
        {
            ...
            bool useProtect = _protectToggle.isOn;
            MuseumComponent.Instance.ReqUpgrade(_itemData.itemId, _maxUpgradeCount, useProtect, (success) =>
            {
                if (success)
                {
                    PlayUpgradeEffect();
        ...
        {
            // 播放音效
            AudioManager.PlaySound("museum_upgrade_success");
            // 播放特效
            EffectManager.PlayEffect("upgrade_success", transform.position);
        }
        ...
        {
            UIManager.ShowTip("升级失败！");
            AudioManager.PlaySound("museum_upgrade_fail");
        }

        /// <summary>
        /// 创建属性项
        /// </summary>
        private void CreateAttrItem(Transform container, int attrType, long value, long diff = 0)
        {
            var go = Instantiate(_attrItemPrefab, container);
            var text = go.GetComponent<TextMeshProUGUI>();

            string attrName = GetAttrName(attrType);
            if (diff > 0)
            {
                text.text = $"{attrName}: {value} <color=green>(+{diff})</color>";
            }
            else
            {
                text.text = $"{attrName}: {value}";
            }
        }
        ...
        /// 创建消耗项
        /// </summary>
        private CostItemUI CreateCostItem(Transform container, string itemId, int count)
        {
            var go = Instantiate(_costItemPrefab, container);
            var item = go.GetComponent<CostItemUI>();
...
### 11.3 MuseumSlotSelectUI - 展位选择组件
...
namespace UI.Museum
{
    ...
    /// 展位选择UI组件
    /// </summary>
    public class MuseumSlotSelectUI : PopupUI
    {
        [Header("展位列表")]
        ...
        private List<MuseumSlotItemUI> _slotItems = new List<MuseumSlotItemUI>();
        ...
        /// 显示展位选择
        /// </summary>
        public void Show(int itemId, System.Action<int> onSelect)
        {
        ...
        private void RefreshSlotList()
        {
            ...
            // 获取所有展位
            var slots = MuseumComponent.Instance.GetSlots();
            var itemConfig = MuseumItemConfig.Get(_selectedItemId);

            foreach (var slot in slots)
            {
                var go = Instantiate(_slotItemPrefab, _slotContainer);
                var slotUI = go.GetComponent<MuseumSlotItemUI>();

                bool canPlace = CanPlaceInSlot(slot, itemConfig);
                slotUI.SetData(slot, canPlace);
                slotUI.SetClickCallback(() => OnSlotClick(slot.slotIndex));

                _slotItems.Add(slotUI);
            }
        }
        ...
        /// 检查是否可放入展位
        /// </summary>
        private bool CanPlaceInSlot(MuseumSlotInfo slot, MuseumItemConfig itemConfig)
        {
            // 展位被锁定
            if (slot.isLocked) return false;

            // 检查展位类型限制
            if (slot.slotType > 0 && itemConfig.rarity < slot.slotType)
            {
                return false;
            }
        ...
        {
            _onSelectCallback?.Invoke(slotIndex);
            Close();
        }
    }
    ...
    /// 展位项UI组件
    /// </summary>
    public class MuseumSlotItemUI : MonoBehaviour
    {
        ...
        private MuseumSlotInfo _slotData;
        private System.Action _onClick;

        /// <summary>
        /// 设置数据
        /// </summary>
        public void SetData(MuseumSlotInfo slot, bool canPlace)
        {
            _slotData = slot;

            _slotIndexText.text = $"展位 #{slot.slotIndex + 1}";
            ...
            else if (slot.itemId > 0)
            {
                ...
                var config = MuseumItemConfig.Get(slot.itemId);
                _itemNameText.text = config?.itemName ?? "未知";
                _itemIcon.sprite = MuseumResourceManager.Instance.GetIcon(slot.itemId);
                _slotImage.color = canPlace ? Color.white : Color.red;
            }
            else
            {
                // 空展位
                _lockedIcon.SetActive(false);
                _itemIcon.gameObject.SetActive(false);
                _itemNameText.text = "空闲";
                _slotImage.color = canPlace ? Color.green : Color.gray;
            }

            _slotButton.interactable = canPlace;
        }
        ...
        public void SetClickCallback(System.Action onClick)
        {
            ...
            _slotButton.onClick.AddListener(() => _onClick?.Invoke());
        }
    }
}
```

### 11.4 MuseumComposeUI - 合成弹窗组件
...
namespace UI.Museum
{
    ...
    /// 合成弹窗UI组件
    /// </summary>
    public class MuseumComposeUI : PopupUI
    {
        [Header("碎片信息")]
        [SerializeField] private Image _fragmentIcon;
        [SerializeField] private TextMeshProUGUI _fragmentNameText;
        [SerializeField] private TextMeshProUGUI _ownedCountText;
        [SerializeField] private TextMeshProUGUI _needCountText;
        ...
        [Header("按钮")]
        [SerializeField] private Button _composeBtn;
        [SerializeField] private Button _closeBtn;

        private int _fragmentId;
        private int _maxComposeCount;
        private int _currentCount = 1;
        private RefMuseumItemFragment _fragmentConfig;

        protected override void OnOpen()
        {
            base.OnOpen();
            _closeBtn.onClick.AddListener(Close);
            _composeBtn.onClick.AddListener(OnCompose);
            _minusBtn.onClick.AddListener(OnMinus);
            _plusBtn.onClick.AddListener(OnPlus);
            _maxBtn.onClick.AddListener(OnMax);
            _countSlider.onValueChanged.AddListener(OnSliderChanged);
        }

        protected override void OnClose()
        {
            _closeBtn.onClick.RemoveListener(Close);
            _composeBtn.onClick.RemoveListener(OnCompose);
            _minusBtn.onClick.RemoveListener(OnMinus);
            _plusBtn.onClick.RemoveListener(OnPlus);
            _maxBtn.onClick.RemoveListener(OnMax);
            _countSlider.onValueChanged.RemoveListener(OnSliderChanged);
            base.OnClose();
        }
        ...
        /// 加载碎片配置
        /// </summary>
        private void LoadFragmentConfig()
        {
            _fragmentConfig = MuseumItemFragmentConfig.Get(_fragmentId);
        }
        ...
        private void UpdateUI()
        {
            if (_fragmentConfig == null) return;

            // 碎片信息
            _fragmentNameText.text = _fragmentConfig.fragmentName;

            int owned = BagComponent.Instance.GetItemCount(_fragmentId);
            int needPerCompose = _fragmentConfig.composeCount;
            _ownedCountText.text = $"拥有: {owned}";
            _needCountText.text = $"每次需要: {needPerCompose}";

            // 计算最大可合成数量
            _maxComposeCount = owned / needPerCompose;

            // 更新数量控制
            _countText.text = _currentCount.ToString();
            _countSlider.maxValue = _maxComposeCount;
            _countSlider.value = _currentCount;
            _countSlider.interactable = _maxComposeCount > 1;

            _minusBtn.interactable = _currentCount > 1;
            _plusBtn.interactable = _currentCount < _maxComposeCount;
            _maxBtn.interactable = _maxComposeCount > 1;

            // 合成结果预览
            var itemConfig = MuseumItemConfig.Get(_fragmentConfig.itemId);
            if (itemConfig != null)
            {
                _resultNameText.text = itemConfig.itemName;
                _resultDescText.text = itemConfig.itemDesc;
                _resultIcon.sprite = MuseumResourceManager.Instance.GetIcon(_fragmentConfig.itemId);
            }

            // 合成按钮
            _composeBtn.interactable = _currentCount > 0 && _currentCount <= _maxComposeCount;
        }
        ...
        private void OnPlus()
        {
            if (_currentCount < _maxComposeCount)
            {
                _currentCount++;
                UpdateUI();
            }
        ...
        private void OnMax()
        {
            _currentCount = _maxComposeCount;
            UpdateUI();
        }
        ...
        {
            MuseumComponent.Instance.ReqCompose(_fragmentId, _currentCount, (success) =>
            {
                if (success)
                {
                    // 播放合成成功动画
        ...
        /// 播放合成特效
        /// </summary>
        private void PlayComposeEffect()
        {
            AudioManager.PlaySound("museum_compose");
            EffectManager.PlayEffect("compose_success", transform.position);
        }
    }
}
```
...
namespace GameComponent.Museum
{
    ...
    /// 博物馆事件ID定义
    /// </summary>
    public static class MuseumEventID
    {
        // 博物馆信息更新
        public static readonly int MuseumInfoUpdate = "MuseumInfoUpdate".GetHashCode();
        ...
        public static readonly int MuseumItemChg = "MuseumItemChg".GetHashCode();
        public static readonly int MuseumItemDel = "MuseumItemDel".GetHashCode();
        public static readonly int MuseumItemUpgrade = "MuseumItemUpgrade".GetHashCode();
        public static readonly int MuseumItemActivate = "MuseumItemActivate".GetHashCode();

        // 展位相关事件
        public static readonly int MuseumSlotChg = "MuseumSlotChg".GetHashCode();
        public static readonly int MuseumSlotSet = "MuseumSlotSet".GetHashCode();
        public static readonly int MuseumSlotClear = "MuseumSlotClear".GetHashCode();

        // 属性相关事件
        public static readonly int MuseumAttrChg = "MuseumAttrChg".GetHashCode();
        public static readonly int MuseumAttrUpdate = "MuseumAttrUpdate".GetHashCode();

        // 套装相关事件
        public static readonly int MuseumSetEffectChg = "MuseumSetEffectChg".GetHashCode();
        public static readonly int MuseumSetActivate = "MuseumSetActivate".GetHashCode();

        // UI相关事件
        public static readonly int MuseumUIRefresh = "MuseumUIRefresh".GetHashCode();
        public static readonly int MuseumFilterChg = "MuseumFilterChg".GetHashCode();
        public static readonly int MuseumDetailShow = "MuseumDetailShow".GetHashCode();
    }
}
```
...
namespace GameComponent.Museum
{
    ...
    /// 藏品变更事件数据
    /// </summary>
    public class MuseumItemChgEventData
    {
        public int itemId;
        public int oldLevel;
        public int newLevel;
        ...
        public int upgradeResult; // 0成功, 1失败, 2保护
    }

    /// <summary>
    /// 属性变更事件数据
    /// </summary>
    public class MuseumAttrChgEventData
    {
        public Dictionary<int, long> oldAttrs;
        public Dictionary<int, long> newAttrs;
        public List<AttrChangeItem> changes;
    }

    /// <summary>
    /// 属性变更项
    /// </summary>
    public class AttrChangeItem
    {
        public int attrType;
    ...
    /// 套装效果变更事件数据
    /// </summary>
    public class MuseumSetEffectChgEventData
    {
        public int setId;
        public string setName;
        public int oldCount;
...
namespace GameComponent.Museum
{
    ...
    /// 博物馆资源配置
    /// </summary>
    [CreateAssetMenu(fileName = "MuseumResourceConfig", menuName = "Game/Museum/ResourceConfig")]
    public class MuseumResourceConfig : ScriptableObject
    {
        [Header("图标资源")]
        public string iconPathPrefix = "UI/Museum/Icons/";
        public string defaultIcon = "UI/Museum/default_icon";

        [Header("稀有度框")]
        public string[] rarityFramePaths = new string[]
        {
            "UI/Museum/frame_common",
            "UI/Museum/frame_rare",
            "UI/Museum/frame_epic",
        ...
        [Header("特效资源")]
        public string upgradeSuccessEffect = "Effects/Museum/upgrade_success";
        public string upgradeFailEffect = "Effects/Museum/upgrade_fail";
        public string activateEffect = "Effects/Museum/activate";
        public string composeEffect = "Effects/Museum/compose";

        [Header("音效资源")]
        public string upgradeSuccessSound = "Sounds/Museum/upgrade_success";
        public string upgradeFailSound = "Sounds/Museum/upgrade_fail";
        public string activateSound = "Sounds/Museum/activate";
        public string composeSound = "Sounds/Museum/compose";

        [Header("预加载配置")]
        public bool preloadCommonIcons = true;
        public int preloadIconCount = 50;
    }
}
```
...
namespace GameComponent.Museum
{
    ...
    /// 博物馆资源异步加载器
    /// </summary>
    public class MuseumResourceLoader : MonoBehaviour
    {
        [SerializeField] private MuseumResourceConfig _config;
        ...
        {
            // 预加载稀有度框
            foreach (var path in _config.rarityFramePaths)
            {
                yield return LoadAsync<Sprite>(path);
            }

            // 预加载特效
            yield return LoadAsync<GameObject>(_config.upgradeSuccessEffect);
            yield return LoadAsync<GameObject>(_config.upgradeFailEffect);
            yield return LoadAsync<GameObject>(_config.activateEffect);
            yield return LoadAsync<GameObject>(_config.composeEffect);

            // 预加载常用图标
            if (_config.preloadCommonIcons)
            {
                var commonItems = MuseumItemConfig.GetCommonItems(_config.preloadIconCount);
                foreach (var item in commonItems)
                {
                    yield return LoadAsync<Sprite>(item.icon);
                }
            }
        }
        ...
        public IEnumerator LoadAsync<T>(string path) where T : Object
        {
            ...
            {
                yield return new WaitUntil(() => !_loadingPaths.Contains(path));
                yield break;
            }
...
namespace GameComponent.Museum
{
    ...
    /// 博物馆动画控制器
    /// </summary>
    public class MuseumAnimationController : MonoBehaviour
    {
        [Header("动画配置")]
        [SerializeField] private float _upgradeAnimDuration = 0.5f;
        [SerializeField] private float _activateAnimDuration = 1.0f;
        [SerializeField] private float _composeAnimDuration = 0.8f;
        [SerializeField] private AnimationCurve _scaleCurve;

        /// <summary>
        /// 播放升级动画
        /// </summary>
        public IEnumerator PlayUpgradeAnimation(Transform target, int oldLevel, int newLevel)
        {
            float elapsed = 0f;
            Vector3 originalScale = target.localScale;

            // 放大-缩小动画
            while (elapsed < _upgradeAnimDuration)
            {
                elapsed += Time.deltaTime;
                float t = elapsed / _upgradeAnimDuration;
                float scale = _scaleCurve.Evaluate(t);
                target.localScale = originalScale * (1f + scale * 0.3f);
                yield return null;
            }
            ...
            // 显示等级变化数字
            ShowLevelUpNumber(target, oldLevel, newLevel);
        }

        /// <summary>
        /// 播放激活动画
        /// </summary>
        public IEnumerator PlayActivateAnimation(Transform target)
        {
            // 闪光效果
            var flash = target.GetComponent<CanvasGroup>();
            if (flash != null)
            {
                float elapsed = 0f;
                while (elapsed < _activateAnimDuration)
                {
                    elapsed += Time.deltaTime;
                    float t = elapsed / _activateAnimDuration;
                    flash.alpha = Mathf.PingPong(t * 4f, 1f);
                    yield return null;
                }
                flash.alpha = 1f;
            }
            ...
            // 粒子特效
            PlayParticleEffect(target.position, "activate_glow");
        }

        /// <summary>
        /// 显示升级数字
        /// </summary>
        private void ShowLevelUpNumber(Transform target, int oldLevel, int newLevel)
        {
            var floatingText = FloatingTextManager.Instance;
            if (floatingText != null)
            {
                floatingText.ShowText(
                    target.position + Vector3.up * 50f,
                    $"+{newLevel - oldLevel}",
                    Color.green
                );
            }
        ...
        /// 播放粒子特效
        /// </summary>
        private void PlayParticleEffect(Vector3 position, string effectName)
        {
