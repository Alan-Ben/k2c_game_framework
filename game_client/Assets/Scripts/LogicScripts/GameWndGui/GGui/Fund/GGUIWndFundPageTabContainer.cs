...
namespace GOE
{
    ...
    /// 基金页签item容器
    /// </summary>
    public class GGUIWndFundPageTabContainer : _ATNPGGUIWndShowAnimContainer<GGUIMonoFundTabContainerItem, GGUIMonoFundTabContainer, GGUIWndFundPageTabContainerItem>
    {
        //item列表
        [NotNull] protected List<GGUIWndFundPageTabContainerItem> _m_lItemList = new List<GGUIWndFundPageTabContainerItem>();
        //当前选中的item
        private GGUIWndFundPageTabContainerItem _m_wCurSelectItem;
        //点击item事件
        private Action<GGUIWndFundPageTabContainerItem, bool> _m_aOnClickItem;
        
        
        public GGUIWndFundPageTabContainer(GGUIMonoFundTabContainer _mono) : base(_mono)
        {
            initWnd();
        }
        ...
        /// 点击item事件
        /// </summary>
        public Action<GGUIWndFundPageTabContainerItem, bool> onClickItem { get { return _m_aOnClickItem; } set { _m_aOnClickItem = value; } }


        protected override GGUIWndFundPageTabContainerItem _createItemWnd(GGUIMonoFundTabContainerItem _itemMono)
        {
            GGUIWndFundPageTabContainerItem item = new GGUIWndFundPageTabContainerItem(_itemMono);
        ...
        /// 设置信息
        /// </summary>
        public void showItemList(List<FundInfoSnapshot> _dataList, long _selectFundId = -1)
        {
            if (_dataList == null || _m_lItemList == null)
                return;

            FundInfoSnapshot lastSelectData = _m_wCurSelectItem?.fundSnapshot;
            _m_wCurSelectItem?.setSelect(false);
            _m_wCurSelectItem = null;
            GGUIWndFundPageTabContainerItem itemWnd = null;
            int count = 0;
            ...
            for (int i = 0; i < _dataList.Count; i++)
            {
                ...
                itemWnd.showWnd();
                itemWnd.setInfo(_dataList[i], count == 0);
                count++;

                //判断当前选中的item是否在列表中，有则选中
                if (_selectFundId < 0 && lastSelectData != null && lastSelectData.fundId == _dataList[i].fundId)
            ...
            if (_m_wCurSelectItem == null && _m_lItemList.Count > 0)
            {
                if (_selectFundId > -1)
                {
                    ...
                    {
                        if (_m_lItemList[i] != null && _m_lItemList[i].fundSnapshot?.fundId == _selectFundId)
                        {
        ...
        //点击选中item
        private void _onClickItem(GGUIWndFundPageTabContainerItem _item, bool _moveToTop)
        {
            if (wnd == null || _item == null)
                return;

            if (_m_wCurSelectItem == null || _m_wCurSelectItem.fundSnapshot?.fundId != _item.fundSnapshot?.fundId)
            {
                _m_wCurSelectItem?.setSelect(false);
