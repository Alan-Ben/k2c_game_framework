...
namespace GOE
{
	...
	public abstract class NPBasicExportMenuItem_excelList<TTemp, Tobj, TMap> : NPBasicExportMenuItemEX where TTemp : new() where Tobj : _IALBasicRefObj, new() where TMap : _TALSOBasicRefSet<Tobj>, new()
	{
	    private string assetPath = "";
	    private string objName = "";

	    public NPBasicExportMenuItem_excelList(string _exportName
	    ...
	    private void exportGeneralRefSet()
	    {
			...
			foreach(string excelPath in excelPathArr)
			{
                ...
                //读取数据
                List<TTemp> readList = NPExportWnd.readXls<TTemp>(excelPath
                                                , tabName
                                                , exportEnum
                                                , _readRefInfo);
