...
| 协议号 | 协议名称 | 方向 | 说明 | 触发时机 |
|--------|----------|------|------|----------|
| 038_001 | ReqStartToGoMars | C->S | 请求开始前往火星 | 玩家点击开始航程 |
| 038_001 | RetStartToGoMars | S->C | 响应开始前往火星 | 请求返回 |
| 038_002 | ReqArriveMarsStage | C->S | 请求到达指定阶段 | 阶段时间到，点击下一阶段 |
| 038_002 | RetArriveMarsStage | S->C | 响应到达指定阶段 | 请求返回 |
| 038_003 | ReqSendDoneMarquee | C->S | 请求发送跑马灯 | 到达火星后触发 |
| 038_003 | RetSendDoneMarquee | S->C | 响应发送跑马灯 | 请求返回 |
| 038_004 | ReqSendStageMsg | C->S | 请求发送阶段留言 | 玩家发送留言 |
| 038_004 | RetSendStageMsg | S->C | 响应发送阶段留言 | 请求返回 |
| 038_005 | ReqGetStageMsgList | C->S | 请求获取留言列表 | 打开留言板 |
| 038_005 | RetGetStageMsgList | S->C | 响应留言列表 | 请求返回 |
| 038_050 | OnGoToStageArrived | S->C | 推送到达新阶段 | 阶段变更时 |
| 038_051 | OnGoToAllStageDone | S->C | 推送航程完成 | 完成所有阶段 |

### 协议依赖关系图

```mermaid
graph TD
    A[登录] --> B[GC2GS_002_072_ReqMarsGoRouteInit]
    B --> C[初始化航程数据]

    D[开始航程] --> E[GC2GS_038_001_ReqStartToGoMars]
    E --> F[GS2GC_038_001_RetStartToGoMars]
    F --> G[GS2GC_038_050_OnGoToStageArrived]

    H[阶段完成] --> I[GC2GS_038_002_ReqArriveMarsStage]
    I --> J[GS2GC_038_002_RetArriveMarsStage]
    J --> K{是否最后阶段}
    K -->|否| G
    K -->|是| L[GS2GC_038_051_OnGoToAllStageDone]

    L --> M[GC2GS_038_003_ReqSendDoneMarquee]
    M --> N[GS2GC_038_003_RetSendDoneMarquee]

    O[发送留言] --> P[GC2GS_038_004_ReqSendStageMsg]
    P --> Q[GS2GC_038_004_RetSendStageMsg]

    R[查看留言] --> S[GC2GS_038_005_ReqGetStageMsgList]
    S --> T[GS2GC_038_005_RetGetStageMsgList]
```
...
```mermaid
flowchart TD
    A[玩家请求发送跑马灯] --> B{航程状态检查}
    B -->|未到达火星| C[返回错误: 未到达火星]
    B -->|已到达| D{跑马灯状态检查}
    D -->|已发送| E[返回错误: 已发送过]
    D -->|未发送| F[发送全服跑马灯]
    F --> G[标记已发送]
...
|--------|------|------|------|
| stage | int | 阶段 | >= 1 |
| msgList | List\<Mars_GoRoute_StageMsg\> | 留言列表 | 按时间倒序 |
...
|--------|------|------|----------|
| stage | int | 当前阶段 | >= 1 |
| startMs | long | 开启前往火星时间（毫秒） | > 0 |
| stageStartMs | long | 当前阶段开始时间（毫秒） | > 0 |
...
// 批量创建
public static List<Mars_GoRoute_StageMsg> createStageMsgList(List<MarsGoRouteStageMsg> dbMsgList)
{
    List<Mars_GoRoute_StageMsg> result = new ArrayList<>();
    for (MarsGoRouteStageMsg dbMsg : dbMsgList)
    {
        result.add(createStageMsg(dbMsg));
    }
    return result;
}
```
...
| 参数名 | 类型 | 说明 | 取值范围 |
|--------|------|------|----------|
| isAllDone | bool | 是否全部完成（已到达火星） | true/false |
| stage | int | 当前阶段（0表示未开始） | >= 0 |
| startMs | long | 开始时间（毫秒） | >= 0 |
| stageStartMs | long | 当前阶段开始时间（毫秒） | >= 0 |
...
stateDiagram-v2
    [*] --> 未开始: 登录/刷新
    未开始 --> 进行中: 001_ReqStartToGoMars成功
    进行中 --> 等待确认: 阶段时间到
    等待确认 --> 进行中: 002_ReqArriveMarsStage成功
    等待确认 --> 航程完成: 最后阶段确认
    航程完成 --> [*]: 可进行其他玩法
...
/// 处理火星模块错误码
/// </summary>
public void handleMarsError(int errorCode)
{
    switch (errorCode)
    {
        case 38001: // 航程已在进行中
            UIManager.ShowToast("航程已在进行中");
            RefreshRouteStatus();
            break;

        case 38002: // 阶段顺序错误
            UIManager.ShowToast("阶段顺序错误");
            RequestRouteInit();
            break;

        case 38003: // 火星功能未解锁
            UIManager.ShowToast("火星功能未解锁");
            ShowUnlockCondition();
            break;

        case 38004: // 阶段时间未到
            UIManager.ShowToast("阶段时间未到");
            UpdateCountdown();
            break;

        case 38005: // 当前阶段不允许留言
            UIManager.ShowToast("当前阶段不允许留言");
            break;

        case 38006: // 已发送过留言
            UIManager.ShowToast("本阶段已发送过留言");
            UpdateMessageStatus();
            break;

        default:
            UIManager.ShowToast("操作失败，请重试");
            break;
    }
}
```
...
| 版本 | 变更内容 | 兼容性 | 说明 |
|------|----------|--------|------|
| v1.0.0 | 初始协议定义 | - | 基础航程功能 |
| v1.1.0 | 新增留言功能 (004, 005) | 向后兼容 | 添加社交功能 |
| v1.2.0 | 新增推送消息 (050, 051) | 向后兼容 | 优化推送机制 |
...
| 用例编号 | 测试场景 | 请求参数 | 预期结果 | 备注 |
|----------|----------|----------|----------|------|
| TC_001 | 正常开始航程 | 无参数 | 成功，推送stage=1 | 前置条件已满足 |
| TC_002 | 重复开始航程 | 无参数 | 失败，错误码38001 | 已在航程中 |
| TC_003 | 未解锁开始航程 | 无参数 | 失败，错误码38003 | 功能未解锁 |
| TC_004 | 正常到达阶段 | stage=2 | 成功，推送新阶段 | 阶段时间已到 |
| TC_005 | 阶段顺序错误 | stage=5 | 失败，错误码38002 | 跳跃阶段 |
| TC_006 | 阶段时间未到 | stage=2 | 失败，错误码38004 | 等待中 |
| TC_007 | 正常发送留言 | stage, content | 成功 | 首次发送 |
| TC_008 | 重复发送留言 | stage, content | 失败，错误码38006 | 已发送 |
| TC_009 | 留言内容过长 | stage, 201字符 | 失败，参数校验 | 超长内容 |
| TC_010 | 正常获取留言列表 | stage | 成功，返回列表 | 有数据 |
| TC_011 | 空留言列表 | stage | 成功，空列表 | 无数据 |
| TC_012 | 正常发送跑马灯 | 无参数 | 成功 | 首次到达火星 |
| TC_013 | 重复发送跑马灯 | 无参数 | 失败 | 已发送过 |
...
[Test]
public void TestOnGoToStageArrived()
{
    var msg = new GS2GC_038_050_OnGoToStageArrived();
    msg.stage = 1;
    msg.startMs = 1712808000000L;
    msg.stageStartMs = 1712808000000L;

    Assert.AreEqual(1, msg.getStage());
    Assert.AreEqual(1712808000000L, msg.getStartMs());
}

[Test]
public void TestRetGetStageMsgList()
{
    var msg = new GS2GC_038_005_RetGetStageMsgList();
    msg.stage = 1;
    msg.msgList = new List<Mars_GoRoute_StageMsg>
    {
        new Mars_GoRoute_StageMsg { playerName = "玩家1", content = "留言1", timeMs = 1000 }
    };

    Assert.AreEqual(1, msg.getStage());
    Assert.AreEqual(1, msg.getMsgList().Count);
}
```
...
| 协议 | 参数 | 校验规则 | 失败处理 |
|------|------|----------|----------|
| 038_002 | stage | 必须 > 0，必须连续 | 返回参数错误 |
| 038_004 | content | 长度 1-200，敏感词过滤 | 返回参数错误或过滤 |
| 038_005 | stage | 必须 > 0，必须存在 | 返回参数错误 |

### 服务端校验代码

```java
// 参数校验工具类
public class MarsParamValidator {

    public static boolean validateStage(int stage) {
        return stage > 0 && stage <= RefMarsGoRoute.Ref().getMaxStage();
    }
    ...
    public static String filterContent(String content) {
        // 敏感词过滤
        return WordFilterUtil.filter(content);
    }
}
```
...
```java
// 批量获取留言列表优化
public List<Mars_GoRoute_StageMsg> getStageMsgListBatch(int stage, int limit) {
    // 使用缓存
    String cacheKey = "mars:stage_msg:" + stage;
    List<Mars_GoRoute_StageMsg> cached = cacheManager.get(cacheKey);
    if (cached != null) {
        return cached;
    }

    // 数据库查询
    List<MarsGoRouteStageMsg> dbList = marsMsgDao.getMsgListByStage(stage, limit);
    List<Mars_GoRoute_StageMsg> result = createStageMsgList(dbList);

    // 缓存结果
    cacheManager.set(cacheKey, result, 60); // 60秒缓存
...
```java
// 大数据量协议压缩
public byte[] compressProtocol(_ALProtocolStructure msg) {
