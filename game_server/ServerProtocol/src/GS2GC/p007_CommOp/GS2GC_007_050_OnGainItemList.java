...
public class GS2GC_007_050_OnGainItemList implements ALBasicProtocolPack._IALProtocolStructure {
/** 奖励展示样式 */
private NPEnum.ENpRewardShowType rewardShowType;
/** 物品列表 */
private java.util.ArrayList<NPCommon.NPCommon_ItemInfo> itemList;


public GS2GC_007_050_OnGainItemList() {
	rewardShowType = NPEnum.ENpRewardShowType.values()[0];
	itemList = new java.util.ArrayList<NPCommon.NPCommon_ItemInfo>();
}

public GS2GC_007_050_OnGainItemList(
	 NPEnum.ENpRewardShowType _rewardShowType
	, java.util.ArrayList<NPCommon.NPCommon_ItemInfo> _itemList
) {	rewardShowType = _rewardShowType;
	itemList = _itemList;
}
...
/** 奖励展示样式 */
public NPEnum.ENpRewardShowType getRewardShowType() { return rewardShowType; }
/** 奖励展示样式 */
public void setRewardShowType(NPEnum.ENpRewardShowType _rewardShowType) { rewardShowType = _rewardShowType; }
/** 物品列表 */
...
public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	...
	if(_buf.remaining() > 0) rewardShowType = NPEnum.ENpRewardShowType.ENpRewardShowType_FromInt(_buf.getInt());
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
...
public final void PutUnzipBuf(ByteBuffer _buf) {
	_buf.putInt(rewardShowType.ordinal());
