...
{
    /// <summary>
    /// Unity VideoPlayer的封装实现
    /// 继承自原有的_ALVideoPlayerDealer，实现IALVideoPlayerCore接口
    /// </summary>
    public class ALVideoPlayerDealerUnity : _AALVideoPlayerDealer
    {
        //构造的原始对象
        ...
        public ALVideoPlayerDealerUnity(Transform _parentTrans, string _name)
        {
            //创建空go
            ...
            //初始化相关参数
            _m_vpPlayer.source = VideoSource.VideoClip;
        ...
        public override void prepare(_IALVideoResource _resource, Action _prePrepare, Action<bool, string> _onPrepareDone)
        {
            ...
            if (!(_resource is ALVideoResourceClip clipResource))
            {
                Debug.LogError("Unity VideoPlayer only supports ALVideoResourceClip");
                if (null != _onPrepareDone)
                    _onPrepareDone(false, "Resource format err!");
                return;
            }

            VideoClip _clip = clipResource.videoClip;
        ...
        {
            if (_m_vpPlayer != null && _m_vpPlayer.clip != null) 
                return _m_vpPlayer.clip.GetInstanceID();
