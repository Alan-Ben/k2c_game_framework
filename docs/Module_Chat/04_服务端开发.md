...
```json
{
  "rate_limit": {
    ...
    "per_player": {
      ...
      "max_private_messages_per_minute": 30,
      "max_join_room_per_minute": 10
    },
    "per_room": {
...
| 指标名称 | 采集频率 | 告警阈值 | 说明 |
|---------|---------|---------|------|
| 在线玩家数 | 10s | > 80%容量 | 容量预警 |
| 消息延迟 | 5s | > 500ms | 响应时间告警 |
| 消息吞吐量 | 10s | < 1000/s | 业务量异常 |
| 消息队列积压 | 5s | > 10000 | 处理能力告警 |
| GC暂停时间 | 1min | > 500ms | JVM性能告警 |
| Redis连接数 | 10s | > 80% | 连接池告警 |
