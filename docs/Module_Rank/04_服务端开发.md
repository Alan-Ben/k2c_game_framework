...
public RankFixed_LikeResult like(long playerId, long rankFixedId, boolean isCross) {
    ...
    if (topPlayer == null || topPlayer.isEmpty()) {
        throw new BusinessException(ErrorCode.RANK_EMPTY);
    }
    ...
    if (todayCount > config.getMaxLikeCount()) {
        throw new BusinessException(ErrorCode.LIKE_COUNT_EXCEEDED);
    }
...
- 使用索引：`idx_rank_type_score` 加速排名查询
- 分库分表：按服务器ID分表
- 定期归档：历史数据归档到历史表
