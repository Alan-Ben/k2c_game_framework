...
@RefBo(isIdAuto= true)
public class SeasonRankBO extends BaseBO {
    ...
    public static final int FIELD_serverId =1;
    ...
    public SeasonRankBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        uid = rs.getLong(2);
        serverId = rs.getInt(3);
        playername = rs.getString(4);
        icon = rs.getLong(5);
        iconBgk = rs.getLong(6);
        grades = rs.getInt(7);
        starhoner = rs.getInt(8);
        legendscore = rs.getLong(9);
        lastRankTime = rs.getInt(10);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new SeasonRankBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `uid`, `serverId`, `playername`, `icon`, `iconBgk`, `grades`, `starhoner`, `legendscore`, `lastRankTime`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(lastRankTime).append("', ");
    ...
    // 最近一次排名变更时间
    public int getLastRankTime() { return this.lastRankTime; }
    public void setLastRankTime(BM _bm, int lastRankTime) {
        if(lastRankTime==this.lastRankTime) 
            return;
        this.lastRankTime = lastRankTime; 
        markField(_bm, FIELD_lastRankTime); 
    }
    public void saveLastRankTime(BM _bm, int lastRankTime) {
        if(lastRankTime==this.lastRankTime) 
            return;
        this.lastRankTime = lastRankTime;
        saveField(_bm, "lastRankTime", lastRankTime);
    }
    ...
    public String getUpdateKeyValue() {
        ...
        sBuilder.append(" `lastRankTime` = '").append(lastRankTime).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_lastRankTime)) sBuilder.append(" `lastRankTime` = '").append(lastRankTime).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
	
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `seasonRank` ("
                ...
                + "`lastRankTime` int(11) NOT NULL DEFAULT '0' COMMENT '最近一次排名变更时间',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='赛季排行榜' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
