...
namespace UnityEngine.UI
{
    ...
    [AddComponentMenu("UI/Legacy/Input Field Emoji", 103)]
    public class InputFieldEmoji
        : Selectable,
        ...
        ILayoutElement
    {
        ...
        private string m_HasInvTransText = "";
        private string m_InvTransText = "";
        ...
        {
            return m_TextComponent != null && m_TextComponent.emojiAtlals != null;
        }
        ...
        private void SetKeyboardText(string str)
        {
            ...
            // Debug.LogWarning($"Emoji  SetKeyboardText '{m_Keyboard.text}'");
        }
        ...
        protected TextEmoji m_TextComponent;
        ...
        public string text
        {
            get
            {
                if (CheckEmojiSupport())
                {
                    if (m_HasInvTransText != m_Text)
                    {
                        m_InvTransText = m_TextComponent.emojiAtlals.InvTranslate(m_Text);
                        m_HasInvTransText = m_Text;
                    }
                    return m_InvTransText;
                }
                return m_Text;
            }
            set
        ...
        /// </summary>
        public TextEmoji textComponent
        {
        ...
        {
            if (pos < 0) pos = 0;
            else if (pos > m_Text.Length) pos = m_Text.Length;
        }
        ...
        {
            caretPositionInternal = m_Text.Length;
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (m_Keyboard == null || m_Keyboard.status != TouchScreenKeyboard.Status.Visible)
            {
                if (m_Keyboard != null)
                {
                    if (!m_ReadOnly)
                    {
                        if (CheckEmojiSupport())
                        {
                            text = m_TextComponent.emojiAtlals.Translate(m_Keyboard.text);
                        }
                        else
                        {
            ...
            if (m_KeyboardText != val)
            {
                ...
                else
                {
                    ...
                    {
                        m_KeyboardText = val;
                        val = m_TextComponent.emojiAtlals.Translate(val);
                    }
        ...
        public virtual void OnUpdateSelected(BaseEventData eventData)
        {
            ...
            string strJoined = "";//将while循环内的输入character拼接成string 去统一处理
            ...
            {
                // Don't allow return chars or tabulator key to be entered into single line fields.
                if (multiLine || strJoined.Length != 1 ||
                    (strJoined[0] != '\t' && strJoined[0] != '\r' && strJoined[0] != 10))
                {
                    Append(strJoined);
                    consumedEvent = true;
                }
        ...
        private string GetSelectedString()
        {
            ...
            var selectedString = m_Text.Substring(startPos, endPos - startPos);
            if (CheckEmojiSupport())
            {
                selectedString = m_TextComponent.emojiAtlals.InvTranslate(selectedString);
            }
        ...
        private int LineDownCharacterPosition(int originalPos, bool goToLastChar)
        {
            ...
            if (originLine + 1 >= cachedInputTextGenerator.lineCount)
                return goToLastChar ? m_Text.Length : originalPos;
        ...
        private void MoveDown(bool shift, bool goToLastChar)
        {
            ...
            int position = multiLine ? LineDownCharacterPosition(caretSelectPositionInternal, goToLastChar) : m_Text.Length;
        ...
        private void Delete()
        {
            ...
            if (caretPositionInternal < caretSelectPositionInternal)
            {
                m_Text = m_Text.Substring(0, caretPositionInternal) + m_Text.Substring(caretSelectPositionInternal, m_Text.Length - caretSelectPositionInternal);
                caretSelectPositionInternal = caretPositionInternal;
            }
            else
            {
                m_Text = m_Text.Substring(0, caretSelectPositionInternal) + m_Text.Substring(caretPositionInternal, m_Text.Length - caretPositionInternal);
        ...
        private void ForwardSpace()
        {
            ...
            else
            {
                ...
                {
                    m_Text = m_Text.Remove(caretPositionInternal, 1);
        ...
        private void Backspace()
        {
            ...
            {
                if (caretPositionInternal > 0 && caretPositionInternal - 1 < m_Text.Length)
                {
                    m_Text = m_Text.Remove(caretPositionInternal - 1, 1);
        ...
        private void Insert(char c)
        {
            ...
            m_Text = m_Text.Insert(m_CaretPosition, replaceString);
        ...
        {
            // Update the TouchKeyboard's m_Text from edit changes
        ...
        /// <summary>
        /// Append the specified m_Text to the end of the current m_Text string. Appends character by character testing validation criteria.
        /// </summary>
        ...
        protected virtual void Append(string input)
        {
            ...
            if(CheckEmojiSupport())
            {
                input = m_TextComponent.emojiAtlals.Translate(input);
            }
        ...
        protected virtual void Append(char input)
        {
            ...
            //Get the m_Text based on selection for validation instead of whole m_Text(case 1253193).
            var validateText = m_Text;
            if (selectionFocusPosition != selectionAnchorPosition)
            {
                if (caretPositionInternal < caretSelectPositionInternal)
                {
                    validateText = m_Text.Substring(0, caretPositionInternal) + m_Text.Substring(caretSelectPositionInternal, m_Text.Length - caretSelectPositionInternal);
                }
                else
                {
                    validateText = m_Text.Substring(0, caretSelectPositionInternal) + m_Text.Substring(caretPositionInternal, m_Text.Length - caretPositionInternal);
                }
            }
        ...
        protected void UpdateLabel()
        {
            if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
            {
                ...
                //
                // This callback also occurs when assigning m_Text to our Text component, i.e.,
                // m_TextComponent.m_Text = processed;
                ...
                if (EventSystem.current != null && gameObject == EventSystem.current.currentSelectedGameObject && compositionString.Length > 0)
                {
                    ...
                    fullText = m_Text.Substring(0, m_CaretPosition) + compositionString + m_Text.Substring(m_CaretPosition);
                }
                else
                {
                ...
                // If not currently editing the m_Text, set the visible range to the whole m_Text.
                // The UpdateLabel method will then truncate it to the part that fits inside the Text area.
                // We can't do this when m_Text is being edited since it would discard the current scroll,
                ...
                string strTranslatedTxt = processed; // 被转译过的文本，提供给Text组件渲染使用
                ...
                if (!isEmpty)
                {
                    ...
                    // 把转码后的Emoji字符串中的自定义编码部分替换为U+2001提供给TextGenerator生成文本渲染数据
                    if(CheckEmojiSupport())
                    {
                        if(Regex.IsMatch(processed, @"[\uE000-\uF8FF]"))
                        {
                            processed = EmojiAtlas.ReplaceEmojiToSpace(processed);
                        }
                    }
                ...
                if(CheckEmojiSupport())
                {
                    TextEmoji soarDText = m_TextComponent as TextEmoji;
                    if(soarDText != null)
                    {
                        // 截取合Processed等长的转译结果文本，通过SetEmojiText接口设置给文本
                        if(strTranslatedTxt != processed)
                        {
                            strTranslatedTxt = strTranslatedTxt.Substring(m_DrawStart, processed.Length);
                        }

                        // Debug.LogWarning($"Emoji  UpdateLabel end text:{text};translate:{strTranslatedTxt} toSpace:{processed}'");
                        soarDText.SetEmojiText(text, strTranslatedTxt, processed);
                    }
                    else
                    {
        ...
        private void ActivateInputFieldInternal()
        {
            ...
            if (CheckEmojiSupport())
            {
                m_KeyboardText = m_TextComponent.emojiAtlals.InvTranslate(m_Text);
            }
            else
            {
            ...
            if (TouchScreenKeyboardShouldBeUsed())
            {
                ...
                m_Keyboard = (inputType == InputType.Password) ?
                    TouchScreenKeyboard.Open(m_KeyboardText, keyboardType, false, multiLine, true, false, "", characterLimit) :
                    TouchScreenKeyboard.Open(m_KeyboardText, keyboardType, inputType == InputType.AutoCorrect, multiLine, false, false, "", characterLimit);
