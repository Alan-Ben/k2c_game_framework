...
namespace ALPackage
{
     ...
     * 缓存池模板对象
     **/
    public abstract class _AALUnsafeThreadCacheController<T, TEMP>
    {
        //是否初始化状态
        ...
        protected _AALUnsafeThreadCacheController(int _minCount, int _maxCount)
        {
            _m_bIsInit = false;
        ...
        protected _AALUnsafeThreadCacheController(int _minCount, int _maxCount, int _addUnit)
        {
            _m_bIsInit = false;
            _m_tTemplateObj = default(TEMP);
