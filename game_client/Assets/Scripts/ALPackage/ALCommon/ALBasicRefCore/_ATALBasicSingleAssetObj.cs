 ...
 * 单个asset资源对象的管理对象
 **/
public abstract class _ATALBasicSingleAssetObj<T> where T : UnityEngine.Object
{
    /** 是否初始化的状态变量 */
    ...
    protected _ATALBasicSingleAssetObj()
    {
        _m_bInited = false;
    ...
    public void init(Action _sucDelegate, Action _failDelegate)
    {
        ...
        //开始加载
#if UNITY_EDITOR
        ALLocalResLoaderMgr.instance.loadGameResObjAsset(_resPath, _objName, _onObjLoaded, null, _onLocalObjLoaded, _resCore);
#else
