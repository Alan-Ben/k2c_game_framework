...
namespace ALPackage
{
    ...
    /// </summary>
    public class ALCommonStepActionMonoTask : _IALBaseMonoTask, _IALCommonTaskMonitorInterface
    {
        #region 对外接口
        /// <summary>
        ...
        /// <returns></returns>
        protected internal static ALCommonStepActionMonoTask _createTask(Action _action, Action _doneDelegate
#if UNITY_EDITOR 
            , ALCommonTaskMonitorContainer _container
#endif
            , Action _failDelegate = null, bool _isFailDoDone = false)
        {
            ALCommonStepActionMonoTask task = ALCommonStepActionTaskCache.instance.popItem();
...
#if AL_UNITY_GUI
        public static void addGuiMonoTask(Action _action, Action _doneDelegate, Action _failDelegate = null, bool _isFailDoDone = false
#if UNITY_EDITOR 
            , ALCommonTaskMonitorContainer _container = null
#endif
            )
        ...
        public static void addGuiMonoTask(Action _action, Action _doneDelegate, Action _failDelegate = null, bool _isFailDoDone = false, float _delayTime = 0f
#if UNITY_EDITOR 
            , ALCommonTaskMonitorContainer _container = null
#endif
            )
...
#if AL_UNITY_GUI
        public static void addGuiNextFrameTask(Action _action, Action _doneDelegate, Action _failDelegate = null, bool _isFailDoDone = false
#if UNITY_EDITOR 
            , ALCommonTaskMonitorContainer _container = null
#endif
            )
        ...
        protected ALCommonStepActionMonoTask()
        {
            _m_dAction = null;
            _m_dDoneAction = null;
            _m_dFailAction = null;
            ...
            )
        {
            ...
            if(_action == null)
            {
                UnityEngine.Debug.LogError("ALCommonStepActionMonoTask 传入Action为空");
            }
#endif
            _m_dAction = _action;
        ...
        public void deal()
        {
            ...
            //放回缓存
            ALCommonStepActionTaskCache.instance.pushBackCacheItem(this);
        }
        ...
        public override string ToString()
        {
            if(_m_dAction != null)
            {
                return $"ALCommonStepActionMonoTask:{ALCommon.ToReadableString(_m_dAction)}";
            }
            else
            {
                return $"ALCommonStepActionMonoTask:null";
            }
        }
        
        public class ALCommonStepActionTaskCache : _AALUnsafeThreadCacheController<ALCommonStepActionMonoTask, ALCommonStepActionMonoTask>
        {
            private static ALCommonStepActionTaskCache _g_instance = new ALCommonStepActionTaskCache();
            public static ALCommonStepActionTaskCache instance
            {
                get
                {
                    if(null == _g_instance)
                        _g_instance = new ALCommonStepActionTaskCache();
                    return _g_instance;
                }
            }

            public ALCommonStepActionTaskCache() : base(32, 128)
            {
                init(new ALCommonStepActionMonoTask());
            }

            protected override ALCommonStepActionMonoTask _createItem(ALCommonStepActionMonoTask _template)
            {
                return new ALCommonStepActionMonoTask();
            }

            //警告信息文字
            protected override string _warningTxt { get { return "ALCommonStepActionTaskCache"; } }

            protected override void _discardItem(ALCommonStepActionMonoTask _item)
            {
                _item.discard();
                return;
            }

            protected override void _onInit(ALCommonStepActionMonoTask _template)
            {
            }

            protected override void _resetItem(ALCommonStepActionMonoTask _item)
            {
                _item._reset();
            }
        }
