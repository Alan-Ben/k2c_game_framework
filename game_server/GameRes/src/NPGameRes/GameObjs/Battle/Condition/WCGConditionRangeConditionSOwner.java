...
public class WCGConditionRangeConditionSOwner extends _AWCGBasicSingleCondition
{
    ...
    {
        return EWCGSingleConditionType.RNG_S_O_COND;
    }

    public static WCGConditionRangeConditionSOwner read(String _infoStr)
    {
        WCGConditionRangeConditionSOwner obj = new WCGConditionRangeConditionSOwner();
        ...
        try
        {
            ...
            if (strs.length > 2)
            {
                String errStr = "RNG_S_O_COND 内置条件配置错误: " + strs[2];
                obj._m_lConditionList = WCGSingleConditionGroupObj.readConditionGroupList(strs[2], errStr);
            }
            return obj;
        } catch (Exception e)
        {
            CommLog.error("条件配置错误 - RNG_S_O_COND example: enum:range:enemy_type:conditionStr Error Str: " + _infoStr, e);
            return null;
        }
    }
}