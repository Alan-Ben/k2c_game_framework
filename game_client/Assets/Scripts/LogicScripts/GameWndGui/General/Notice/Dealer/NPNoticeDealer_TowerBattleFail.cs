...
namespace GOE
{
    ...
    /// 爬塔挑战失败 notice
    /// </summary>
    public class NPNoticeDealer_TowerBattleFail : NPUINoticeMgr._ANPUINoticeDealer
    {
        private TowerChallengeResult _m_info;
        //窗口是否已经加载，如果已经加载的情况下。在shownotice的时候不会再加载，避免窗口驻留
        private bool _m_bWndLoaded;

        public NPNoticeDealer_TowerBattleFail(TowerChallengeResult _info)
        {
            _m_info = _info;
        }
        ...
        protected override void _onDealerDone()
        {
            if (_m_bWndLoaded)
            {
                GGUIWndTowerBattleFail.instance.discard();
                _m_bWndLoaded = false;
            }
        }
        ...
        public override void dealShowNotice()
        {
            ...
            {
                _m_bWndLoaded = true;
                GGUIWndTowerBattleFail.instance.load(() =>
                {
                    GGUIWndTowerBattleFail.instance.showWnd();
                    GGUIWndTowerBattleFail.instance.setInfo(_m_info, setDealerDone);
                });
            }
            else
            {
                //已经加载的，需要刷新层级
                if (null != GGUIWndTowerBattleFail.instance.wnd)
                    GCommon.moveTransformToLastAndRefreshLayer(GGUIWndTowerBattleFail.instance.rectTransform);
            }
        }
