...
@RefBo(isIdAuto= true)
public class NpCrossRankBO extends BaseBO {
    ...
    public NpCrossRankBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        rank_id = rs.getLong(2);
        cross_instance_id = rs.getLong(3);
        is_discard = rs.getBoolean(4);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new NpCrossRankBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `rank_id`, `cross_instance_id`, `is_discard`";
    }

    @Override
    public String getTableName() {
        return "`np_cross_rank`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `np_cross_rank` ("
