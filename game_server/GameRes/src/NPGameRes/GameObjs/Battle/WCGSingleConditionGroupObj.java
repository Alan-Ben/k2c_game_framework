...
public class WCGSingleConditionGroupObj
{
    ...
    public String toString()
    {
        ...
        sb.append(conditionObj.conditionType.toString());
        sb.append(":[");
        for (int i = 0; i < childConditionList.size(); i++)
        {
            sb.append(childConditionList.toString());
            sb.append(",");
        }
        sb.append("]");
        return sb.toString();
    ...
    public WCGSingleConditionObj conditionObj;
    public List<WCGSingleConditionGroupObj> childConditionList;
    public EWCGJudgeChar judgeType = EWCGJudgeChar.NONE;

    protected WCGSingleConditionGroupObj()
    {
        trueEnable = true;
        conditionObj = new WCGSingleConditionObj();
        childConditionList = new ArrayList<WCGSingleConditionGroupObj>();
    }

    public boolean hasCondition()
    {
        return (conditionObj != null && conditionObj.conditionType != EWCGSingleConditionType.NONE)
    ...
    // * 将字符串转化为本对象
    // **/
    //public static List<WCGSingleConditionGroupObj> readConditionGroupList(String _str)
    //{
    //    List<WCGSingleConditionGroupObj> list = new ArrayList<WCGSingleConditionGroupObj>();
    ...
    //        WCGSingleConditionGroupObj obj = new WCGSingleConditionGroupObj();
    //        obj.conditionList = WCGSingleConditionObj.readConditionList(infoStr);
    //        if (null == obj.conditionList)
    //            continue;
    ...
    //将字符串转化为本对象
    public static WCGSingleConditionGroupObj readConditionGroupList(String _str, String _err)
    {
        return readConditionGroupList(_str, 0, 1, _err);
    }

    public static WCGSingleConditionGroupObj readConditionGroupList(String _str, int _startIdx, int _depth, String _err)
    {
        if (_depth > 5)
        {
            CommLog.error(_err);
            CommLog.error(" Notice!!! Single Condition depth more than 5!!! conditionStr: " + _str + " depth: " + _depth);
        }
        ...
        do
        {
            ...
            if (index == startIdx)
            {
                ...
                if (tmpChar == _g_bracket_b)
                {
                    ...
                    //读取内容对象
                    //UnityEngine.Debug.LogError(" sub conddi11: " + infoStr.ToString());
                    WCGSingleConditionGroupObj newObj = readConditionGroupList(infoStr.toString(), 0, _depth + 1, _err);
                    //设置正向还是反向判断
            ...
            } else if (index != -1)//此时取出字符串进行条件读取
            {
                ...
                WCGSingleConditionGroupObj newObj = new WCGSingleConditionGroupObj();
                newObj.conditionObj = WCGSingleConditionObj.readCondition(infoStr.toString());
                //设置正向还是反向判断
                newObj.trueEnable = isTrueEnable;
                //重置正向
                isTrueEnable = true;
            ...
            } else //此时取出字符串进行条件读取
            {
                ...
                //UnityEngine.Debug.LogError(index + " - " + startIdx + " single conddi: " + infoStr.ToString());
                //取完字符串，使用截取的字符串读取对应条件
                WCGSingleConditionGroupObj newObj = new WCGSingleConditionGroupObj();
                newObj.conditionObj = WCGSingleConditionObj.readCondition(infoStr.toString());
                //设置正向还是反向判断
