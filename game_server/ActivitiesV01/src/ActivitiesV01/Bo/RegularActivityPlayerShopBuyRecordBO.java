...
@RefBo(isIdAuto= true)
public class RegularActivityPlayerShopBuyRecordBO extends BaseBO {
    ...
    @DataBaseField(type = "bigint(20)", fieldname = "shop_db_id", comment = "商店实例id")
    ...
    public RegularActivityPlayerShopBuyRecordBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        instance_id = rs.getLong(2);
        shop_db_id = rs.getLong(3);
        cid = rs.getLong(4);
        item_id = rs.getLong(5);
        had_buy_count = rs.getLong(6);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new RegularActivityPlayerShopBuyRecordBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `instance_id`, `shop_db_id`, `cid`, `item_id`, `had_buy_count`";
    }

    @Override
    public String getTableName() {
        return "`regular_activity_player_shop_buy_record`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `regular_activity_player_shop_buy_record` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`instance_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '活动实例ID',"
                + "`shop_db_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '商店实例id',"
                ...
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='活动玩家商店购买记录数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
