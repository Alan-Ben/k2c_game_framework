...
//变体优化版AlphaModulate
half3 AlphaModulate_VariantOptimize(float AlphaPremultiply_On,float AlphaModulate_On,half3 albedo, half alpha)
{
    if(AlphaModulate_On == 1)
        return lerp(half3(1.0h, 1.0h, 1.0h), albedo, alpha);
    
    if(AlphaPremultiply_On == 1)
        return albedo * alpha;
    return albedo;
}
