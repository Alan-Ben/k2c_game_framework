...
public class MCC_Common_User implements ALBasicProtocolPack._IALProtocolStructure {
private long uid;
...
public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	...
	if(_buf.remaining() > 0) uid = _buf.getLong();
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
	if(_buf.remaining() > 0) gameUid = _buf.getLong();
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
	if(_buf.remaining() <= 0) return;
	int _extraInfoCustLen = _buf.getInt();
	int _extraInfoCurPos = _buf.position();
	extraInfo.ReadUnzipBuf(_buf, _extraInfoCurPos + _extraInfoCustLen);
	_buf.position(_extraInfoCurPos + _extraInfoCustLen);
