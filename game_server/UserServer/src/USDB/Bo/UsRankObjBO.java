...
@RefBo(isIdAuto= true)
public class UsRankObjBO extends BaseBO {
    
    @DataBaseField(type = "bigint(20)", fieldname = "id", comment = "数据库表唯一键值")
    private long id;

    public static final int FIELD_instanceId =0;
    @DataBaseField(type = "bigint(20)", fieldname = "instanceId", comment = "排行榜实例ID")
    ...
    public UsRankObjBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        instanceId = rs.getLong(2);
        rankId = rs.getLong(3);
        objId = rs.getLong(4);
        score_source_id = rs.getLong(5);
        score = rs.getLong(6);
        updatedMs = rs.getLong(7);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new UsRankObjBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `instanceId`, `rankId`, `objId`, `score_source_id`, `score`, `updatedMs`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `us_rank_obj` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`instanceId` bigint(20) NOT NULL DEFAULT '0' COMMENT '排行榜实例ID',"
