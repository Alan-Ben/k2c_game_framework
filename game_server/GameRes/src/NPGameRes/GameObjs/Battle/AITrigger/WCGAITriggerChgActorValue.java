...
/// </summary>
public class WCGAITriggerChgActorValue extends _AWCGBasicAITrigger
{
    ...
    {
        return EWCGAITriggerType.CHG_ACTOR_V;
    }

    public static WCGAITriggerChgActorValue read(String _infoStr)
    {
        WCGAITriggerChgActorValue obj = new WCGAITriggerChgActorValue();
        String[] strs = CommonFunc.charSplit(_infoStr, ':');
        if (strs.length < 2)
        {
            CommLog.error("ai CHG_ACTOR_V配置错误 正确配置: 枚举:下标索引:值");
            return null;
        }
        try
        {
