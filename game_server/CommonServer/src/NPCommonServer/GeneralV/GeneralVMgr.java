...
public class GeneralVMgr
{
    ...
    private GeneralVMgr()
    {
        ENPCommonGeneralEnum[] arrTypes = ENPCommonGeneralEnum.values();
        _m_arrObjs = new GeneralVObj[arrTypes.length];
        for (ENPCommonGeneralEnum eType : arrTypes)
        {
            _m_arrObjs[eType.ordinal()] = new GeneralVObj(eType);
        }
    }

    public GeneralVObj getVObj(ENPCommonGeneralEnum _eType)
    {
    ...
    {
        //初始化全局变量
        List<GeneralVBO> boList = NPCommonServer.getInstance().getBM().getBM(GeneralVBO.class).s_findAll();
        if (null == boList)
        {
            return false;
        }
        for (GeneralVBO generalVBO : boList)
        {
            ...
            if (type < 0 || type >= ENPCommonGeneralEnum.values().length)
            {
                CommLog.error("GeneralVBO invalid type:{}", type);
                continue;
            }
            ENPCommonGeneralEnum eType = ENPCommonGeneralEnum.values()[type];
    ...
    public long makeNewId(ENPCommonGeneralEnum _eType)
    {
