...
@RefBo(isIdAuto= true)
public class UsStepRewardEventTaskBO extends BaseBO {
    ...
    public static final int FIELD_event_task_id =2;
    @DataBaseField(type = "bigint(20)", fieldname = "event_task_id", comment = "阶段任务ID")
    private long event_task_id;

    public static final int FIELD_score =3;
    @DataBaseField(type = "bigint(20)", fieldname = "score", comment = "积分")
    private long score;

    public UsStepRewardEventTaskBO() {
        id = 0;
    ...
    public UsStepRewardEventTaskBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        step_reward_instance_id = rs.getLong(2);
        cid = rs.getLong(3);
        event_task_id = rs.getLong(4);
        score = rs.getLong(5);
    }
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new UsStepRewardEventTaskBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `step_reward_instance_id`, `cid`, `event_task_id`, `score`";
    }

    @Override
    public String getTableName() {
        return "`us_step_reward_event_task`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(event_task_id).append("', ");
    ...
    // 阶段任务ID
    public long getEventTaskId() { return this.event_task_id; }
    public void setEventTaskId(BM _bm, long event_task_id) {
        if(event_task_id==this.event_task_id) 
            return;
        this.event_task_id = event_task_id; 
        markField(_bm, FIELD_event_task_id); 
    }
    public void saveEventTaskId(BM _bm, long event_task_id) {
        if(event_task_id==this.event_task_id) 
            return;
        this.event_task_id = event_task_id;
        saveField(_bm, "event_task_id", event_task_id);
    }
    ...
    public String getUpdateKeyValue() {
        ...
        sBuilder.append(" `event_task_id` = '").append(event_task_id).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_event_task_id)) sBuilder.append(" `event_task_id` = '").append(event_task_id).append("',");
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `us_step_reward_event_task` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`step_reward_instance_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '阶段奖励实例ID',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家cid',"
                + "`event_task_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '阶段任务ID',"
                + "`score` bigint(20) NOT NULL DEFAULT '0' COMMENT '积分',"
                + "KEY `step_reward_instance_id` (`step_reward_instance_id`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='阶段奖励事件任务数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
