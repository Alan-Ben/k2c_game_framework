...
stateDiagram-v2
    [*] --> Idle
    Idle --> Forward: 点击前进按钮
    Forward --> ForwardAnim: 播放前进动画
    ForwardAnim --> RewardShow: 动画完成
    RewardShow --> Idle: 关闭奖励界面

    Idle --> BossFight: 点击Boss战按钮
    BossFight --> BossAppearAnim: Boss出场动画
    BossAppearAnim --> BossFightAnim: Boss战斗动画
    BossFightAnim --> BossWinAnim: 战斗胜利
    BossFightAnim --> BossFailAnim: 战斗失败
    BossWinAnim --> Idle: 显示奖励
    BossFailAnim --> Idle: 返回界面

    Idle --> Event: 触发事件
    Event --> EventShowAnim: 事件弹出动画
    EventShowAnim --> EventDeal: 处理事件
    EventDeal --> EventCloseAnim: 关闭动画
    EventCloseAnim --> Idle: 动画完成

    note right of ForwardAnim
        前进动画包含:
        - 节点移动
        - 特效播放
        - 音效触发
...
public class ChapterAnimationController : MonoBehaviour
{
    ...
    [SerializeField] private Animator _eventAnimator;

    [Header("动画状态名称")]
    [SerializeField] private string _idleState = "Idle";
    [SerializeField] private string _forwardState = "Forward";
    [SerializeField] private string _bossAppearState = "BossAppear";
    [SerializeField] private string _bossFightState = "BossFight";
    [SerializeField] private string _bossWinState = "BossWin";
    ...
    /// 播放前进动画
    /// </summary>
    public void PlayForwardAnimation(int _fromPoint, int _toPoint, Action _onComplete)
    {
        _onAnimationComplete = _onComplete;

        // 设置动画参数
        _mainAnimator.SetInteger("FromPoint", _fromPoint);
        _mainAnimator.SetInteger("ToPoint", _toPoint);
        _mainAnimator.SetTrigger("Forward");

        // 启动动画监控
        StartAnimationMonitor(_forwardState);
    }

    /// <summary>
    /// 播放Boss出场动画
    /// </summary>
    public void PlayBossAppearAnimation(ChapterBossStyleRefObj _bossStyle, Action _onComplete)
    {
        _onAnimationComplete = _onComplete;

        // 设置Boss动画参数
        _bossAnimator.SetTrigger("Appear");

        // 启动动画监控
        StartAnimationMonitor(_bossAppearState);
    }

    /// <summary>
    /// 播放Boss战斗动画
    /// </summary>
    public void PlayBossFightAnimation(Action _onComplete)
    {
        _onAnimationComplete = _onComplete;
        _bossAnimator.SetTrigger("Fight");
        StartAnimationMonitor(_bossFightState);
    }

    /// <summary>
    /// 播放Boss胜利动画
    /// </summary>
    public void PlayBossWinAnimation(List<NPCommon_ItemInfo> _rewards, Action _onComplete)
    {
        _onAnimationComplete = _onComplete;
        _bossAnimator.SetTrigger("Win");
        StartAnimationMonitor(_bossWinState);
    }

    /// <summary>
    /// 启动动画监控
    /// </summary>
    private void StartAnimationMonitor(string _stateName)
    {
        if (_currentAnimationCoroutine != null)
        {
            StopCoroutine(_currentAnimationCoroutine);
        }
        _currentAnimationCoroutine = StartCoroutine(MonitorAnimation(_stateName));
    }

    /// <summary>
    /// 监控动画完成
    /// </summary>
    private IEnumerator MonitorAnimation(string _stateName)
    {
        // 等待进入目标状态
        yield return new WaitUntil(() =>
        {
            var stateInfo = _mainAnimator.GetCurrentAnimatorStateInfo(0);
            return stateInfo.IsName(_stateName);
        });
    ...
    public void SkipCurrentAnimation()
    {
        if (_currentAnimationCoroutine != null)
        {
            ...
            // 强制切换到Idle状态
            _mainAnimator.Play(_idleState, 0, 1.0f);
...
classDiagram
    ...
    class ChapterEffectPlayer {
        -Dictionary~long,GameObject~ _effectPool
        +PlayEffect(config, position)
        +StopEffect(effectId)
        +PreloadEffects(ids)
    }

    ChapterEffectPlayer --> ChapterEffectConfig
    ChapterEffectPlayer --> ChapterEffectType
```
...
/// 章节特效播放器
/// </summary>
public class ChapterEffectPlayer : MonoBehaviour
{
    [Header("特效容器")]
    [SerializeField] private Transform _effectRoot;

    // 特效对象池
    private Dictionary<long, Queue<GameObject>> _effectPool = new();
    private Dictionary<long, GameObject> _activeEffects = new();

    /// <summary>
    /// 预加载特效
    /// </summary>
    public IEnumerator PreloadEffects(List<long> _effectIds)
    {
        foreach (var effectId in _effectIds)
        {
            var config = GetEffectConfig(effectId);
            if (config == null) continue;

            // 异步加载预制体
            var loader = Resources.LoadAsync<GameObject>(config.prefabPath);
            yield return loader;

            if (loader.asset != null)
            {
                // 创建对象池
                if (!_effectPool.ContainsKey(effectId))
                {
                    _effectPool[effectId] = new Queue<GameObject>();
                }

                // 预创建2个实例
                for (int i = 0; i < 2; i++)
                {
                    var go = Instantiate(loader.asset as GameObject, _effectRoot);
                    go.SetActive(false);
                    _effectPool[effectId].Enqueue(go);
                }
            }
        }
    }
    ...
    /// 播放特效
    /// </summary>
    public GameObject PlayEffect(long _effectId, Vector3 _position, Transform _parent = null)
    {
        var config = GetEffectConfig(_effectId);
        if (config == null) return null;

        GameObject effect;

        // 从对象池获取
        if (_effectPool.TryGetValue(_effectId, out var pool) && pool.Count > 0)
        {
            effect = pool.Dequeue();
            effect.transform.SetParent(_parent ?? _effectRoot);
        ...
        {
            // 创建新实例
            effect = Instantiate(Resources.Load<GameObject>(config.prefabPath),
                _position, Quaternion.identity, _parent ?? _effectRoot);
        }

        // 记录活动特效
        _activeEffects[_effectId] = effect;

        // 自动回收
        if (!config.loop)
        {
            StartCoroutine(AutoRecycleEffect(_effectId, effect, config.duration));
        }
    ...
    /// 停止特效
    /// </summary>
    public void StopEffect(long _effectId)
    {
        if (_activeEffects.TryGetValue(_effectId, out var effect))
        {
            RecycleEffect(_effectId, effect);
            _activeEffects.Remove(_effectId);
        }
    }
    ...
    /// 自动回收特效
    /// </summary>
    private IEnumerator AutoRecycleEffect(long _effectId, GameObject _effect, float _duration)
    {
        yield return new WaitForSeconds(_duration);
        RecycleEffect(_effectId, _effect);
    }

    /// <summary>
    /// 回收特效到对象池
    /// </summary>
    private void RecycleEffect(long _effectId, GameObject _effect)
    {
        if (_effect == null) return;

        _effect.SetActive(false);
        _effect.transform.SetParent(_effectRoot);

        if (!_effectPool.ContainsKey(_effectId))
        {
            _effectPool[_effectId] = new Queue<GameObject>();
        }
        _effectPool[_effectId].Enqueue(_effect);
    }

    /// <summary>
    /// 获取特效配置
    /// </summary>
    private ChapterEffectConfig GetEffectConfig(long _effectId)
    {
        // 从配置表获取
        return GRefdataCoreMgr.instance.effectRefCore?.getRef(_effectId);
    }
    ...
    public void ClearAllEffects()
    {
        ...
        foreach (var pool in _effectPool.Values)
        {
            while (pool.Count > 0)
            {
                var go = pool.Dequeue();
...
| 音效ID | 音效名称 | 触发时机 | 资源路径 |
|--------|----------|----------|----------|
| 10001 | forward_click | 点击前进按钮 | Audio/UI/forward_click.wav |
| 10002 | forward_move | 前进移动中 | Audio/Chapter/move.wav |
| 10003 | critical_hit | 暴击触发 | Audio/Chapter/critical.wav |
| 10004 | boss_appear | Boss出场 | Audio/Chapter/boss_appear.wav |
| 10005 | boss_hit | Boss受击 | Audio/Chapter/boss_hit.wav |
| 10006 | boss_win | Boss胜利 | Audio/Chapter/boss_win.wav |
| 10007 | event_trigger | 事件触发 | Audio/Chapter/event.wav |
| 10008 | reward_gain | 获得奖励 | Audio/UI/reward.wav |
...
{
    private static ChapterAudioManager _instance;
    public static ChapterAudioManager instance => _instance;
    ...
    // 音频缓存
    private Dictionary<long, AudioClip> _audioCache = new();
    ...
    /// 播放背景音乐
    /// </summary>
    public void PlayBGM(long _bgmId, bool _loop = true)
    {
        var clip = GetAudioClip(_bgmId);
        if (clip == null) return;
    ...
    /// 播放语音
    /// </summary>
    public void PlayVoice(long _voiceId)
    {
        var clip = GetAudioClip(_voiceId);
        if (clip == null) return;
    ...
    /// 设置音量
    /// </summary>
    public void SetVolume(float _bgm, float _sfx, float _voice)
    {
        bgmVolume = _bgm;
        sfxVolume = _sfx;
        voiceVolume = _voice;
    ...
    /// 获取音频剪辑
    /// </summary>
    private AudioClip GetAudioClip(long _audioId)
    {
        if (_audioCache.TryGetValue(_audioId, out var clip))
        {
            return clip;
        }

        // 从配置表获取路径并加载
        var config = GRefdataCoreMgr.instance.audioRefCore?.getRef(_audioId);
        if (config == null) return null;

        clip = Resources.Load<AudioClip>(config.path);
        if (clip != null)
        {
            _audioCache[_audioId] = clip;
        }
...
/// 章节引导管理器
/// </summary>
public class ChapterTutorialManager : MonoBehaviour
{
    // 引导步骤
    ...
    [SerializeField] private RectTransform _highlightArea;
    ...
    {
        var chapterComp = NPPlayer.instance.chapterComp;
        var chapterId = chapterComp.curChapterId;

        // 检查是否需要引导
        if (ShouldShowTutorial(chapterId))
        {
            StartTutorial();
        }
    }
    ...
    /// 是否应该显示引导
    /// </summary>
    private bool ShouldShowTutorial(long _chapterId)
    {
        // 首次进入该章节
        if (_chapterId == 1 && !PlayerPrefs.HasKey("tutorial_chapter_1"))
        {
            return true;
        }
        ...
        // 首次遇到Boss
        if (NPPlayer.instance.chapterComp.curIsBossPoint() &&
            !PlayerPrefs.HasKey("tutorial_boss"))
        {
            return true;
        }
    ...
    public void StartTutorial()
    {
        ...
        if (NPPlayer.instance.chapterComp.curIsBossPoint())
        {
            _currentStep = ETutorialStep.BossIntro;
        }
        else
        {
            _currentStep = ETutorialStep.ChapterIntro;
        }
    ...
    private void ProcessStep()
    {
        switch (_currentStep)
        {
            case ETutorialStep.ChapterIntro:
                ShowChapterIntro();
                break;

            case ETutorialStep.ForwardButton:
                ShowForwardButtonGuide();
                break;

            case ETutorialStep.ForwardAction:
                ShowForwardActionGuide();
                break;

            case ETutorialStep.BossIntro:
                ShowBossIntro();
                break;

            case ETutorialStep.PowerCheck:
                ShowPowerCheckGuide();
                break;

            case ETutorialStep.InspireGuide:
                ShowInspireGuide();
                break;

            case ETutorialStep.BossFight:
                ShowBossFightGuide();
                break;
    ...
    {
        var chapterRef = NPPlayer.instance.chapterComp.chapterRefObj;
        _tutorialText.text = $"欢迎来到【{chapterRef.name}】章节！\n点击前进按钮推进关卡。";
        _highlightArea.gameObject.SetActive(false);
        _nextButton.onClick.AddListener(OnIntroComplete);
    }

    /// <summary>
    /// 显示前进按钮引导
    /// </summary>
    private void ShowForwardButtonGuide()
    {
        _tutorialText.text = "点击这里前进到下一关卡";
        HighlightButton("ForwardButton");
    }
    ...
    {
        var chapterRef = NPPlayer.instance.chapterComp.chapterRefObj;
        var bossStyle = chapterRef.getBossStyleRefObj();

        _tutorialText.text = $"你遇到了Boss【{bossStyle.boss_name}】！\n战力需求: {chapterRef.boss_power}";
        _highlightArea.gameObject.SetActive(false);
        _nextButton.onClick.AddListener(OnBossIntroComplete);
    }

    /// <summary>
    /// 高亮按钮
    /// </summary>
    private void HighlightButton(string _buttonName)
    {
        var button = GameObject.Find(_buttonName);
        if (button != null)
        {
            var rectTransform = button.GetComponent<RectTransform>();
            _highlightArea.position = rectTransform.position;
            _highlightArea.sizeDelta = rectTransform.sizeDelta + new Vector2(20, 20);
    ...
    private void CompleteTutorial()
    {
        ...
        // 标记引导完成
        var chapterId = NPPlayer.instance.chapterComp.curChapterId;
        PlayerPrefs.SetInt($"tutorial_chapter_{chapterId}", 1);
        PlayerPrefs.Save();
    }
    ...
    {
        _nextButton.onClick.RemoveListener(OnIntroComplete);
        _currentStep = ETutorialStep.ForwardButton;
        ProcessStep();
    }

    private void OnBossIntroComplete()
    {
        _nextButton.onClick.RemoveListener(OnBossIntroComplete);
        _currentStep = ETutorialStep.PowerCheck;
        ProcessStep();
    }
}
```
