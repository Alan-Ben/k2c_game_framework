...
@RefBo(isIdAuto= true)
public class PlayerTowerChapterResearchBO extends BaseBO {
    ...
    public PlayerTowerChapterResearchBO() {
        id = 0;
        cid = 0L;
        chapter_id = 0L;
    }

    public PlayerTowerChapterResearchBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new PlayerTowerChapterResearchBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `chapter_id`";
    }

    @Override
    public String getTableName() {
        return "`player_tower_chapter_research`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_tower_chapter_research` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家CID',"
                + "`chapter_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '章节ID',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家爬塔章节研究数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
