...
namespace ALPackage
{
    public abstract class _AUITextureCacheMgr
    {
        //需要加载的图片信息对象队列
        private List<UITextureLoadInfo> _m_lNeedLoadTextureInfoList;
        //当前已经加载的所有图片对象的队列
        private Dictionary<long, UITextureLoadInfo> _m_dicAllTextureInfoDic;

        //当前正在加载的对象
        private UITextureLoadInfo _m_liLoadingInfo;

        protected _AUITextureCacheMgr()
        {
            _m_lNeedLoadTextureInfoList = new List<UITextureLoadInfo>();
            _m_dicAllTextureInfoDic = new Dictionary<long, UITextureLoadInfo>();
        ...
        public void addTexture(int _mainId, int _subId, Action<int, int, Texture> _delegate)
        {
            ...
            loadInfo.addUsedCountFromStart();
            loadInfo.updateLastUsedTime();
            //增加关联
        ...
        public void releaseUnusedTexture()
        {
            ...
            //清空已加载数据中信息
            foreach(UITextureLoadInfo loadInfo in _m_dicAllTextureInfoDic.Values)
            {
        ...
        public void releaseTexture(int _mainId, int _subId, bool _force = false)
        {
            UITextureLoadInfo loadInfo = null;
            long mergeIdx = ALCommon.mergeInt(_mainId, _subId);
            ...
            else
            {
                for(int i = 0; i < _m_lNeedLoadTextureInfoList.Count; i++)
                {
                    ...
                    //判断数据是否有效
                    if (_m_lNeedLoadTextureInfoList[i].mainId == _mainId&&_m_lNeedLoadTextureInfoList[i].subId == _subId)
                    {
                        if (_force)
                        {
                            _m_lNeedLoadTextureInfoList.RemoveAt(i);
                ...
                _m_lNeedLoadTextureInfoList.Remove(loadInfo);
            }
            //移除数据
            for(int i = 0; i < removeIdxList.Count; i++)
            {
        ...
        {
            //取出队列第一个
            UITextureLoadInfo loadInfo = _popFirstLoadInfo();
        ...
        protected void _onAssetLoaded(bool _isSuc, ALAssetBundleObj _assetObj)
        {
            ...
            //成功则加载对应图片后处理
            Texture texture = _assetObj.load(_getAssetObjName(_m_liLoadingInfo.mainId, _m_liLoadingInfo.subId)) as Texture;
        ...
        protected void _onAssetLoaded(Texture _texture)
        {
            ...
            // TextureMemoryAllocate texAllocate = new TextureMemoryAllocate(this, _m_liLoadingInfo, _texture.width, _texture.height);
            // MGMemoryMgr.instance.regCacheController(EMemoryResType.TEXTURE,_texture.name, texAllocate);
            _m_liLoadingInfo = null;
        ...
        /** 取出第一个加载信息对象 */
        protected UITextureLoadInfo _popFirstLoadInfo()
        {
            if (_m_lNeedLoadTextureInfoList.Count <= 0)
                return null;

            //取出队列第一个
            UITextureLoadInfo loadInfo = _m_lNeedLoadTextureInfoList[0];
