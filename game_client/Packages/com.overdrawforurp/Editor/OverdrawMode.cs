	...
	public class OverdrawMode
	{
		...
		private static void OnCameraModeChanged(SceneView sceneView)
		{
			...
			if (cameraMode.name.Equals(overdrawMode.name))
			{
				if (!Utility.IsExistOverdrawRenderer())
				{
					UniversalRendererData rendererData = Utility.GetFirst<UniversalRendererData>("OverdrawRenderer");

					SerializedProperty propArray = so.FindProperty("m_RendererDataList");
					int count = propArray.arraySize;
					propArray.InsertArrayElementAtIndex(propArray.arraySize - 1);
					SerializedProperty propElem = propArray.GetArrayElementAtIndex(count);
					propElem.objectReferenceValue = rendererData;
			...
			else
			{
				...
				if (overdrawRendererIndex != -1)
				{
					SerializedProperty propArray = so.FindProperty("m_RendererDataList");
					propArray.DeleteArrayElementAtIndex(overdrawRendererIndex);
					so.ApplyModifiedProperties();
				}
			}
		}
