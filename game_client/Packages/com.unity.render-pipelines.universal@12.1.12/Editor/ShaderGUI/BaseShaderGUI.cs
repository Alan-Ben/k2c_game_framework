...
namespace UnityEditor
{
    public abstract class BaseShaderGUI : ShaderGUI
    {
        ...
        private static readonly int AlphaPremultiplyOn = Shader.PropertyToID("_AlphaPremultiply_On");
        private static readonly int AlphaModulateOn = Shader.PropertyToID("_AlphaModulate_On");
        ...
        // this function is shared with ShaderGraph Lit/Unlit GUIs and also the hand-written GUIs
        //变体优化版
        internal static void UpdateMaterialSurfaceOptions_VariantOptimize(Material material, bool automaticRenderQueue)
        {
            // Setup blending - consistent across all Universal RP shaders
            // SetupMaterialBlendModeInternal(material, out int renderQueue);
            SetupMaterialBlendModeInternal_VariantOptimize(material, out int renderQueue);
        ...
        public static void SetMaterialKeywords(Material material, Action<Material> shadingModelFunc = null, Action<Material> shaderFunc = null)
        {
            ...
            if (material.HasProperty("_EmissionEnabled"))
                CoreUtils.SetKeyword(material, ShaderKeywordStrings._EMISSION, material.GetFloat("_EmissionEnabled") >= 0.5f);
            /*
            // Emission
        ...
        // this is the function used by Lit.shader, Unlit.shader GUIs
        //变体优化版本
        public static void SetMaterialKeywords_VariantOptimize(Material material, Action<Material> shadingModelFunc = null, Action<Material> shaderFunc = null)
        {
            // UpdateMaterialSurfaceOptions(material, automaticRenderQueue: true);
            UpdateMaterialSurfaceOptions_VariantOptimize(material, automaticRenderQueue: true);
        ...
        //变体优化版材质混合
        public static void SetupMaterialBlendModeInternal_VariantOptimize(Material material, out int automaticRenderQueue)
        {
            if (material == null)
                throw new ArgumentNullException("material");
            ...
            if (material.HasProperty(Property.SurfaceType))
            {
                ...
                if (surfaceType == SurfaceType.Opaque)
                {
                    ...
                    material.DisableKeyword(ShaderKeywordStrings._ALPHAMODULATE_ON);
                    // material.DisableKeyword(ShaderKeywordStrings._SURFACE_TYPE_TRANSPARENT);
                    SetMaterialProperty_AlphaPremultiply_On(material, false);
                }
                else // SurfaceType Transparent
                {
                    BlendMode blendMode = (BlendMode)material.GetFloat(Property.BlendMode);
                    ...
                    material.DisableKeyword(ShaderKeywordStrings._ALPHAMODULATE_ON);
                    SetMaterialProperty_AlphaPremultiply_On(material, false);
                    SetMaterialProperty_AlphaModulate_On(material, false);
                    ...
                    switch (blendMode)
                    {
                        ...
                        case BlendMode.Premultiply:
                            SetMaterialSrcDstBlendProperties(material,
                                ...
                                UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
                            // material.EnableKeyword(ShaderKeywordStrings._ALPHAPREMULTIPLY_ON);
                            SetMaterialProperty_AlphaPremultiply_On(material, true);
                            break;
                        case BlendMode.Additive:
                            SetMaterialSrcDstBlendProperties(material,
                                UnityEngine.Rendering.BlendMode.SrcAlpha,
                        ...
                        case BlendMode.Multiply:
                            SetMaterialSrcDstBlendProperties(material,
                                ...
                                UnityEngine.Rendering.BlendMode.Zero);
                            // material.EnableKeyword(ShaderKeywordStrings._ALPHAMODULATE_ON);
                            SetMaterialProperty_AlphaModulate_On(material, true);
                            break;
                    }
        ...
        public static void SetMaterialProperty_AlphaPremultiply_On(Material material, bool zwriteEnabled)
        {
            if (material.HasProperty(AlphaPremultiplyOn))
                material.SetFloat(AlphaPremultiplyOn, zwriteEnabled ? 1.0f : 0.0f);
        }
        
        public static void SetMaterialProperty_AlphaModulate_On(Material material, bool zwriteEnabled)
        {
            if (material.HasProperty(AlphaModulateOn))
                material.SetFloat(AlphaModulateOn, zwriteEnabled ? 1.0f : 0.0f);
        }
