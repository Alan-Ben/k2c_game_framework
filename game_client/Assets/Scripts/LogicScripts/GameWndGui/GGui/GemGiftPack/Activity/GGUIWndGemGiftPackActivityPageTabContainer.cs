...
namespace GOE
{
    ...
    /// 钻石礼包页签item容器
    /// </summary>
    public class GGUIWndGemGiftPackActivityPageTabContainer : _ATNPGGUIWndShowAnimContainer<GGUIMonoGemGiftPackActivityPageTabContainerItem, GGUIMonoGemGiftPackActivityPageTabContainer, GGUIWndGemGiftPackActivityPageTabContainerItem>
    {
        //item列表
        protected List<GGUIWndGemGiftPackActivityPageTabContainerItem> _m_lItemList;
        //当前选中的item
        private GGUIWndGemGiftPackActivityPageTabContainerItem _m_wCurSelectItem;
        //点击item事件
        ...
        /// 点击item事件
        /// </summary>
        public Action<GGUIWndGemGiftPackActivityPageTabContainerItem, bool> onClickItem { get { return _m_aOnClickItem; } set { _m_aOnClickItem = value; } }

        public GGUIWndGemGiftPackActivityPageTabContainer(GGUIMonoGemGiftPackActivityPageTabContainer _mono) : base(_mono)
        {
            initWnd();
        }

        protected override GGUIWndGemGiftPackActivityPageTabContainerItem _createItemWnd(GGUIMonoGemGiftPackActivityPageTabContainerItem _itemMono)
        {
            GGUIWndGemGiftPackActivityPageTabContainerItem item = new GGUIWndGemGiftPackActivityPageTabContainerItem(_itemMono);
        ...
        protected override void _onWndInitDone()
        {
            ...
            _m_lItemList = new List<GGUIWndGemGiftPackActivityPageTabContainerItem>();
        ...
        /// 设置信息
        /// </summary>
        public void showItemList(List<_ABaseActivityInfo> _infoList)
        {
            if (_infoList == null || _m_lItemList == null)
                return;

            _ABaseActivityInfo lastSelectActivityInfo = _m_wCurSelectItem?.activityInfo;
            _m_wCurSelectItem?.setSelect(false);
            _m_wCurSelectItem = null;
            GGUIWndGemGiftPackActivityPageTabContainerItem itemWnd = null;
            int count = 0;
            //遍历玩家数据
            for (int i = 0; i < _infoList.Count; i++)
            {
                ...
                itemWnd.showWnd();
                itemWnd.setInfo(_infoList[i], count == 0);
                count++;

                //判断当前选中的item是否在列表中，有则选中
                if (lastSelectActivityInfo != null && lastSelectActivityInfo.instanceId == _infoList[i].instanceId)
            ...
            //列表为空显隐
            ALUGUICommon.setGameObjEnable(wnd.goEmptyShowList, _infoList.Count <= 0);
        ...
        //点击选中item
        private void _onClickItem(GGUIWndGemGiftPackActivityPageTabContainerItem _item, bool _moveToTop)
        {
            if (wnd == null || _item == null)
                return;

            if (_m_wCurSelectItem == null || (_m_wCurSelectItem.activityInfo?.instanceId != _item.activityInfo?.instanceId))
            {
                _m_wCurSelectItem?.setSelect(false);
