...
namespace GOE
{
    ...
    /// 公会协作奖励据点item对象池
    /// </summary>
    public class GGuildCooperatePosPrefabCacheMgr
    {
        private static GGuildCooperatePosPrefabCacheMgr _g_instance;
        [NotNull]
        public static GGuildCooperatePosPrefabCacheMgr instance
        {
            get
            {
                if (_g_instance == null)
                    _g_instance = new GGuildCooperatePosPrefabCacheMgr();
                return _g_instance;
            }
        }
        ...
        //不同资源缓存池
        [NotNull] private Dictionary<long, GGuildCooperatePosPrefabCache> _m_dCacheDic;
        //根节点
        [NotNull] private readonly GameObject _m_cacheRoot;

        public GGuildCooperatePosPrefabCacheMgr()
        {
            _m_cacheRoot = new GameObject("guild_cooperate_pos_item_cache_root");
            _m_cacheRoot.SetActive(false);
            _m_cacheRoot.transform.localPosition = Vector3.up * -10000;
            Object.DontDestroyOnLoad(_m_cacheRoot);
            _m_dCacheDic = new Dictionary<long, GGuildCooperatePosPrefabCache>();
        }
        ...
        public void popItem(long _uiResId,Transform _itemParent,Action<GGUIWndGuildCooperatePosPrefab>  _onPopAction)
        {
            ...
            // 临时的cache
            GGuildCooperatePosPrefabCache cache = null;
            // Cache加载完后的处理
            void onCacheLoaded()
            {
                if (cache == null)
                ...
                GGUIWndGuildCooperatePosPrefab wnd = cache.popItem();
            ...
            {
                // 如果没有获取cache，就构建一个新的
                cache = new GGuildCooperatePosPrefabCache(_uiResId, _m_cacheRoot.transform, _m_iMinCacheCount, _m_iMaxCacheCount);
                // 添加到字典中
        ...
        public void pushBackCacheItem(long _uiResId, GGUIWndGuildCooperatePosPrefab _item)
        {
            ...
            // 尝试找到对应的缓存池
            if (_m_dCacheDic.TryGetValue(_uiResId, out GGuildCooperatePosPrefabCache cache) && cache != null)
            {
                // 放回对应的缓存池
            ...
            {
                // 如果没有找到缓存池，弹出错误提示
                ALLog.Error($"GGuildCooperatePosPrefabCacheMgr 出现了未知的异常，Cache回收资源时，未找到对应缓存池");
        ...
        public void discard()
        {
            foreach (GGuildCooperatePosPrefabCache cache in _m_dCacheDic.Values)
            {
                cache?.forceDiscard();
            }
            _m_dCacheDic.Clear();
