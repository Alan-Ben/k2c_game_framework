...
import Hotfix.V02.GC2GS.p202_NumMergeOp.GC2GS_202_003_ReqNumMergeMove;
 ...
 * 3. 返回更新后的棋盘数据
 */
public class MsgDealer_GC2GS_202_003_ReqNumMergeMove extends NPUserMsgDealer<GC2GS_202_003_ReqNumMergeMove>
{
    @Override
    protected void _dealMessage(_ANPUSUserBasicMsgItem _committer, GC2GS_202_003_ReqNumMergeMove _msg)
    {
        // 获取玩家数据
        ...
        // 创建操作上下文
        NPPlayerContext context = NPPlayerContext.createNew(ENumMerge_GameEvent.NUM_MERGE_MOVE.value());

        // 执行移动操作
        Result result = playerInfo.move(userData, _msg.getModeType(), _msg.getMoveDir(), context);
        if (!result.isSucc())
        {
        ...
        // 返回成功响应
        _committer.commitSucRes(US2GCWriter_202_NumMergeOp.make_003_RetNumMergeMove());
    }
}
