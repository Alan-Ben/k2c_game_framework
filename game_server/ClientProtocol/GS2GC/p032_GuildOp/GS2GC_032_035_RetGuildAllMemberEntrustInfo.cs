...
public class GS2GC_032_035_RetGuildAllMemberEntrustInfo : ALBasicProtocolPack._IALProtocolStructure {
private List<Common.GuildObj.Guild_MemberEntrustInfo> infoList;


public GS2GC_032_035_RetGuildAllMemberEntrustInfo() {
	infoList = new List<Common.GuildObj.Guild_MemberEntrustInfo>();
}

public GS2GC_032_035_RetGuildAllMemberEntrustInfo(
	List<Common.GuildObj.Guild_MemberEntrustInfo> _infoList
) {	infoList = _infoList;
}
...
public List<Common.GuildObj.Guild_MemberEntrustInfo> getInfoList() { return infoList; }
public void addInfoList(Common.GuildObj.Guild_MemberEntrustInfo _infoList) { infoList.Add(_infoList); }


public int GetBufSize() {
	int _size = 0;
	_size += 2 + (infoList.Count * 20);
