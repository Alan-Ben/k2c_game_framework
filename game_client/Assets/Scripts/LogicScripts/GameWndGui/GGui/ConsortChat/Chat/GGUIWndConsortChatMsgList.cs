...
namespace GOE
{
    ...
    /// item容器
    /// </summary>
    public class GGUIWndConsortChatMsgList : ALUGUIWndVerticalMultiSizeLayout<GGUIMonoConsortChatMsgList>
    {
        // 现在在wnd列表里的所有消息item，包括了不在视口范围里的所有item
        [NotNull] private readonly List<GGUIWndConsortChatMsgListItem> _m_showMsgItemList;
        ...
        private bool _m_bShowTyping;
        private ConsortChatMsgTyping _m_typingMsg;
        private GGUIWndConsortChatMsgListItem _m_wTypingMsgItem;
        // 当前wnd展示的chatInfo
        ...
        public GGUIWndConsortChatMsgList(GGUIMonoConsortChatMsgList _wnd) : base(_wnd)
        {
            // list初始化
            _m_showMsgItemList = new List<GGUIWndConsortChatMsgListItem>();
        ...
        /// </summary>
        public void setShowData(_AConsortChatInfo _chatInfo, long _consortId)
        {
            ...
            if (_m_bIsShow)
            {
                ...
                if (oldChatInfo != null)
                {
                    ...
                    oldChatInfo.changeTypingState -= _onTypingStateChange;
                }
                // 重置内部和数据有关的东西
                _resetInfo();
                // 刷新
                ...
                if (_m_chatInfo != null)
                {
                    ...
                    _m_chatInfo.changeTypingState += _onTypingStateChange;
                }
            }
        }
        ...
        protected override void _onShowWnd()
        {
            ...
            if (wnd != null)
            {
                if (wnd.scrollRect != null) wnd.scrollRect.verticalNormalizedPosition = 0;
        ...
        {
            _m_typingMsg = null;
            _m_wTypingMsgItem?.discard();
            _m_wTypingMsgItem = null;
        }
        ...
        protected sealed override void _onFrameRefresh()
        {
            ...
            if (_m_bIsNeedRefreshList)
            {
                ...
                bool needRemoveTyping = false;
                if (_m_bShowTyping)
                {
                    if (_m_wTypingMsgItem == null)
                    {
                        if (_m_typingMsg == null)
                            _m_typingMsg = new ConsortChatMsgTyping(FpsAndPingMgr.instance.serverTimeTag);
                        _m_wTypingMsgItem = new GGUIWndConsortChatMsgListItem(_m_typingMsg, _m_consortId, itemContainer);
                        _bindItemWnd(_m_wTypingMsgItem);
                    }

                    if (_m_wTypingMsgItem != null)
                    {
                        _m_wTypingMsgItem.setConsortId(_m_consortId);
                        _m_showMsgItemListForRefresh.Add(_m_wTypingMsgItem);
                        needRemoveTyping = true;
                    }
                }
                else // 如果关闭打字状态，则要检查打字item是否释放
                {
                    // 如果解除之前在视口内，调用一次出视口的操作
                    if (_m_wTypingMsgItem != null && _m_wTypingMsgItem.isInViewport)
                        _m_wTypingMsgItem.outViewport();
                }

                bool fixView = _m_lItemList.Count != _m_showMsgItemListForRefresh.Count;
                ...
                {
                    RectTransform content = wnd.scrollRect.content;
                    if (content != null)
                    {
                        float distance = contentHeight - beforeHeight;
                        content.anchoredPosition = new Vector2(content.anchoredPosition.x, content.anchoredPosition.y + distance);
                        if (distance < -1)
                        {
                ...
                if (needRemoveTyping)
                {
                    _m_showMsgItemListForRefresh.Remove(_m_wTypingMsgItem);
                }
                
                // 尽量还原当前显示的内容
                _fixCurViewport(curTopItem, fixView);
                // 刷新过后标记为不用再刷新了
        ...
        private void _onTypingStateChange(bool _showTyping)
        {
            _m_bShowTyping = _showTyping;
        ...
        protected void _addFirstMsgItem(List<_AConsortChatMsgInfo> _msgInfoList)
        {
            ...
            {
                GGUIWndConsortChatMsgListItem wnd = new GGUIWndConsortChatMsgListItem(_msgInfoList[i], _m_consortId, itemContainer);
                if (wnd == null)
                    continue;
        ...
        {
            // 添加消息到待处理列表
            GGUIWndConsortChatMsgListItem wnd = new GGUIWndConsortChatMsgListItem(_msgInfo, _m_consortId, itemContainer);
            if (wnd == null)
                return;
        ...
        // 对item进行相关绑定和加载
        private void _bindItemWnd(GGUIWndConsortChatMsgListItem _itemWnd)
        {
            if (_itemWnd == null)
                return;
        ...
        private void _releaseItemWnd(GGUIWndConsortChatMsgListItem _itemWnd)
        {
            if (_itemWnd == null)
                return;
        ...
        private void _fixCurViewport(_IALBasicMultiSizeLayoutItem _lastTopItem, bool _fixView = true)
        {
            if (wnd == null || wnd.scrollRect == null)
                return;
