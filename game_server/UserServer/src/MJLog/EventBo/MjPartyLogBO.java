...
public class MjPartyLogBO extends MJEventLogBo {
    ...
    public static final int FIELD_party_type =7;
    @DataBaseField(type = "bigint(20)", fieldname = "party_type", comment = "舞会类型")
    private long party_type;

    public static final int FIELD_action =8;
    @DataBaseField(type = "int(11)", fieldname = "action", comment = "参与类别：1=参与；2=举办")
    private int action;
    ...
    public MjPartyLogBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        uid = rs.getString(3);
        vip_lv = rs.getInt(4);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new MjPartyLogBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `uid`, `vip_lv`, `server_id`, `platform`, `region`, `create_time`, `party_type`, `action`, `timestamp`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(party_type).append("', ");
   ...
   @Override
    ...
    // 舞会类型
    public long getPartyType() { return this.party_type; }
    public void setPartyType(BM _bm, long party_type) {
        if(party_type==this.party_type) 
            return;
        this.party_type = party_type; 
        markField(_bm, FIELD_party_type); 
    }
    public void savePartyType(BM _bm, long party_type) {
        if(party_type==this.party_type) 
            return;
        this.party_type = party_type;
        saveField(_bm, "party_type", party_type);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `party_type` = '").append(party_type).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_party_type)) sBuilder.append(" `party_type` = '").append(party_type).append("',");
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `mj_party_log` ("
                ...
                + "`party_type` bigint(20) NOT NULL DEFAULT '0' COMMENT '舞会类型',"
                + "`action` int(11) NOT NULL DEFAULT '0' COMMENT '参与类别：1=参与；2=举办',"
                + "`timestamp` int(11) NOT NULL DEFAULT '0' COMMENT '事件发生时间戳(10位)',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='梦加日志-宴会参与记录' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
