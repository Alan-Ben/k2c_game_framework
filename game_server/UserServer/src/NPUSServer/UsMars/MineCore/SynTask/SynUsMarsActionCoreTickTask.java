 ...
 * 矿场部分的tick执行任务
 */
public class SynUsMarsActionCoreTickTask implements _IALSynTask
{
	private UsMarsActionCore _m_mpActionCore;
	//本Tick的序列号，避免意外情况重开后，多任务执行的问题
	private long _m_lTickSerialize;

	public SynUsMarsActionCoreTickTask(UsMarsActionCore _core, long _tickSerialize)
	{
		_m_mpActionCore = _core;
	...
	public void run() 
	{
		//执行Tick处理
		try {
			_m_mpActionCore.tick(_m_lTickSerialize, CommonFunc.getNowTimeMS());
		}
		catch (Exception _ex)
		{
