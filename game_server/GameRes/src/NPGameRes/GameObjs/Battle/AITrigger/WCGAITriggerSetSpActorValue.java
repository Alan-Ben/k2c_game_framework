...
/// </summary>
public class WCGAITriggerSetSpActorValue extends _AWCGBasicAITrigger
{
    ...
    {
        return EWCGAITriggerType.SET_SP_ACTOR_V;
    }

    public static WCGAITriggerSetSpActorValue read(String _infoStr)
    {
        WCGAITriggerSetSpActorValue obj = new WCGAITriggerSetSpActorValue();
        String[] strs = CommonFunc.charSplit(_infoStr, ':');
        if (strs.length < 3)
        {
            CommLog.error("ai SET_SP_ACTOR_V配置错误 正确配置: 枚举:下标索引:值");
            return null;
        }
        try
        {
        ...
        {
            CommLog.error("ai SET_SP_ACTOR_V配置错误 正确配置: 枚举:下标索引:值", e);
            return null;
        }
    }
}