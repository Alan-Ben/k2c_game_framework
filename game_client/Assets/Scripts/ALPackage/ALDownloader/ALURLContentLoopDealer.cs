...
namespace ALPackage
{
    ...
    /// 轮询基本只针对文本进行
    /// </summary>
    public class ALURLContentLoopDealer
    {
        /// <summary>
        /// 操作序列号，用于屏蔽不同操作
        /// </summary>
        ...
        public ALURLContentLoopDealer(string _fileSubPath, List<ALURLDownloaderURLInfo> _urlList, float _maxWaitTimeS = 20f)
        {
        ...
        public ALURLContentLoopDealer(string _fileSubPath, List<ALURLDownloaderURLInfo> _urlList, bool _isFailDone, float _maxWaitTimeS = 20f)
        {
            _m_iOpSerialzie = ALSerializeOpMgr.next();
            _m_fOpStartTime = 0f;
            _m_fOpDealTime = 0f;
        ...
        protected bool _setFinalURL(int _serialize, string _url, string _fileInfo)
        {
            //序列号不一致不处理
            ...
            //判断结果是否有效，如果无效则不处理
            if (null != _m_fCheckResEnable && !_m_fCheckResEnable(_fileInfo))
            {
                return false;
            }
            ...
            if (null != _m_dSucDelegate)
            {
                ...
                _m_dSucDelegate = null;
                tempSucDelegate(_m_sRecordURL, _fileInfo);
            }
                ...
                if (_m_lCheckURLList[i].delayTimeS <= deltaTime)
                {
                    ...
                    if (_m_lCheckURLList[i].delayTimeS > _m_fOpDealTime)
                    {
                        ...
                        _checkURL(
                                ...
                                {
                                    //设置最终值，返回设置是否成功
                                    if (_setFinalURL(_serialize, tmpCheckURL, _fileInfo))
                                    {
                                        //重置step
                                ...
                                , (string _error, int _errStat) =>
                                {
                                    //如果是检测失败，判断当前经过时间，超过1秒才允许快速失败，只有404找不到文件才算失败
        ...
        /// <returns></returns>
        protected void _checkURL(string _url, Action<string> _synOnGetURLDone, Action<string, int> _synFailAction)
        {
            if (!string.IsNullOrEmpty(_m_sCheckFile) && _m_sCheckFile[0] == '/')
                _m_sCheckFile = _m_sCheckFile.Substring(1);

            ALHttpCommon.GetContentAsync(ALCommon.urlEndInsure(_url) + _m_sCheckFile, _synOnGetURLDone, _synFailAction);
        }
    }
}
