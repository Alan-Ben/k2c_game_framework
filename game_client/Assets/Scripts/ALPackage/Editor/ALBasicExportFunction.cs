...
namespace ALPackage
{
     ...
     * 基本的导出处理函数
     **/
    public class ALBasicExportFunction
    {
        public static void exportXmlFile(XmlDocument _xmlDoc, string _xmlPath)
        {
            ...
            //获取xml导出的根目录
            string xmlRootPath = ALExportDataCore.instance.getXmlRootPath();
            //路径无效则不继续处理
        ...
        public static string exportAsset(UnityEngine.Object _assetObj, string _assetPath)
        {
            ...
            //同时导出一份文本数据到 __DLExport_txt
            //string txtFolderPath = Application.dataPath + "/Resources/Refdata/__DLExport_txt/" + _assetPath;
            //if (!Directory.Exists(txtFolderPath))
            //    Directory.CreateDirectory(txtFolderPath);

            //WriteToTxtFile(txtFolderPath + "/" + _assetObj.name + ".txt", ALXlsCommon.makeObjStr(_assetObj));
        ...
        public static void exportAsset(UnityEngine.Object _assetObj, string _assetPath, string _assetBundlePath)
        {
            ...
            //设置对应的assetbundle路径
            ALAssetBundleCommon.setAssetBundleName(assetPath, _assetBundlePath);
        }
        ...
        public static Vector3 readVector3(string _str)
        {
            ...
            Vector3 vec = new Vector3(ALCommon.ParseFloat(strs[0]), ALCommon.ParseFloat(strs[1]), ALCommon.ParseFloat(strs[2]));
         ...
         * 读取对应的索引队列信息
         **/
        public static List<T> readIndexList<T>(string _str, Func<T> _createDelegate) where T : ALBasicResIndexInfo
        {
