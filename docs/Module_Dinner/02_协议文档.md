# 宴会系统协议文档

**模块编号**: p019 (DinnerOp)
**文档版本**: 2.1
**更新日期**: 2026-04-12

---

## 一、协议总览

### 1.1 协议模块架构图

```mermaid
graph TB
    subgraph 客户端
        A[玩家操作] --> B[DinnerComponent]
        B --> C[协议发送器]
    end

    subgraph 网络层
        C --> D{协议路由}
        D -->|主序号19| E[DinnerOp协议处理]
    end

    subgraph 服务端
        E --> F[DinnerService]
        F --> G[业务逻辑处理]
        G --> H[数据库操作]
        G --> I[推送响应]
    end

    subgraph 推送
        I --> J[GS2GC响应]
        J --> K[客户端监听器]
        K --> B
    end

    style A fill:#e3f2fd
    style F fill:#e8f5e9
    style J fill:#fff3e0
```

### 1.2 协议序号分配

| 子序号范围 | 用途 | 示例协议 |
|------------|------|----------|
| 001-010 | 宴会操作 | 001举办宴会, 002使用许可举办, 008加入宴会 |
| 011-020 | 查询操作 | 003参与者列表, 004宴会记录, 006宴会列表 |
| 021-030 | 奖励操作 | 009领取奖励 |
| 031-040 | 邀请操作 | 013发送邀请, 014检查邀请 |
| 050-059 | 推送通知 | 050宴会添加, 051宴会结束, 052许可添加, 053参与者添加 |

### 1.3 协议分类一览表

```mermaid
mindmap
  root((p019 DinnerOp))
    宴会操作
      001 举办宴会
      002 使用许可举办
      008 加入宴会
    查询操作
      003 参与者列表
      004 宴会记录索引
      005 宴会记录详情
      006 宴会列表
      007 宴会信息
      011 预览信息
      012 下一个宴会
    奖励操作
      009 领取奖励
    邀请操作
      013 发送邀请
      014 检查邀请
    推送通知
      050 宴会添加
      051 宴会结束
      052 许可添加
      053 参与者添加
```

---

## 二、请求协议 (GC2GS)

### 2.1 GC2GS_019_001_ReqStartDinner - 请求举办宴会

**主序号**: 19 | **子序号**: 1

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerCfgId | int | 宴会配置ID | 是 | 必须为有效配置ID | > 0，配置必须存在 |

#### 协议源码

```csharp
/// <summary>
/// 请求举办宴会
/// </summary>
public class GC2GS_019_001_ReqStartDinner : _IALProtocolStructure
{
    private int dinnerCfgId;  // 宴会配置ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)1; }

    // Getter/Setter
    public int getDinnerCfgId() { return dinnerCfgId; }
    public void setDinnerCfgId(int value) { dinnerCfgId = value; }
}
```

#### 请求示例

```csharp
// 通过宴会组件发送举办请求
NPPlayer.instance.dinnerComp.reqStartDinner(dinnerCfgId, () => {
    Debug.Log("宴会举办成功");
});

// 或者直接构建协议发送
var req = new GC2GS_019_001_ReqStartDinner();
req.setDinnerCfgId(2);  // 豪华宴会
NPGSClientListener.sendRequest(req, new CommonCallback<GS2GC_019_001_RetStartDinner>((msg) => {
    if (msg.getResult() == 0) {
        Debug.Log("宴会举办成功");
    }
}));
```

#### 服务端处理流程

```mermaid
sequenceDiagram
    participant C as 客户端
    participant H as Handler
    participant S as Service
    participant DB as 数据库

    C->>H: GC2GS_019_001_ReqStartDinner
    H->>S: startDinner(cid, dinnerCfgId)

    S->>S: 检查配置是否存在
    S->>S: 检查是否已举办宴会
    S->>S: 检查解锁条件
    S->>S: 计算消耗

    alt 检查失败
        S-->>C: 返回错误码
    else 检查通过
        S->>DB: 扣除消耗
        S->>DB: 创建宴会记录
        S->>DB: 举办者加入
        S-->>C: 返回成功
    end
```

---

### 2.2 GC2GS_019_002_ReqStartDinnerByPermit - 使用许可举办宴会

**主序号**: 19 | **子序号**: 2

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerCfgId | int | 宴会配置ID | 是 | 必须为有效配置ID | > 0，配置必须存在 |
| permitId | int | 许可ID | 是 | 必须为玩家拥有的许可 | > 0，许可必须存在且未使用 |

#### 协议源码

```csharp
/// <summary>
/// 使用许可举办宴会
/// </summary>
public class GC2GS_019_002_ReqStartDinnerByPermit : _IALProtocolStructure
{
    private int dinnerCfgId;  // 宴会配置ID
    private int permitId;     // 许可ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)2; }

    // Getter/Setter
    public int getDinnerCfgId() { return dinnerCfgId; }
    public void setDinnerCfgId(int value) { dinnerCfgId = value; }
    public int getPermitId() { return permitId; }
    public void setPermitId(int value) { permitId = value; }
}
```

#### 请求示例

```csharp
// 使用许可举办宴会
NPPlayer.instance.dinnerComp.reqStartDinnerByPermit(dinnerCfgId, permitId, () => {
    Debug.Log("使用许可举办宴会成功");
});
```

---

### 2.3 GC2GS_019_003_ReqJoinedPlayerList - 请求已加入玩家列表

**主序号**: 19 | **子序号**: 3

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerId | long | 宴会ID | 是 | 必须存在 | > 0 |
| hostPlayerId | long | 举办者ID | 是 | 必须匹配宴会举办者 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求已加入玩家列表
/// </summary>
public class GC2GS_019_003_ReqJoinedPlayerList : _IALProtocolStructure
{
    private long dinnerId;      // 宴会ID
    private long hostPlayerId;  // 举办者ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)3; }

    // Getter/Setter
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public long getHostPlayerId() { return hostPlayerId; }
    public void setHostPlayerId(long value) { hostPlayerId = value; }
}
```

---

### 2.4 GC2GS_019_004_ReqGetStartLogIdxList - 请求宴会举办记录索引列表

**主序号**: 19 | **子序号**: 4

#### 协议字段

无请求参数，返回玩家自己的宴会举办记录索引。

#### 协议源码

```csharp
/// <summary>
/// 请求宴会举办记录索引列表
/// </summary>
public class GC2GS_019_004_ReqGetStartLogIdxList : _IALProtocolStructure
{
    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)4; }
}
```

---

### 2.5 GC2GS_019_005_ReqGetStartLogInfo - 请求宴会举办记录详情

**主序号**: 19 | **子序号**: 5

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| logId | long | 记录ID | 是 | 必须存在 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求宴会举办记录详情
/// </summary>
public class GC2GS_019_005_ReqGetStartLogInfo : _IALProtocolStructure
{
    private long logId;  // 记录ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)5; }

    // Getter/Setter
    public long getLogId() { return logId; }
    public void setLogId(long value) { logId = value; }
}
```

---

### 2.6 GC2GS_019_006_ReqGetDinnerIdxList - 请求宴会索引列表

**主序号**: 19 | **子序号**: 6

#### 协议字段

无请求参数，返回当前可参加的宴会索引列表。

#### 协议源码

```csharp
/// <summary>
/// 请求宴会索引列表
/// </summary>
public class GC2GS_019_006_ReqGetDinnerIdxList : _IALProtocolStructure
{
    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)6; }
}
```

---

### 2.7 GC2GS_019_007_ReqGetDinnerInfo - 请求宴会信息

**主序号**: 19 | **子序号**: 7

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerId | long | 宴会ID | 是 | 必须存在 | > 0 |
| hostPlayerId | long | 举办者ID | 是 | 必须匹配宴会举办者 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求宴会信息
/// </summary>
public class GC2GS_019_007_ReqGetDinnerInfo : _IALProtocolStructure
{
    private long dinnerId;      // 宴会ID
    private long hostPlayerId;  // 举办者ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)7; }

    // Getter/Setter
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public long getHostPlayerId() { return hostPlayerId; }
    public void setHostPlayerId(long value) { hostPlayerId = value; }
}
```

---

### 2.8 GC2GS_019_008_ReqJoinDinner - 请求加入宴会

**主序号**: 19 | **子序号**: 8

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerId | long | 宴会ID | 是 | 必须存在且进行中 | > 0 |
| hostPlayerId | long | 举办者ID | 是 | 必须匹配宴会举办者 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求加入宴会
/// </summary>
public class GC2GS_019_008_ReqJoinDinner : _IALProtocolStructure
{
    private long dinnerId;      // 宴会ID
    private long hostPlayerId;  // 举办者ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)8; }

    // Getter/Setter
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public long getHostPlayerId() { return hostPlayerId; }
    public void setHostPlayerId(long value) { hostPlayerId = value; }
}
```

#### 请求示例

```csharp
// 加入宴会
NPPlayer.instance.dinnerComp.reqJoinDinner(dinnerId, hostPlayerId, () => {
    Debug.Log("加入宴会成功");
});
```

#### 服务端处理流程

```mermaid
sequenceDiagram
    participant C as 客户端
    participant H as Handler
    participant S as Service
    participant DB as 数据库

    C->>H: GC2GS_019_008_ReqJoinDinner
    H->>S: joinDinner(cid, dinnerId, hostPlayerId)

    S->>DB: 查询宴会信息
    DB-->>S: 返回宴会数据

    alt 宴会不存在
        S-->>C: 返回错误码 19003
    else 宴会已结束
        S-->>C: 返回错误码 19004
    else 宴会已满
        S-->>C: 返回错误码 19005
    else 已加入
        S-->>C: 返回错误码 19006
    else 检查通过
        S->>DB: 添加参与记录
        S->>DB: 更新参与人数
        S-->>C: 返回成功
    end
```

---

### 2.9 GC2GS_019_009_ReqTakeOpenReward - 请求领取奖励

**主序号**: 19 | **子序号**: 9

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerId | long | 宴会ID | 是 | 必须存在 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求领取奖励
/// </summary>
public class GC2GS_019_009_ReqTakeOpenReward : _IALProtocolStructure
{
    private long dinnerId;  // 宴会ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)9; }

    // Getter/Setter
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
}
```

#### 请求示例

```csharp
// 领取奖励
NPPlayer.instance.dinnerComp.reqTakeOpenReward(dinnerId, () => {
    Debug.Log("领取奖励成功");
});
```

---

### 2.10 GC2GS_019_011_ReqGetPreDinnerInfo - 请求预览宴会信息

**主序号**: 19 | **子序号**: 11

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| dinnerCfgId | int | 宴会配置ID | 是 | 必须为有效配置ID | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求预览宴会信息
/// </summary>
public class GC2GS_019_011_ReqGetPreDinnerInfo : _IALProtocolStructure
{
    private int dinnerCfgId;  // 宴会配置ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)11; }

    // Getter/Setter
    public int getDinnerCfgId() { return dinnerCfgId; }
    public void setDinnerCfgId(int value) { dinnerCfgId = value; }
}
```

---

### 2.11 GC2GS_019_012_ReqGetNextDinnerInfo - 请求下一个宴会信息

**主序号**: 19 | **子序号**: 12

#### 协议字段

无请求参数，返回下一个可参加的宴会信息。

#### 协议源码

```csharp
/// <summary>
/// 请求下一个宴会信息
/// </summary>
public class GC2GS_019_012_ReqGetNextDinnerInfo : _IALProtocolStructure
{
    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)12; }
}
```

---

### 2.12 GC2GS_019_013_ReqSendInviteToPlayer - 请求发送邀请

**主序号**: 19 | **子序号**: 13

#### 协议字段

| 字段名 | 类型 | 说明 | 必填 | 校验规则 | 约束条件 |
|--------|------|------|------|----------|----------|
| targetPlayerId | long | 目标玩家ID | 是 | 必须为好友关系 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 请求发送邀请
/// </summary>
public class GC2GS_019_013_ReqSendInviteToPlayer : _IALProtocolStructure
{
    private long targetPlayerId;  // 目标玩家ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)13; }

    // Getter/Setter
    public long getTargetPlayerId() { return targetPlayerId; }
    public void setTargetPlayerId(long value) { targetPlayerId = value; }
}
```

#### 请求示例

```csharp
// 发送邀请
NPPlayer.instance.dinnerComp.reqSendInvite(targetPlayerId, () => {
    Debug.Log("邀请发送成功");
});
```

---

### 2.13 GC2GS_019_014_ReqCheckDinnerInvite - 请求检查宴会邀请

**主序号**: 19 | **子序号**: 14

#### 协议字段

无请求参数，返回收到的邀请列表。

#### 协议源码

```csharp
/// <summary>
/// 请求检查宴会邀请
/// </summary>
public class GC2GS_019_014_ReqCheckDinnerInvite : _IALProtocolStructure
{
    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)14; }
}
```

---

### 2.14 完整请求协议列表

| 协议名 | 主序号 | 子序号 | 说明 | 触发时机 | 频率限制 |
|--------|--------|--------|------|----------|----------|
| GC2GS_019_001_ReqStartDinner | 19 | 1 | 举办宴会 | 玩家点击举办 | 3次/秒 |
| GC2GS_019_002_ReqStartDinnerByPermit | 19 | 2 | 使用许可举办 | 玩家使用许可举办 | 3次/秒 |
| GC2GS_019_003_ReqJoinedPlayerList | 19 | 3 | 获取参与者列表 | 查看宴会详情 | 10次/秒 |
| GC2GS_019_004_ReqGetStartLogIdxList | 19 | 4 | 获取宴会记录索引 | 查看历史记录 | 10次/秒 |
| GC2GS_019_005_ReqGetStartLogInfo | 19 | 5 | 获取宴会记录详情 | 查看记录详情 | 10次/秒 |
| GC2GS_019_006_ReqGetDinnerIdxList | 19 | 6 | 获取宴会列表 | 进入宴会列表 | 10次/秒 |
| GC2GS_019_007_ReqGetDinnerInfo | 19 | 7 | 获取宴会信息 | 查看宴会详情 | 10次/秒 |
| GC2GS_019_008_ReqJoinDinner | 19 | 8 | 加入宴会 | 玩家点击加入 | 10次/秒 |
| GC2GS_019_009_ReqTakeOpenReward | 19 | 9 | 领取奖励 | 玩家点击领奖 | 5次/秒 |
| GC2GS_019_011_ReqGetPreDinnerInfo | 19 | 11 | 预览宴会信息 | 预览举办效果 | 10次/秒 |
| GC2GS_019_012_ReqGetNextDinnerInfo | 19 | 12 | 获取下一个宴会 | 快速找到目标 | 10次/秒 |
| GC2GS_019_013_ReqSendInviteToPlayer | 19 | 13 | 发送邀请 | 邀请好友 | 5次/秒 |
| GC2GS_019_014_ReqCheckDinnerInvite | 19 | 14 | 检查邀请 | 查看收到邀请 | 10次/秒 |

---

## 三、响应协议 (GS2GC)

### 3.1 GS2GC_019_001_RetStartDinner - 举办宴会响应

**主序号**: 19 | **子序号**: 1

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| result | int | 结果码（0表示成功） | >= 0 |
| dinnerInfo | Dinner_Info | 宴会完整数据 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 举办宴会响应
/// </summary>
public class GS2GC_019_001_RetStartDinner : _IALProtocolStructure
{
    private int result;               // 结果码
    private Dinner_Info dinnerInfo;   // 宴会信息

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)1; }

    // Getter/Setter
    public int getResult() { return result; }
    public void setResult(int value) { result = value; }
    public Dinner_Info getDinnerInfo() { return dinnerInfo; }
    public void setDinnerInfo(Dinner_Info value) { dinnerInfo = value; }
}
```

#### 客户端处理

```csharp
/// <summary>
/// 举办宴会响应处理
/// </summary>
public void handleRetStartDinner(GS2GC_019_001_RetStartDinner msg)
{
    if (msg.getResult() != 0)
    {
        // 显示错误提示
        UIManager.ShowErrorCode(msg.getResult());
        return;
    }

    // 更新宴会数据
    Dinner_Info dinnerInfo = msg.getDinnerInfo();
    DinnerInfo dinner = new DinnerInfo(dinnerInfo);
    _m_dinnerList.Add(dinner);

    // 发送消息通知UI
    WinMsg.SendMsg(WinMsgType.ON_DINNER_HOSTED, dinner);

    // 更新红点
    _m_redDealer?.refreshRedTip();
}
```

---

### 3.2 GS2GC_019_002_RetStartDinnerByPermit - 使用许可举办响应

**主序号**: 19 | **子序号**: 2

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| result | int | 结果码（0表示成功） | >= 0 |
| dinnerInfo | Dinner_Info | 宴会完整数据 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 使用许可举办宴会响应
/// </summary>
public class GS2GC_019_002_RetStartDinnerByPermit : _IALProtocolStructure
{
    private int result;               // 结果码
    private Dinner_Info dinnerInfo;   // 宴会信息

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)2; }

    // Getter/Setter
    public int getResult() { return result; }
    public void setResult(int value) { result = value; }
    public Dinner_Info getDinnerInfo() { return dinnerInfo; }
    public void setDinnerInfo(Dinner_Info value) { dinnerInfo = value; }
}
```

---

### 3.3 GS2GC_019_003_RetJoinedPlayerList - 已加入玩家列表响应

**主序号**: 19 | **子序号**: 3

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| result | int | 结果码 | >= 0 |
| dinnerId | long | 宴会ID | > 0 |
| joinerList | List\<DinnerJoinerInfo\> | 参与者列表 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 已加入玩家列表响应
/// </summary>
public class GS2GC_019_003_RetJoinedPlayerList : _IALProtocolStructure
{
    private int result;                           // 结果码
    private long dinnerId;                        // 宴会ID
    private List<DinnerJoinerInfo> joinerList;    // 参与者列表

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)3; }

    // Getter/Setter
    public int getResult() { return result; }
    public void setResult(int value) { result = value; }
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public List<DinnerJoinerInfo> getJoinerList() { return joinerList; }
    public void setJoinerList(List<DinnerJoinerInfo> value) { joinerList = value; }
}
```

---

### 3.4 GS2GC_019_008_RetJoinDinner - 加入宴会响应

**主序号**: 19 | **子序号**: 8

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| result | int | 结果码（0表示成功） | >= 0 |
| dinnerId | long | 宴会ID | > 0 |
| joinerInfo | DinnerJoinerInfo | 参与者信息 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 加入宴会响应
/// </summary>
public class GS2GC_019_008_RetJoinDinner : _IALProtocolStructure
{
    private int result;                 // 结果码
    private long dinnerId;              // 宴会ID
    private DinnerJoinerInfo joinerInfo; // 参与者信息

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)8; }

    // Getter/Setter
    public int getResult() { return result; }
    public void setResult(int value) { result = value; }
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public DinnerJoinerInfo getJoinerInfo() { return joinerInfo; }
    public void setJoinerInfo(DinnerJoinerInfo value) { joinerInfo = value; }
}
```

---

### 3.5 GS2GC_019_009_RetTakeOpenReward - 领取奖励响应

**主序号**: 19 | **子序号**: 9

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| result | int | 结果码（0表示成功） | >= 0 |
| dinnerId | long | 宴会ID | > 0 |
| rewardList | List\<NPCommonCostItem\> | 奖励列表 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 领取奖励响应
/// </summary>
public class GS2GC_019_009_RetTakeOpenReward : _IALProtocolStructure
{
    private int result;                         // 结果码
    private long dinnerId;                      // 宴会ID
    private List<NPCommonCostItem> rewardList;  // 奖励列表

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)9; }

    // Getter/Setter
    public int getResult() { return result; }
    public void setResult(int value) { result = value; }
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public List<NPCommonCostItem> getRewardList() { return rewardList; }
    public void setRewardList(List<NPCommonCostItem> value) { rewardList = value; }
}
```

#### 客户端处理

```csharp
/// <summary>
/// 领取奖励响应处理
/// </summary>
public void handleRetTakeOpenReward(GS2GC_019_009_RetTakeOpenReward msg)
{
    if (msg.getResult() != 0)
    {
        UIManager.ShowErrorCode(msg.getResult());
        return;
    }

    // 更新本地数据
    DinnerInfo dinner = _m_dinnerList.Find(d => d.dinnerId == msg.getDinnerId());
    if (dinner != null)
    {
        // 更新领取状态
        long myCid = NPPlayer.instance.cid;
        DinnerJoinerInfo joiner = dinner.joinerList.Find(j => j.playerId == myCid);
        if (joiner != null)
        {
            joiner.rewardTaken = true;
        }
    }

    // 显示奖励
    var rewardList = msg.getRewardList();
    UIManager.ShowRewardWindow(rewardList);

    // 更新红点
    _m_redDealer?.refreshRedTip();
}
```

---

### 3.6 GS2GC_019_013_RetSendInviteToPlayer - 发送邀请响应

**主序号**: 19 | **子序号**: 13

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| result | int | 结果码（0表示成功） | >= 0 |

#### 协议源码

```csharp
/// <summary>
/// 发送邀请响应
/// </summary>
public class GS2GC_019_013_RetSendInviteToPlayer : _IALProtocolStructure
{
    private int result;  // 结果码

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)13; }

    // Getter/Setter
    public int getResult() { return result; }
    public void setResult(int value) { result = value; }
}
```

---

### 3.7 完整响应协议列表

| 协议名 | 主序号 | 子序号 | 说明 | 返回数据 |
|--------|--------|--------|------|----------|
| GS2GC_019_001_RetStartDinner | 19 | 1 | 举办宴会响应 | dinnerInfo |
| GS2GC_019_002_RetStartDinnerByPermit | 19 | 2 | 使用许可举办响应 | dinnerInfo |
| GS2GC_019_003_RetJoinedPlayerList | 19 | 3 | 参与者列表响应 | joinerList |
| GS2GC_019_004_RetGetStartLogIdxList | 19 | 4 | 宴会记录索引响应 | logIdxList |
| GS2GC_019_005_RetGetStartLogInfo | 19 | 5 | 宴会记录详情响应 | logInfo |
| GS2GC_019_006_RetGetDinnerIdxList | 19 | 6 | 宴会列表响应 | dinnerIdxList |
| GS2GC_019_007_RetGetDinnerInfo | 19 | 7 | 宴会信息响应 | dinnerInfo |
| GS2GC_019_008_RetJoinDinner | 19 | 8 | 加入宴会响应 | dinnerId, joinerInfo |
| GS2GC_019_009_RetTakeOpenReward | 19 | 9 | 领取奖励响应 | dinnerId, rewardList |
| GS2GC_019_011_RetGetPreDinnerInfo | 19 | 11 | 预览宴会信息响应 | preDinnerInfo |
| GS2GC_019_012_RetGetNextDinnerInfo | 19 | 12 | 下一个宴会响应 | dinnerInfo |
| GS2GC_019_013_RetSendInviteToPlayer | 19 | 13 | 发送邀请响应 | result |
| GS2GC_019_014_RetCheckDinnerInvite | 19 | 14 | 检查邀请响应 | inviteList |

---

## 四、推送协议 (GS2GC)

### 4.1 GS2GC_019_050_OnDinnerAdd - 宴会添加推送

**主序号**: 19 | **子序号**: 50

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| dinnerInfo | Dinner_Info | 新增的宴会信息 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 宴会添加推送
/// </summary>
public class GS2GC_019_050_OnDinnerAdd : _IALProtocolStructure
{
    private Dinner_Info dinnerInfo;  // 宴会信息

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)50; }

    // Getter/Setter
    public Dinner_Info getDinnerInfo() { return dinnerInfo; }
    public void setDinnerInfo(Dinner_Info value) { dinnerInfo = value; }
}
```

#### 客户端处理

```csharp
/// <summary>
/// 宴会添加推送监听器
/// </summary>
public class GSSubDealer_019_050_OnDinnerAdd : _AGSSubDealerBase
{
    public override void deal(NPPlayer _player, GS2GC_019_050_OnDinnerAdd _msg)
    {
        Dinner_Info dinnerInfo = _msg.getDinnerInfo();
        _player.dinnerComp.retOnDinnerAdd(dinnerInfo);
    }
}

/// <summary>
/// 组件处理宴会添加
/// </summary>
public void retOnDinnerAdd(Dinner_Info _info)
{
    if (null == _info) return;

    DinnerInfo dinnerInfo = new DinnerInfo(_info);

    // 检查是否已存在，更新或添加
    int index = _m_dinnerList.FindIndex(d => d.dinnerId == dinnerInfo.dinnerId);
    if (index >= 0)
    {
        _m_dinnerList[index] = dinnerInfo;
    }
    else
    {
        _m_dinnerList.Add(dinnerInfo);
    }

    // 发送消息通知UI
    WinMsg.SendMsg(WinMsgType.ON_DINNER_ADD, dinnerInfo);

    // 更新红点
    _m_redDealer?.refreshRedTip();
}
```

---

### 4.2 GS2GC_019_051_OnDinnerEnd - 宴会结束推送

**主序号**: 19 | **子序号**: 51

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| dinnerId | long | 结束的宴会ID | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 宴会结束推送
/// </summary>
public class GS2GC_019_051_OnDinnerEnd : _IALProtocolStructure
{
    private long dinnerId;  // 宴会ID

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)51; }

    // Getter/Setter
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
}
```

#### 客户端处理

```csharp
/// <summary>
/// 宴会结束处理
/// </summary>
public void retOnDinnerEnd(long _dinnerId)
{
    // 更新宴会状态
    DinnerInfo dinner = _m_dinnerList.Find(d => d.dinnerId == _dinnerId);
    if (dinner != null)
    {
        dinner.state = EDinnerState.ENDED;
    }

    // 发送消息通知UI
    WinMsg.SendMsg(WinMsgType.ON_DINNER_END, _dinnerId);

    // 更新红点
    _m_redDealer?.refreshRedTip();
}
```

---

### 4.3 GS2GC_019_052_OnPermitAdd - 许可添加推送

**主序号**: 19 | **子序号**: 52

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| permitId | int | 许可配置ID | > 0 |
| count | int | 数量 | > 0 |

#### 协议源码

```csharp
/// <summary>
/// 许可添加推送
/// </summary>
public class GS2GC_019_052_OnPermitAdd : _IALProtocolStructure
{
    private int permitId;  // 许可配置ID
    private int count;     // 数量

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)52; }

    // Getter/Setter
    public int getPermitId() { return permitId; }
    public void setPermitId(int value) { permitId = value; }
    public int getCount() { return count; }
    public void setCount(int value) { count = value; }
}
```

#### 客户端处理

```csharp
/// <summary>
/// 许可添加处理
/// </summary>
public void retOnPermitAdd(int _permitId, int _count)
{
    // 更新许可数据
    // ...

    // 发送消息通知UI
    WinMsg.SendMsg(WinMsgType.ON_PERMIT_ADD, _permitId, _count);

    // 更新红点
    _m_redDealer?.refreshRedTip();
}
```

---

### 4.4 GS2GC_019_053_OnJoinerAdd - 参与者添加推送

**主序号**: 19 | **子序号**: 53

#### 协议字段

| 字段名 | 类型 | 说明 | 约束条件 |
|--------|------|------|----------|
| dinnerId | long | 宴会ID | > 0 |
| joinerInfo | DinnerJoinerInfo | 新增参与者信息 | 非空 |

#### 协议源码

```csharp
/// <summary>
/// 参与者添加推送
/// </summary>
public class GS2GC_019_053_OnJoinerAdd : _IALProtocolStructure
{
    private long dinnerId;              // 宴会ID
    private DinnerJoinerInfo joinerInfo; // 参与者信息

    public byte getMainOrder() { return (byte)19; }
    public byte getSubOrder() { return (byte)53; }

    // Getter/Setter
    public long getDinnerId() { return dinnerId; }
    public void setDinnerId(long value) { dinnerId = value; }
    public DinnerJoinerInfo getJoinerInfo() { return joinerInfo; }
    public void setJoinerInfo(DinnerJoinerInfo value) { joinerInfo = value; }
}
```

#### 客户端处理

```csharp
/// <summary>
/// 参与者添加处理
/// </summary>
public void retOnJoinerAdd(long _dinnerId, DinnerJoinerInfo _joiner)
{
    DinnerInfo dinner = _m_dinnerList.Find(d => d.dinnerId == _dinnerId);
    if (dinner != null && _joiner != null)
    {
        // 添加参与者
        dinner.joinerList.Add(new DinnerJoinerInfo(_joiner));

        // 发送消息通知UI
        WinMsg.SendMsg(WinMsgType.ON_DINNER_JOINER_ADD, _dinnerId, _joiner);
    }
}
```

---

### 4.5 完整推送协议列表

| 协议名 | 主序号 | 子序号 | 说明 | 触发时机 | 推送范围 |
|--------|--------|--------|------|----------|----------|
| GS2GC_019_050_OnDinnerAdd | 19 | 50 | 宴会添加推送 | 新宴会创建 | 全服玩家 |
| GS2GC_019_051_OnDinnerEnd | 19 | 51 | 宴会结束推送 | 宴会时间到 | 宴会参与者 |
| GS2GC_019_052_OnPermitAdd | 19 | 52 | 许可添加推送 | 获得新许可 | 单个玩家 |
| GS2GC_019_053_OnJoinerAdd | 19 | 53 | 参与者添加推送 | 新玩家加入 | 宴会参与者 |

---

## 五、协议交互流程

### 5.1 举办宴会完整流程

```mermaid
sequenceDiagram
    participant C as 客户端
    participant S as 服务端
    participant DB as 数据库
    participant Item as 道具模块
    participant Push as 推送服务

    C->>S: GC2GS_019_001_ReqStartDinner<br/>(dinnerCfgId)

    S->>S: 验证宴会配置
    S->>S: 检查是否已举办宴会
    S->>S: 计算消耗资源

    alt 资源不足
        S-->>C: 返回错误码: 资源不足
    else 检查通过
        S->>Item: 扣除消耗资源
        S->>DB: 创建宴会记录
        S->>DB: 举办者自动加入
        S->>S: 设置结束时间

        S-->>C: GS2GC_019_001_RetStartDinner<br/>(dinnerInfo)

        S->>Push: 推送宴会添加通知
        Push-->>C: GS2GC_019_050_OnDinnerAdd
    end
```

### 5.2 加入宴会完整流程

```mermaid
sequenceDiagram
    participant C as 客户端
    participant S as 服务端
    participant DB as 数据库

    C->>S: GC2GS_019_008_ReqJoinDinner<br/>(dinnerId, hostPlayerId)

    S->>DB: 查询宴会信息
    DB-->>S: 返回宴会数据

    alt 宴会不存在
        S-->>C: 返回错误码: 宴会不存在
    else 宴会已结束
        S-->>C: 返回错误码: 宴会已结束
    else 宴会已满
        S-->>C: 返回错误码: 宴会已满
    else 已加入过
        S-->>C: 返回错误码: 已加入
    else 检查通过
        S->>DB: 添加参与记录
        S->>DB: 更新参与人数
        S-->>C: GS2GC_019_008_RetJoinDinner<br/>(dinnerId, joinerInfo)
        S-->>C: GS2GC_019_053_OnJoinerAdd
    end
```

### 5.3 领取奖励流程

```mermaid
sequenceDiagram
    participant C as 客户端
    participant S as 服务端
    participant DB as 数据库
    participant Reward as 奖励模块

    C->>S: GC2GS_019_009_ReqTakeOpenReward<br/>(dinnerId)

    S->>DB: 查询参与记录
    DB-->>S: 返回参与数据

    alt 未参加宴会
        S-->>C: 返回错误码: 未参加宴会
    else 已领取
        S-->>C: 返回错误码: 奖励已领取
    else 检查通过
        S->>S: 计算基础奖励
        S->>S: 检查许可额外奖励
        S->>Reward: 发放奖励
        S->>DB: 更新领取状态
        S-->>C: GS2GC_019_009_RetTakeOpenReward<br/>(rewardList)
    end
```

### 5.4 发送邀请流程

```mermaid
sequenceDiagram
    participant C as 客户端
    participant S as 服务端
    participant DB as 数据库
    participant Friend as 好友模块

    C->>S: GC2GS_019_013_ReqSendInviteToPlayer<br/>(targetPlayerId)

    S->>DB: 查询正在举办的宴会
    DB-->>S: 返回宴会数据

    alt 没有举办宴会
        S-->>C: 返回错误码: 没有进行中的宴会
    else 宴会已结束
        S-->>C: 返回错误码: 宴会已结束
    else
        S->>Friend: 检查好友关系
        alt 非好友关系
            S-->>C: 返回错误码: 非好友关系
        else 是好友
            S->>DB: 创建邀请记录
            S-->>C: GS2GC_019_013_RetSendInviteToPlayer
            Note over S: 推送邀请给目标玩家
        end
    end
```

### 5.5 宴会结束定时检查流程

```mermaid
sequenceDiagram
    participant Timer as 定时器
    participant S as 服务端
    participant DB as 数据库
    participant C as 客户端

    loop 每分钟检查
        Timer->>S: 检查过期宴会
        S->>DB: 查询state=1且end_time<=now
        DB-->>S: 返回过期宴会列表

        loop 遍历每个宴会
            S->>DB: 更新状态为已结束
            S-->>C: GS2GC_019_051_OnDinnerEnd
        end
    end
```

---

## 六、协议错误码

### 6.1 宴会模块错误码定义

| 错误码 | 常量名 | 说明 | 触发场景 | 用户提示 |
|--------|--------|------|----------|----------|
| 19001 | DINNER_CONFIG_NOT_FOUND | 宴会配置不存在 | 查询配置时 | "宴会类型不存在" |
| 19002 | DINNER_ALREADY_HOSTED | 已举办未结束的宴会 | 举办宴会时 | "您已举办了宴会，请等待结束" |
| 19003 | DINNER_NOT_FOUND | 宴会不存在 | 查询宴会时 | "该宴会不存在" |
| 19004 | DINNER_ENDED | 宴会已结束 | 加入宴会时 | "该宴会已结束" |
| 19005 | DINNER_FULL | 宴会已满 | 加入宴会时 | "该宴会人数已满" |
| 19006 | ALREADY_JOINED | 已加入该宴会 | 加入宴会时 | "您已参加该宴会" |
| 19007 | NOT_JOINED | 未参加该宴会 | 领取奖励时 | "您未参加该宴会" |
| 19008 | REWARD_TAKEN | 奖励已领取 | 领取奖励时 | "奖励已领取" |
| 19009 | PERMIT_NOT_FOUND | 许可不存在 | 使用许可时 | "许可不存在" |
| 19010 | PERMIT_USED | 许可已使用 | 使用许可时 | "该许可已使用" |
| 19011 | NOT_FRIEND | 非好友关系 | 发送邀请时 | "只能邀请好友" |
| 19012 | NO_HOSTING_DINNER | 没有举办的宴会 | 发送邀请时 | "您没有正在举办的宴会" |
| 19013 | RESOURCE_NOT_ENOUGH | 资源不足 | 举办宴会时 | "资源不足" |
| 19014 | LEVEL_NOT_ENOUGH | 等级不足 | 举办宴会时 | "等级不足" |
| 19015 | VIP_LEVEL_NOT_ENOUGH | VIP等级不足 | 举办宴会时 | "VIP等级不足" |

### 6.2 错误码分类图

```mermaid
graph LR
    subgraph 宴会相关
        A[19001<br/>配置不存在]
        B[19002<br/>已举办宴会]
        C[19003<br/>宴会不存在]
        D[19004<br/>宴会已结束]
        E[19005<br/>宴会已满]
    end

    subgraph 参与相关
        F[19006<br/>已加入]
        G[19007<br/>未参加]
    end

    subgraph 奖励相关
        H[19008<br/>奖励已领取]
    end

    subgraph 许可相关
        I[19009<br/>许可不存在]
        J[19010<br/>许可已使用]
    end

    subgraph 邀请相关
        K[19011<br/>非好友]
        L[19012<br/>无举办的宴会]
    end

    subgraph 条件相关
        M[19013<br/>资源不足]
        N[19014<br/>等级不足]
        O[19015<br/>VIP等级不足]
    end
```

### 6.3 错误处理流程

```mermaid
flowchart TD
    A[收到协议响应] --> B{检查result字段}
    B -->|result=0| C[正常处理]
    B -->|result≠0| D[进入错误处理]

    D --> E{错误码分类}
    E -->|资源不足| F[显示充值引导]
    E -->|宴会已满| G[显示其他宴会]
    E -->|宴会已结束| H[刷新宴会列表]
    E -->|奖励已领取| I[忽略处理]
    E -->|其他错误| J[显示通用错误提示]

    C --> K[更新本地数据]
    K --> L[刷新UI显示]
```

### 6.4 错误码定义代码

```csharp
/// <summary>
/// 宴会模块错误码
/// </summary>
public static class DinnerErrorCode
{
    /// <summary>宴会配置不存在</summary>
    public const int DINNER_CONFIG_NOT_FOUND = 19001;
    /// <summary>已举办未结束的宴会</summary>
    public const int DINNER_ALREADY_HOSTED = 19002;
    /// <summary>宴会不存在</summary>
    public const int DINNER_NOT_FOUND = 19003;
    /// <summary>宴会已结束</summary>
    public const int DINNER_ENDED = 19004;
    /// <summary>宴会已满</summary>
    public const int DINNER_FULL = 19005;
    /// <summary>已加入该宴会</summary>
    public const int ALREADY_JOINED = 19006;
    /// <summary>未参加该宴会</summary>
    public const int NOT_JOINED = 19007;
    /// <summary>奖励已领取</summary>
    public const int REWARD_TAKEN = 19008;
    /// <summary>许可不存在</summary>
    public const int PERMIT_NOT_FOUND = 19009;
    /// <summary>许可已使用</summary>
    public const int PERMIT_USED = 19010;
    /// <summary>非好友关系</summary>
    public const int NOT_FRIEND = 19011;
    /// <summary>没有举办的宴会</summary>
    public const int NO_HOSTING_DINNER = 19012;
    /// <summary>资源不足</summary>
    public const int RESOURCE_NOT_ENOUGH = 19013;
    /// <summary>等级不足</summary>
    public const int LEVEL_NOT_ENOUGH = 19014;
    /// <summary>VIP等级不足</summary>
    public const int VIP_LEVEL_NOT_ENOUGH = 19015;

    /// <summary>
    /// 获取错误提示信息
    /// </summary>
    public static string GetErrorMessage(int errorCode)
    {
        return errorCode switch
        {
            DINNER_CONFIG_NOT_FOUND => "宴会类型不存在",
            DINNER_ALREADY_HOSTED => "您已举办了宴会，请等待结束",
            DINNER_NOT_FOUND => "该宴会不存在",
            DINNER_ENDED => "该宴会已结束",
            DINNER_FULL => "该宴会人数已满",
            ALREADY_JOINED => "您已参加该宴会",
            NOT_JOINED => "您未参加该宴会",
            REWARD_TAKEN => "奖励已领取",
            PERMIT_NOT_FOUND => "许可不存在",
            PERMIT_USED => "该许可已使用",
            NOT_FRIEND => "只能邀请好友",
            NO_HOSTING_DINNER => "您没有正在举办的宴会",
            RESOURCE_NOT_ENOUGH => "资源不足",
            LEVEL_NOT_ENOUGH => "等级不足",
            VIP_LEVEL_NOT_ENOUGH => "VIP等级不足",
            _ => "未知错误"
        };
    }
}
```

---

## 七、协议安全机制

### 7.1 请求频率限制

| 协议类型 | 频率限制 | 限制策略 | 超限处理 |
|----------|----------|----------|----------|
| 举办宴会 | 3次/秒 | 滑动窗口 | 返回"操作过于频繁" |
| 加入宴会 | 10次/秒 | 滑动窗口 | 返回"操作过于频繁" |
| 发送邀请 | 5次/秒 | 滑动窗口 | 返回"操作过于频繁" |
| 领取奖励 | 5次/秒 | 滑动窗口 | 返回"操作过于频繁" |

### 7.2 参数校验规则

```mermaid
flowchart TD
    A[请求参数] --> B{基础校验}
    B -->|通过| C{业务校验}
    B -->|失败| D[返回参数错误]

    C --> E{ID有效性}
    E -->|无效| F[返回配置不存在]
    E -->|有效| G{数值范围}
    G -->|超限| H[返回数值超限]
    G -->|正常| I{关联校验}
    I -->|失败| J[返回关联错误]
    I -->|成功| K[继续处理]

    subgraph 校验项
        L["dinnerCfgId > 0"]
        M["dinnerId > 0"]
        N["hostPlayerId > 0"]
        O["宴会配置必须存在"]
        P["宴会必须存在且进行中"]
    end
```

### 7.3 服务端校验代码

```java
/**
 * 宴会请求参数校验器
 */
public class DinnerRequestValidator {

    /**
     * 校验举办宴会请求
     */
    public void validateStartDinner(long cid, int dinnerCfgId) {
        // 基础校验
        if (dinnerCfgId <= 0) {
            throw new BusinessException(ErrorCode.INVALID_PARAM, "dinnerCfgId无效");
        }

        // 配置校验
        DinnerConfig config = dinnerConfigDao.getById(dinnerCfgId);
        if (config == null) {
            throw new BusinessException(ErrorCode.DINNER_CONFIG_NOT_FOUND);
        }

        // 玩家等级校验
        Player player = playerDao.getById(cid);
        if (player.getLevel() < config.getUnlockLevel()) {
            throw new BusinessException(ErrorCode.LEVEL_NOT_ENOUGH);
        }

        // VIP等级校验
        if (player.getVipLevel() < config.getVipLevel()) {
            throw new BusinessException(ErrorCode.VIP_LEVEL_NOT_ENOUGH);
        }

        // 状态校验
        Dinner existingDinner = dinnerDao.selectOngoingByHost(cid);
        if (existingDinner != null) {
            throw new BusinessException(ErrorCode.DINNER_ALREADY_HOSTED);
        }
    }

    /**
     * 校验加入宴会请求
     */
    public void validateJoinDinner(long cid, long dinnerId, long hostPlayerId) {
        // 基础校验
        if (dinnerId <= 0 || hostPlayerId <= 0) {
            throw new BusinessException(ErrorCode.INVALID_PARAM, "参数无效");
        }

        // 宴会校验
        Dinner dinner = dinnerDao.selectById(dinnerId);
        if (dinner == null || dinner.getHostPlayerId() != hostPlayerId) {
            throw new BusinessException(ErrorCode.DINNER_NOT_FOUND);
        }

        // 状态校验
        if (dinner.getState() != EDinnerState.ONGOING.getValue()) {
            throw new BusinessException(ErrorCode.DINNER_ENDED);
        }

        // 时间校验
        if (dinner.getEndTime().before(new Date())) {
            throw new BusinessException(ErrorCode.DINNER_ENDED);
        }

        // 人数校验
        DinnerConfig config = dinnerConfigDao.getById(dinner.getDinnerCfgId());
        if (dinner.getJoinerCount() >= config.getMaxJoiner()) {
            throw new BusinessException(ErrorCode.DINNER_FULL);
        }

        // 参与校验
        DinnerJoiner existingJoiner = joinerDao.selectByDinnerAndPlayer(dinnerId, cid);
        if (existingJoiner != null) {
            throw new BusinessException(ErrorCode.ALREADY_JOINED);
        }
    }
}
```

---

## 八、客户端协议封装

### 8.1 协议发送器

```csharp
/// <summary>
/// 宴会协议发送器
/// </summary>
public static class GSWriter_019_DinnerOp
{
    /// <summary>
    /// 举办宴会
    /// </summary>
    public static GC2GS_019_001_ReqStartDinner make_001_ReqStartDinner(int dinnerCfgId)
    {
        GC2GS_019_001_ReqStartDinner req = new GC2GS_019_001_ReqStartDinner();
        req.setDinnerCfgId(dinnerCfgId);
        return req;
    }

    /// <summary>
    /// 使用许可举办宴会
    /// </summary>
    public static GC2GS_019_002_ReqStartDinnerByPermit make_002_ReqStartDinnerByPermit(
        int dinnerCfgId, int permitId)
    {
        GC2GS_019_002_ReqStartDinnerByPermit req = new GC2GS_019_002_ReqStartDinnerByPermit();
        req.setDinnerCfgId(dinnerCfgId);
        req.setPermitId(permitId);
        return req;
    }

    /// <summary>
    /// 加入宴会
    /// </summary>
    public static GC2GS_019_008_ReqJoinDinner make_008_ReqJoinDinner(
        long dinnerId, long hostPlayerId)
    {
        GC2GS_019_008_ReqJoinDinner req = new GC2GS_019_008_ReqJoinDinner();
        req.setDinnerId(dinnerId);
        req.setHostPlayerId(hostPlayerId);
        return req;
    }

    /// <summary>
    /// 领取奖励
    /// </summary>
    public static GC2GS_019_009_ReqTakeOpenReward make_009_ReqTakeOpenReward(long dinnerId)
    {
        GC2GS_019_009_ReqTakeOpenReward req = new GC2GS_019_009_ReqTakeOpenReward();
        req.setDinnerId(dinnerId);
        return req;
    }

    /// <summary>
    /// 发送邀请
    /// </summary>
    public static GC2GS_019_013_ReqSendInviteToPlayer make_013_ReqSendInviteToPlayer(
        long targetPlayerId)
    {
        GC2GS_019_013_ReqSendInviteToPlayer req = new GC2GS_019_013_ReqSendInviteToPlayer();
        req.setTargetPlayerId(targetPlayerId);
        return req;
    }

    /// <summary>
    /// 检查宴会邀请
    /// </summary>
    public static GC2GS_019_014_ReqCheckDinnerInvite make_014_ReqCheckDinnerInvite()
    {
        return new GC2GS_019_014_ReqCheckDinnerInvite();
    }
}
```

### 8.2 组件封装调用

```csharp
/// <summary>
/// 宴会组件协议发送方法
/// </summary>
public partial class NPPlayerDinnerComponent : _ANPBasicPlayerComponent
{
    /// <summary>
    /// 请求举办宴会
    /// </summary>
    public void reqStartDinner(int dinnerCfgId, Action _backAction = null)
    {
        NPGSClientListener.sendRequest(
            GSWriter_019_DinnerOp.make_001_ReqStartDinner(dinnerCfgId),
            new CommonErrCodeRequestCallbackProtocolDealer<GS2GC_019_001_RetStartDinner>((msg) =>
            {
                if (msg.getResult() == 0)
                {
                    retOnDinnerAdd(msg.getDinnerInfo());
                }
                _backAction?.Invoke();
            }));
    }

    /// <summary>
    /// 请求使用许可举办宴会
    /// </summary>
    public void reqStartDinnerByPermit(int dinnerCfgId, int permitId, Action _backAction = null)
    {
        NPGSClientListener.sendRequest(
            GSWriter_019_DinnerOp.make_002_ReqStartDinnerByPermit(dinnerCfgId, permitId),
            new CommonErrCodeRequestCallbackProtocolDealer<GS2GC_019_002_RetStartDinnerByPermit>((msg) =>
            {
                if (msg.getResult() == 0)
                {
                    retOnDinnerAdd(msg.getDinnerInfo());
                }
                _backAction?.Invoke();
            }));
    }

    /// <summary>
    /// 请求加入宴会
    /// </summary>
    public void reqJoinDinner(long dinnerId, long hostPlayerId, Action _backAction = null)
    {
        NPGSClientListener.sendRequest(
            GSWriter_019_DinnerOp.make_008_ReqJoinDinner(dinnerId, hostPlayerId),
            new CommonErrCodeRequestCallbackProtocolDealer<GS2GC_019_008_RetJoinDinner>((msg) =>
            {
                if (msg.getResult() == 0)
                {
                    retOnJoinerAdd(dinnerId, msg.getJoinerInfo());
                }
                _backAction?.Invoke();
            }));
    }

    /// <summary>
    /// 请求领取奖励
    /// </summary>
    public void reqTakeOpenReward(long dinnerId, Action _backAction = null)
    {
        NPGSClientListener.sendRequest(
            GSWriter_019_DinnerOp.make_009_ReqTakeOpenReward(dinnerId),
            new CommonErrCodeRequestCallbackProtocolDealer<GS2GC_019_009_RetTakeOpenReward>((msg) =>
            {
                if (msg.getResult() == 0)
                {
                    // 处理奖励
                    handleReward(dinnerId, msg.getRewardList());
                }
                _backAction?.Invoke();
            }));
    }

    /// <summary>
    /// 请求发送邀请
    /// </summary>
    public void reqSendInviteToPlayer(long targetPlayerId, Action _backAction = null)
    {
        NPGSClientListener.sendRequest(
            GSWriter_019_DinnerOp.make_013_ReqSendInviteToPlayer(targetPlayerId),
            new CommonErrCodeRequestCallbackProtocolDealer<GS2GC_019_013_RetSendInviteToPlayer>((msg) =>
            {
                _backAction?.Invoke();
            }));
    }
}
```

### 8.3 监听器注册

```csharp
/// <summary>
/// 注册宴会协议监听器
/// </summary>
public void RegisterDinnerProtocolListeners()
{
    // 宴会添加推送
    NPGSClientListener.registSubDealer(
        new GSSubDealer_019_050_OnDinnerAdd());

    // 宴会结束推送
    NPGSClientListener.registSubDealer(
        new GSSubDealer_019_051_OnDinnerEnd());

    // 许可添加推送
    NPGSClientListener.registSubDealer(
        new GSSubDealer_019_052_OnPermitAdd());

    // 参与者添加推送
    NPGSClientListener.registSubDealer(
        new GSSubDealer_019_053_OnJoinerAdd());
}
```

---

## 九、协议监控与日志

### 9.1 协议监控指标

| 指标名称 | 计算方式 | 告警阈值 | 说明 |
|----------|----------|----------|------|
| 举办成功率 | 成功数/总请求数 | <90% | 核心业务指标 |
| 加入成功率 | 成功数/总请求数 | <85% | 核心业务指标 |
| 平均响应时间 | 总耗时/请求数 | >200ms | 性能指标 |
| P99响应时间 | 第99百分位耗时 | >500ms | 极端情况监控 |
| 错误率 | 错误数/总请求数 | >5% | 稳定性指标 |

### 9.2 协议日志格式

```json
{
  "timestamp": "2026-04-12T10:30:00.000Z",
  "protocol": "GC2GS_019_008_ReqJoinDinner",
  "mainOrder": 19,
  "subOrder": 8,
  "cid": 12345678,
  "request": {
    "dinnerId": 50001,
    "hostPlayerId": 20001
  },
  "response": {
    "result": 0,
    "dinnerId": 50001
  },
  "duration": 45,
  "serverIp": "192.168.1.100"
}
```

---

## 十、协议版本兼容

### 10.1 版本兼容策略

```mermaid
flowchart TD
    A[新版本协议] --> B{检查客户端版本}
    B -->|旧版本| C[兼容处理]
    B -->|新版本| D[正常处理]

    C --> E[忽略新增字段]
    C --> F[使用默认值填充]
    C --> G[返回兼容响应]

    D --> H[使用完整功能]

    subgraph 兼容规则
        I["新增字段: 旧版本忽略"]
        J["删除字段: 服务端保留兼容"]
        K["修改字段: 保持向后兼容"]
    end
```

### 10.2 协议升级指南

| 升级类型 | 兼容性 | 处理方式 |
|----------|--------|----------|
| 新增字段 | 向后兼容 | 旧版本忽略，新版本使用 |
| 新增协议 | 向后兼容 | 旧版本不调用，新版本使用 |
| 删除字段 | 不兼容 | 需要强制更新 |
| 修改字段类型 | 不兼容 | 需要强制更新 |

---

*文档生成时间: 2026-04-12*
*文档版本: v2.1*
