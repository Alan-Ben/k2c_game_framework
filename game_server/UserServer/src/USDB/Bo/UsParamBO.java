...
@RefBo(isIdAuto= true)
public class UsParamBO extends BaseBO {
    ...
    public UsParamBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        tag = rs.getInt(2);
        param = rs.getLong(3);
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `us_param` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`tag` int(11) NOT NULL DEFAULT '0' COMMENT '参数标记',"
                + "`param` bigint(20) NOT NULL DEFAULT '0' COMMENT '参数值',"
                + "KEY `tag` (`tag`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='user服务器参数信息' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
