...
public class TileMatchLogic_BoxBox extends _ATileMatchGameLogic
{
    private int _m_blockIndex;
    private int _m_subBlockIndex;

    public TileMatchLogic_BoxBox(int _blockIndex, int _subBlockIndex)
    {
    ...
    {
        return ETileMatchLogicEnum.BOX_BOX;
    }
    ...
    public void runLogic(TileMatchBlockList _blockList, TileMatchGameLogicContext _gameContext)
    {
        ...
        if (refMainBlock == null || refMainBlock.type != ETileMatch_BlockType.BOOM)
            return;
        ...
        if (refSubBlock == null || refSubBlock.type != ETileMatch_BlockType.BOOM)
            return;
        ...
        //标记周围25格清除
        ArrayList<Integer> nearPosList_25
                = TileMatchBlockList.calPosNearPosList_x(posXY.first(), posXY.second(), RefTileMatchOther.Ref().boom_boom_arg);
        for (Integer index : nearPosList_25)
        {
            ...
            //如果范围内的道具被触发，需要排除掉自己
            RefTileMatchBlock refTempBlock = RefTileMatchBlock.getMgr().get(block.getBlockId());
        ...
        _gameContext.addLogicResult(ETileMatch_LogicType.BOX_BOX, gainScore, proto);
    }
}
