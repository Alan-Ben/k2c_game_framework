...
namespace GOE
{
    ...
    /// 联盟冲榜详情主界面
    /// </summary>
    public class GGUIWndGuildRankRushDetail : _ANPGGUIBasicResBarWnd<GGUIMonoGuildRankRushDetail>
    {
        private static GGUIWndGuildRankRushDetail _g_instance;
        public static GGUIWndGuildRankRushDetail instance
        {
            get
            {
                if (_g_instance == null)
                    _g_instance = new GGUIWndGuildRankRushDetail();
                return _g_instance;
            }
        }
        ...
        //当前选中的页签
        private GGUIWndRankRushDetailTab _m_wSelectTabWnd;
        //冲榜详情前几名信息附加窗口
        private List<GGUIWndRankRushTopGuildInfo> _m_lTopGuildInfoList;
        //排行榜列表
        private List<GuildRankInfo> _m_lRankShowInfoList;
        //任务刷新定时器
        private ALCommonEnableTaskController _m_tcTickTaskController;
        //显示序列号
        private long _m_lShowSerialize;
        //奖励页面
        private GGUIWndGuildRankRushDetailRewardPage _m_wRewardPage;
        //排行榜页面
        private GGUIWndGuildRankRushDetailRankPage _m_wRankPage;
        //自己的排名
        ...
        public GGUIWndGuildRankRushDetail() : base(EALUIWndLayer.NORMAL)
        {
        }

        protected override _AALResourceCore _resourceCore { get { return GameResCore.instance; } }
        protected override string _monoAssetPath { get { return GGUIMonoGuildRankRushDetail.assetPath; } }
        protected override string _monoObjName { get { return GGUIMonoGuildRankRushDetail.objName; } }
        ...
        protected override void _onHideWnd()
        {
            ...
            _hideAllPage();
            if (_m_lTopGuildInfoList != null)
            {
                foreach (GGUIWndRankRushTopGuildInfo subTopPlayerInfo in _m_lTopGuildInfoList)
                {
        ...
        protected override void _onWndInitDone()
        {
            ...
            _m_lTopGuildInfoList = new List<GGUIWndRankRushTopGuildInfo>();
            if (wnd.monoTopPlayerList != null)
            {
                for (int i = 0; i < wnd.monoTopPlayerList.Count; i++)
                {
                    _m_lTopGuildInfoList.Add(new GGUIWndRankRushTopGuildInfo(wnd.monoTopPlayerList[i]));
                }
            }
        ...
        //刷新前几名玩家
        private void _refreshTopGuild()
        {
            if (wnd == null || _m_rankRushInfo == null || _m_rankRushInfo.activityInfo == null || _m_rankRushInfo.activityRankRushRefObj == null || _m_lTopGuildInfoList == null)
                return;

            //先重置显示
            for (int i = 0; i < _m_lTopGuildInfoList.Count; i++)
            {
                _m_lTopGuildInfoList[i]?.showWnd();
                _m_lTopGuildInfoList[i]?.setInfo(null, 0);
            }
                ...
                NPPlayer.instance.commonActivityComp.reqActivityRankBaseList(_m_rankRushInfo.activityInfo.instanceId, _m_rankRushInfo.activityRankRushRefObj.rank_id, _m_rankRushInfo.activityInfo.isCross,
                    _msg =>
                    {
                        ...
                        if (_m_lRankShowInfoList == null)
                            _m_lRankShowInfoList = new List<GuildRankInfo>();
                        _m_lRankShowInfoList.Clear();

                        for (int i = 0; i < _msg.getBaseItemlist().Count; i++)
                        {
                            GuildRankInfo rankCommonShowInfo = new GuildRankInfo(_msg.getBaseItemlist()[i], 0, _m_rankRushInfo.activityInfo.isCross);
                            rankCommonShowInfo.setRankId(_m_rankRushInfo.activityRankRushRefObj.rank_id);
                            _m_lRankShowInfoList.Add(rankCommonShowInfo);
                            _m_lRankShowInfoList.Sort((_a,_b)=>_a.rankSortOrder.CompareTo(_b.rankSortOrder));
                        }

                        for (int i = 0; i < _m_lTopGuildInfoList.Count; i++)
                        {
                            _m_lTopGuildInfoList[i]?.showWnd();
                            _m_lTopGuildInfoList[i]?.setInfo(_m_lRankShowInfoList.Count > i ? _m_lRankShowInfoList[i] : null, _m_rankRushInfo.activityInfo.instanceId);
                        }
        ...
        {
            if (wnd == null || _m_rankRushInfo == null || _m_rankRushInfo.activityInfo == null || _m_rankRushInfo.activityRankRushRefObj == null || _m_lTopGuildInfoList == null || _m_rankRushInfo.rankRefObj == null)
                return;
            ...
            ALUGUICommon.setGameObjEnable(wnd.goNoInititalValueHideList, _m_rankRushInfo.activityRankRushRefObj.haveMaxRecordValue && NPPlayer.instance.guildComp.isJoinGuild());
            ...
            if (NPPlayer.instance.guildComp.isJoinGuild())
            {
                ...
                switch (_m_rankRushInfo.activityInfo.activityState)
                {
                    ...
                    case EActivityState.SETTLING:
                        //请求自己的排名信息
                        NPPlayer.instance.commonActivityComp.reqActivityRankBaseInfoByKey(_m_rankRushInfo.activityInfo.instanceId, _m_rankRushInfo.activityRankRushRefObj.rank_id, NPPlayer.instance.guildComp.guildInfo?.guildId ?? 0, _m_rankRushInfo.activityInfo.isCross,
                            _msg =>
                            {
                                if (_msg == null || _msg.getBaseItem() == null || wnd == null || !isShow || _m_lShowSerialize != curSerialize)
                                    return;

                                _setSelfShowInfo(_msg.getBaseItem().getRank(), _msg.getBaseItem().getScore());
                            });
                        //请求自己的结算信息
                        _setSettleInfo(0, false);
                        break;
                    case EActivityState.REWARDING:
                        ...
                        {
                            if (wnd == null || !isShow || _m_lShowSerialize != curSerialize)
                                return;

                            if(_settleInfo != null)
                                _setSettleInfo(_settleInfo.getRank(), _settleInfo.getHadDraw());
                            else
                                _setSettleInfo(0, false);
                        }, true);
                        break;
        ...
        private void _setSelfShowInfo(long _selfRank, long _score)
        {
            ...
            if (!NPPlayer.instance.guildComp.isJoinGuild())
            {
                ALUGUICommon.setLabelTxt(wnd.txtMyRank, TextTranslate.instance.getLanguage(TransKeyConst.rankRush_myRank_str, TransKeyConst.rankRush_notJoinGuild_none));
                ALUGUICommon.setLabelTxt(wnd.txtMyScore, "");
            }
            else
            {
                //设置排名
            ...
            //设置冲榜初始值
            if (_m_rankRushInfo.activityRankRushRefObj.haveMaxRecordValue && NPPlayer.instance.guildComp.isJoinGuild())
            {
        ...
        //设置按钮结算领奖状态
        private void _setSettleInfo(long _selfRank, bool _hadDraw)
        {
            if (wnd == null || _m_rankRushInfo == null || _m_rankRushInfo.activityInfo == null)
                return;
        ...
        private void _showRewardPage()
        {
            ...
            {
                _m_wRewardPage = new GGUIWndGuildRankRushDetailRewardPage(_getPageAssetPathByType(ERankRushDetailTabType.REWARD), wnd.pageParent);
                _m_wRewardPage.load(() =>
                {
        ...
        private void _showRankPage()
        {
            ...
            if (_m_wRankPage != null)
            {
                ...
                _m_wRankPage.setInfo(_m_rankRushInfo.activityRankRushRefObj, _m_lRankShowInfoList, _m_rankRushInfo.activityInfo.instanceId);
            }
            else
            {
                _m_wRankPage = new GGUIWndGuildRankRushDetailRankPage(_getPageAssetPathByType(ERankRushDetailTabType.RANK), wnd.pageParent);
                _m_wRankPage.load(() =>
                {
        ...
        private void _onClickGetReward(GameObject _go)
        {
            ...
            if (!NPPlayer.instance.guildComp.isJoinGuild())
            {
                NPGUIAddSceneCenterTip.instance.showTransTextInfo(TransKeyConst.rankRush_notJoinGuild_none);
                return;
            }
        ...
        {
            QueueMgr.instance.forceCloseNodeByTag(UINodeTagConst.C_GUILD_RANK_RUSH_DETAIL);
        }
