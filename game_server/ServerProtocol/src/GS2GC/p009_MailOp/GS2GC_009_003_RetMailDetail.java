package GS2GC.p009_MailOp;

import java.nio.ByteBuffer;
public class GS2GC_009_003_RetMailDetail implements ALBasicProtocolPack._IALProtocolStructure {
/** 邮件详情 */
private Common.MailObj.Mail_DetailInfo mailDetail;


public GS2GC_009_003_RetMailDetail() {
	mailDetail = new Common.MailObj.Mail_DetailInfo();
}

public GS2GC_009_003_RetMailDetail(
	 Common.MailObj.Mail_DetailInfo _mailDetail
) {	mailDetail = _mailDetail;
}

public final byte getMainOrder() { return (byte)9; }

public final byte getSubOrder() { return (byte)3; }

/** 邮件详情 */
public Common.MailObj.Mail_DetailInfo getMailDetail() { return mailDetail; }
/** 邮件详情 */
public void setMailDetail(Common.MailObj.Mail_DetailInfo _mailDetail) { mailDetail = _mailDetail; }


public final int GetBufSize() {
	int _size = 0;
	_size += 4 + mailDetail.GetBufSize();

	return _size;
}

public final int GetFullPackBufSize() {
	int _size = 2;
	_size += 4 + mailDetail.GetBufSize();

	return _size;
}



public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
	if(_buf.remaining() <= 0) return;
	int _mailDetailCustLen = _buf.getInt();
	int _mailDetailCurPos = _buf.position();
	mailDetail.ReadUnzipBuf(_buf, _mailDetailCurPos + _mailDetailCustLen);
	_buf.position(_mailDetailCurPos + _mailDetailCustLen);

}

public final void PutUnzipBuf(ByteBuffer _buf) {
	_buf.putInt(mailDetail.GetBufSize());
	mailDetail.PutUnzipBuf(_buf);
}

public final ByteBuffer makeFullPackage() {
	int _bufSize = GetBufSize() + 2;
	ByteBuffer _buf = ByteBuffer.allocate(_bufSize);
	_buf.put((byte)9);
	_buf.put((byte)3);
	PutUnzipBuf(_buf);
	_buf.flip();
	return _buf;
}
public final void makeFullPackage(ByteBuffer _recBuf) {
	if(null == _recBuf)
		return ;
	_recBuf.put((byte)9);
	_recBuf.put((byte)3);
	PutUnzipBuf(_recBuf);
}
public final ByteBuffer makePackage() {
	int _bufSize = GetBufSize();
	ByteBuffer _buf = ByteBuffer.allocate(_bufSize);
	PutUnzipBuf(_buf);
	_buf.flip();
	return _buf;
}
public final void readPackage(ByteBuffer _buf) {
	ReadUnzipBuf(_buf, -1);
}
}

