...
**配置参数**：
- `mars_building_oxygen_coefficient` - 氧气系数
- `mars_building_satiety_coefficient` - 饱腹系数
- `mars_building_sleep_coefficient` - 睡眠系数
...
**配置参数**：
- `mars_building_oxygen_yield_per_consume` - 每人口消耗氧气值
- `mars_building_oxygen_index_max` - 氧气指数上限
...
**配置参数**：
- `mars_building_comfort_coefficient` - 舒适系数
- `mars_building_mood_coefficient` - 心情系数
...
stateDiagram-v2
    [*] --> 未到达火星
    未到达火星 --> 阶段完成待确认: 阶段时间到
    阶段完成待确认 --> 未到达火星: 确认进入下一阶段
    未到达火星 --> 已到达火星: 完成所有阶段
    已到达火星 --> 待选择区域: 功能解锁
    待选择区域 --> 无红点: 选择区域完成
    无红点 --> [*]

    note right of 阶段完成待确认
        红点数 = 1
        RED_MARS_GO_TO_NEXT
    end note
...
| 配置类型 | 文件路径 | 加载方式 |
|----------|----------|----------|
| 航程配置 | `refdata/mars_refdata.unity3d` | 资源加载 |
| 通用配置 | `NPGeneral.cs` | 代码定义 |
...
// 获取通用配置
var npGeneral = GRefdataCoreMgr.instance.npGeneral;
int oxygenCoeff = npGeneral.mars_building_oxygen_coefficient;
```
