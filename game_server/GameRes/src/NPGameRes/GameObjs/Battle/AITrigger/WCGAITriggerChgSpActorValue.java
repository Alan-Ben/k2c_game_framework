...
/// <summary>
/// AI 触发效果： 修改ai中对应索引的值(idx索引暂定0-99)
/// </summary>
public class WCGAITriggerChgSpActorValue extends _AWCGBasicAITrigger
{
    ...
    {
        return EWCGAITriggerType.CHG_SP_ACTOR_V;
    }

    public static WCGAITriggerChgSpActorValue read(String _infoStr)
    {
        WCGAITriggerChgSpActorValue obj = new WCGAITriggerChgSpActorValue();
        String[] strs = CommonFunc.charSplit(_infoStr, ':');
        if (strs.length < 3)
        {
            CommLog.error("ai CHG_SP_ACTOR_V配置错误 正确配置: 枚举:下标索引:值");
            return null;
        }
        try
        {
            obj._m_lActorId = Long.parseLong(strs[0].trim());
            obj._m_iIndex = Integer.parseInt(strs[1].trim());
            obj._m_iValue = Integer.parseInt(strs[2].trim());
            if (strs.length > 3)
                obj._m_sValueVariable = WCGVariableGroupObj.readVariableGroup(strs[2], "AI Trigger　Chg　Index高级公式错误：　");
            return obj;
        } catch (Exception e)
        {
            CommLog.error("ai CHG_SP_ACTOR_V配置错误 正确配置: 枚举:下标索引:值", e);
            return null;
        }
    }
}