...
namespace Hotfix
{
    public abstract class _AHotfixCommonAssetCacheControllerMgr<T, TEMP, T_Cache>
        where T_Cache : _AHotfixCacheController<T, TEMP>
    {
        // 每个index的缓存池集合
        [NotNull] protected Dictionary<int, T_Cache> _m_CacheMap = new Dictionary<int, T_Cache>();
        //正在加载的cache回调管理数据集
        [NotNull] protected Dictionary<int, Action<T_Cache>> _m_dLoadingCacheDic = new Dictionary<int, Action<T_Cache>>();
        //是否已经销毁
        private bool _m_hasDiscard = false;
        //缓存池父节点
        protected GameObject _m_RootGo;
        
        public _AHotfixCommonAssetCacheControllerMgr(string _rootFolderName)
        {            
        ...
        public _AHotfixCommonAssetCacheControllerMgr(GameObject _cacheParent)
        {
...
#if UNITY_EDITOR
                Debug.LogWarning($"_AHotfixCommonAssetCacheControllerMgr 释放后还有往里push，建议确认一下:{_index}");
#endif
                return;
            }
