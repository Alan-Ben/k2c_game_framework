...
/// </summary>
public class WCGAITriggerSetBattleValue extends _AWCGBasicAITrigger
{
    ...
    {
        return EWCGAITriggerType.SET_BATTLE_V;
    }

    public static WCGAITriggerSetBattleValue read(String _infoStr)
    {
        WCGAITriggerSetBattleValue obj = new WCGAITriggerSetBattleValue();
        String[] strs = CommonFunc.charSplit(_infoStr, ':');
        if (strs.length < 2)
        {
            CommLog.error("ai SET_BATTLE_V配置错误 正确配置: 枚举:下标索引:值");
            return null;
        }
        try
        {
        ...
        {
            CommLog.error("ai SET_BATTLE_V配置错误 正确配置: 枚举:下标索引:值", e);
            return null;
        }
    }
}