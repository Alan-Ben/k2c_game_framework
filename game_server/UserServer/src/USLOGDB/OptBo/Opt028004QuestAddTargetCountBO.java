...
public class Opt028004QuestAddTargetCountBO extends BaseOptLogBo {
    ...
    public static final int FIELD_target =9;
    @DataBaseField(type = "bigint(20)", fieldname = "target", comment = "目标ID")
    private long target;

    public static final int FIELD_chg_count =10;
    @DataBaseField(type = "int(11)", fieldname = "chg_count", comment = "变化数量")
    private int chg_count;

    public Opt028004QuestAddTargetCountBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt028004QuestAddTargetCountBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        step = rs.getLong(10);
        target = rs.getLong(11);
        chg_count = rs.getInt(12);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt028004QuestAddTargetCountBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `quest_id`, `step`, `target`, `chg_count`";
    }

    @Override
    public String getTableName() {
        return "`opt_028_004_quest_add_target_count`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(chg_count).append("', ");
   ...
   @Override
    ...
    // 变化数量
    public int getChgCount() { return this.chg_count; }
    public void setChgCount(BM _bm, int chg_count) {
        if(chg_count==this.chg_count) 
            return;
        this.chg_count = chg_count; 
        markField(_bm, FIELD_chg_count); 
    }
    public void saveChgCount(BM _bm, int chg_count) {
        if(chg_count==this.chg_count) 
            return;
        this.chg_count = chg_count;
        saveField(_bm, "chg_count", chg_count);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `chg_count` = '").append(chg_count).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_chg_count)) sBuilder.append(" `chg_count` = '").append(chg_count).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_028_004_quest_add_target_count` ("
                ...
                + "`chg_count` int(11) NOT NULL DEFAULT '0' COMMENT '变化数量',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='任务-玩家增加任务目标计数' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
