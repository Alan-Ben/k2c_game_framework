...
namespace GOE
{
    //红点组mono
    [System.Serializable]
    public class NPGGUIRedTipGroupMono_Priority
    {
        [ALHeader("红点列表，这边配置的多个红点会按列表顺序按优先级显示")]
        public List<NPGGUIRedTipTypeMono> redTipList;
        [ALHeader("红点列表都不满足的时候控制显示的GO")]
        public GameObject[] goDefaultShow;
        [ALHeader("红点列表都不满足的时候控制隐藏的GO")]
        public GameObject[] goDefaultHide;
    ...
    /// </summary>
    public class NPGGUICustomMonoRedTipWnd_Priority : MonoBehaviour
    {
        ...
        [ALHeader("红点组列表")]
        public List<NPGGUIRedTipGroupMono_Priority> redGroupMonoList;
        ...
        private void Awake()
        {
            ...
            {
                foreach (NPGGUIRedTipGroupMono_Priority npgguiRedTipGroupMono in redGroupMonoList)
                {
            ...
            if(gameObject.activeInHierarchy)
            {
                ...
                foreach (NPGGUIRedTipGroupMono_Priority npgguiRedTipGroupMono in redGroupMonoList)
                {
                    ...
                    if(npgguiRedTipGroupMono.redTipList != null)
                    {
                        ...
                        if (!thisGroupHasShow)
                        {
                            ALUGUICommon.setGameObjEnable(npgguiRedTipGroupMono.goDefaultShow, true);
                            ALUGUICommon.setGameObjEnable(npgguiRedTipGroupMono.goDefaultHide, false);
                        }
                    }
                }
#endif
