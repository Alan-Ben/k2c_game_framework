...
import Hotfix.V02.GC2GS.p202_NumMergeOp.GC2GS_202_006_ReqNumMergeUseEliminateItem;
 ...
 * 5. 返回更新后的棋盘数据
 */
public class MsgDealer_GC2GS_202_006_ReqNumMergeUseEliminateItem extends NPUserMsgDealer<GC2GS_202_006_ReqNumMergeUseEliminateItem>
{
    @Override
    protected void _dealMessage(_ANPUSUserBasicMsgItem _committer, GC2GS_202_006_ReqNumMergeUseEliminateItem _msg)
    {
        // 获取玩家数据
        ...
        // 创建操作上下文
        NPPlayerContext context = NPPlayerContext.createNew(ENumMerge_GameEvent.NUM_MERGE_USE_ELIMINATE_ITEM.value());

        // 执行使用消除道具操作
        Result result = playerInfo.useEliminateItem(userData, _msg.getBlockIndex(), context);
        if (!result.isSucc())
        {
        ...
        // 返回成功响应
        _committer.commitSucRes(US2GCWriter_202_NumMergeOp.make_006_RetNumMergeUseEliminateItem());
    }
}
