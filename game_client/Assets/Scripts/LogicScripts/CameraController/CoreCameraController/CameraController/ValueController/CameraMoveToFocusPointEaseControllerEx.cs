...
{
    /// <summary>
    /// 相较于CameraMoveToFocusPointEaseController, 在updateCameraPos更新时多做了一个判断, 当当前位置非常靠近目标位置时, 直接返回目标位置
    /// </summary>
    public class CameraMoveToFocusPointEaseControllerEx : _ACameraMoveController
    {
        ...
        public CameraMoveToFocusPointEaseControllerEx(Vector3 _targetPos, ESceneMoveType _moveType, EaseType _easeType, float _totalTime, Action _complete = null)
        {
        ...
        public CameraMoveToFocusPointEaseControllerEx(Vector3 _targetPos, ESceneMoveType _moveType, float _totalTime, Action _complete = null)
        {
            _m_targetPos = _targetPos;
            _m_moveType = _moveType;
        ...
        public override Vector3 updateCameraPos(_APosLimiter _posLimiter, _APosLimiter _focusPosLimiter)
        {
            ...
            Vector3 currentPos = Vector3.Lerp(_m_startPos, _m_finalTargetPos, _m_easeType.evaluate(_m_currentTime, _m_totalTime));
            if(Mathf.Approximately(Vector3.Distance(currentPos, _m_finalTargetPos), 0f))
            {
                if (!_m_isDone)
                {
                    _m_isDone = true;
