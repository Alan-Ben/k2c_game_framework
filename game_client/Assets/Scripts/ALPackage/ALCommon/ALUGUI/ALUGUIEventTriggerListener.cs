 ...
 * ugui中的事件监听对象
 **/
//public class ALUGUIEventTriggerListener : UnityEngine.EventSystems.EventTrigger
public class ALUGUIEventTriggerListener : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler, IMoveHandler
{
    //上次点击帧数
    private static int _g_lastClickFrame;
    //重置当前点击判断帧数
    public static void g_resetLastClickFrame() { _g_lastClickFrame = -1; }
    ...
    private int _m_iOneClickPerFrame = ALConst.CLICK_INTERVAL_FRAME;//是否每帧只触发一次点击
    ...
    private void Update ()
    {
        ...
        if (_m_bIsPointerDown && !_m_bIsMove && !_m_bLongPressTriggered) 
        {
            if (Time.unscaledTime >= _m_fLongPressTriggerTime)
            {
                ...
                {
                    UnityEngine.Debug.LogError("ALUGUIEventTriggerListener OnLongPress has Exception:\n" + e);
    ...
    public static ALUGUIEventTriggerListener Get(GameObject _go, bool _triggerEventAgain = false, int _oneClickPerFrame = ALConst.CLICK_INTERVAL_FRAME)
    {
        if (null == _go)
            return null;

        ALUGUIEventTriggerListener listener = _go.GetComponent<ALUGUIEventTriggerListener>();
        if (null == listener)
            listener = _go.AddComponent<ALUGUIEventTriggerListener>();
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        {
            //判断等级
            ALUGUIClickVersionJudgeMono versionJudge = GetComponent<ALUGUIClickVersionJudgeMono>();
            if(versionJudge != null)
            {
                if(versionJudge.needMinVersionNum > 0 && versionJudge.needMinVersionNum > _AALMonoMain.instance.curVersionNum)
                {
                    _AALMonoMain.instance.onClickVersionLowerErr();
                    return;
                }
                if(versionJudge.needMinServerVersionNum > 0 && versionJudge.needMinServerVersionNum > _AALMonoMain.instance.curServerVersionNum)
                {
                    _AALMonoMain.instance.onClickServerVersionLowerErr();
                    return;
                }
#if AL_ILRUNTIME
                if(versionJudge.needMinHotfixVersionNum > 0 && versionJudge.needMinHotfixVersionNum > ALHotfixMgr_ILRuntime_Global.instance.version.curVersionNum)
                {
                    _AALMonoMain.instance.onClickHotfixVersionLowerErr();
                    return;
                }
#endif
            }
            ...
            //发送点击
            ALMsgSys.SendMsg((int) ALMsgType.UI_CLICK, gameObject);
//        WinMsg.SendMsg(WinMsgType.UI_CLICK, gameObject);
        ...
        {
            UnityEngine.Debug.LogError("ALUGUIEventTriggerListener OnPointerClick has Exception:\n" + e);
            if(_AALMonoMain.instance != null)
            {
                _AALMonoMain.instance.onUnknowErrorOccurred(e);
            }
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        {
            UnityEngine.Debug.LogError("ALUGUIEventTriggerListener OnPointerDown has Exception:\n" + e);
            if(_AALMonoMain.instance != null)
            {
                _AALMonoMain.instance.onUnknowErrorOccurred(e);
            }
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        {
            UnityEngine.Debug.LogError("ALUGUIEventTriggerListener OnPointerUp has Exception:\n" + e);
            if(_AALMonoMain.instance != null)
            {
                _AALMonoMain.instance.onUnknowErrorOccurred(e);
            }
