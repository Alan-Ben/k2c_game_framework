...
public class ShareCodeServerConfMgr
{
    private static ShareCodeServerConfMgr g_instance = new ShareCodeServerConfMgr();

    public static ShareCodeServerConfMgr getInstance()
    {
        return g_instance;
    }

    private List<ShareCodeServerConf> m_confList = new ArrayList<>();
    /*******************
     * 初始化函数
     *
     * @param _properties
    ...
    public static final String defualtFilePath = "./conf/ShareCodeServerListConf.json";
    public static final String customFilePath = "./customConf/ShareCodeServerListConf.json";
    ...
    private boolean initCore()
    {
        ...
        List<ShareCodeServerConf> defaultConfList = new ArrayList<>();
        ...
        {
            List<ShareCodeServerConf> customConfList = new ArrayList<>();
     ...
     * 初始化单个配置文件的处理函数
     */
    private boolean _initConfList(List<ShareCodeServerConf> _confList, String txt)
    {
        _confList.clear();
        ...
        for (JsonElement confElement : arrayRoot)
        {
            ...
            ShareCodeServerConf conf = parseConf(confElement.getAsJsonObject());
            if (null == conf)
            {
    ...
    private boolean addConf(List<ShareCodeServerConf> _confList, ShareCodeServerConf _conf)
    {
        for (ShareCodeServerConf conf : _confList)
        {
     ...
     * @return
     */
    private ShareCodeServerConf parseConf(JsonObject _confObj)
    {
        if (null == _confObj)
            return null;
        ...
        return new ShareCodeServerConf(id
                , RecBufferLen
                , Port
                , PlatServerIp
                , PlatServerPort
    ...
    public String toString()
    {
        ...
        sb.append(String.format("Total %d ShareCodeServer:\n", m_confList.size()));
        for (int i = 0; i < m_confList.size(); i++)
        {
            ShareCodeServerConf conf = m_confList.get(i);
    ...
    public List<ShareCodeServerConf> getConfList()
    {
