...
public class LogExpiredItemBO extends BaseLogBo {
    ...
    public static final int FIELD_item_type =6;
    @DataBaseField(type = "int(11)", fieldname = "item_type", comment = "物品类型")
    private int item_type;

    public static final int FIELD_sub_id =7;
    @DataBaseField(type = "bigint(20)", fieldname = "sub_id", comment = "物品配置ID")
    private long sub_id;

    public static final int FIELD_dbid =8;
    @DataBaseField(type = "bigint(20)", fieldname = "dbid", comment = "物品数据ID")
    private long dbid;

    public static final int FIELD_expired_ts =9;
    @DataBaseField(type = "int(11)", fieldname = "expired_ts", comment = "过期时间")
    private int expired_ts;

    public LogExpiredItemBO() {
        id = 0;
        event_id = 0;
        guid = 0L;
        date_time = 0;
    ...
    public LogExpiredItemBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        ...
        cid = rs.getLong(6);
        logType = rs.getInt(7);
        item_type = rs.getInt(8);
        sub_id = rs.getLong(9);
        dbid = rs.getLong(10);
        expired_ts = rs.getInt(11);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new LogExpiredItemBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `event_id`, `guid`, `date_time`, `timestamp`, `cid`, `logType`, `item_type`, `sub_id`, `dbid`, `expired_ts`";
    }

    @Override
    public String getTableName() {
        return "`log_expired_item`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(dbid).append("', ");
   ...
   @Override
    ...
    public void setDbid(BM _bm, long dbid) {
        if(dbid==this.dbid) 
            return;
        this.dbid = dbid; 
        markField(_bm, FIELD_dbid); 
    }
    public void saveDbid(BM _bm, long dbid) {
        if(dbid==this.dbid) 
            return;
        this.dbid = dbid;
        saveField(_bm, "dbid", dbid);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `dbid` = '").append(dbid).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_dbid)) sBuilder.append(" `dbid` = '").append(dbid).append("',");
        if(isFieldMarked(FIELD_expired_ts)) sBuilder.append(" `expired_ts` = '").append(expired_ts).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `log_expired_item` ("
                ...
                + "`sub_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '物品配置ID',"
                + "`dbid` bigint(20) NOT NULL DEFAULT '0' COMMENT '物品数据ID',"
                + "`expired_ts` int(11) NOT NULL DEFAULT '0' COMMENT '过期时间',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='时效物品数据日志表' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
