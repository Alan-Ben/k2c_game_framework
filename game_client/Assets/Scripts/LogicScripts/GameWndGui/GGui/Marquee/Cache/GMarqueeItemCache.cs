...
namespace GOE
{
    ...
    /// 跑马灯item缓存池
    /// </summary>
    public class GMarqueeItemCache: _AALLoadCacheController<GGUIWndSubMarqueeItem, GGUIMonoSubMarqueeItem>
    {
        private long _m_lUiResId;
        private Transform _m_cacheRoot;

        public GMarqueeItemCache(long _uiResId, Transform _cacheRoot, int _minCount, int _maxCount) : base(_minCount, _maxCount)
        {
            _m_lUiResId = _uiResId;
            _m_cacheRoot = _cacheRoot;
        }

        public GMarqueeItemCache(long _uiResId, Transform _cacheRoot, int _minCount, int _maxCount, int _addUnit) : base(_minCount, _maxCount, _addUnit)
        {
            _m_lUiResId = _uiResId;
            _m_cacheRoot = _cacheRoot;
        }
        ...
        {
            get { return $"GMarqueeItemCache uiResId:{_m_lUiResId}"; }
        }
        ...
        /// </remarks>
        public GGUIMonoSubMarqueeItem template { get { return _m_tTemplateObj; } }

        protected override void _onInit(GGUIMonoSubMarqueeItem _template)
        {
        }

        protected override GGUIWndSubMarqueeItem _createItem(GGUIMonoSubMarqueeItem _template)
        {
            if (_template == null)
                return null;

            // 使用模板实例化一个新的
            GGUIMonoSubMarqueeItem wnd = Object.Instantiate(_template);
            ...
            // 返回创建的对象
            return new GGUIWndSubMarqueeItem(wnd);

        }

        protected override void _discardItem(GGUIWndSubMarqueeItem _item)
        {
            if (null == _item)
                return;
        ...
        protected override void _resetItem(GGUIWndSubMarqueeItem _item)
        {
        ...
        protected override void _loadTemplate(Action<GGUIMonoSubMarqueeItem> _loaded)
        {
            if (_loaded == null)
                return;
            ...
            void _loadGoDelegate(GameObject _go)
            {
                ...
                GGUIMonoSubMarqueeItem mono = _go.GetComponent<GGUIMonoSubMarqueeItem>();
                if (mono == null)
                {
                    Debug.LogError($"Load GUI Asset: {assetPath}, {objectName}, res file load fail!");
                    _loaded.Invoke(null);
        ...
        protected override void _discardTemplate(GGUIMonoSubMarqueeItem _template)
        {
        }
    }
}