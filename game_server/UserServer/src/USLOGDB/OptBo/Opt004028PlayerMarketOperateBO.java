...
public class Opt004028PlayerMarketOperateBO extends BaseOptLogBo {
    ...
    public Opt004028PlayerMarketOperateBO() {
        id = 0;
        cid = 0L;
        level = 0;
        vip_lvl = 0;
    ...
    public Opt004028PlayerMarketOperateBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        level = rs.getInt(3);
        vip_lvl = rs.getInt(4);
        ...
        market_id = rs.getLong(9);
        op_count = rs.getInt(10);
        op_ms = rs.getLong(11);
    }
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new Opt004028PlayerMarketOperateBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `level`, `vip_lvl`, `event_id`, `guid`, `date_time`, `timestamp`, `market_id`, `op_count`, `op_ms`";
    }

    @Override
    public String getTableName() {
        return "`opt_004_028_player_market_operate`";
    }
   ...
   @Override
    ...
    public void setOpCount(BM _bm, int op_count) {
        if(op_count==this.op_count) 
            return;
        this.op_count = op_count; 
        markField(_bm, FIELD_op_count); 
    }
    public void saveOpCount(BM _bm, int op_count) {
        if(op_count==this.op_count) 
            return;
        this.op_count = op_count;
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `opt_004_028_player_market_operate` ("
                ...
                + "`op_count` int(11) NOT NULL DEFAULT '0' COMMENT '经营次数',"
                + "`op_ms` bigint(20) NOT NULL DEFAULT '0' COMMENT '经营时间',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家-经营市场' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
