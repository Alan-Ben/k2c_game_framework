...
| isChatLoggedIn | bool | 是否已登录聊天服务器 |
| currentRoomId | long | 当前所在房间ID |
...
| JoinRoom | roomId | void | 加入房间 |
| QuitRoom | roomId | void | 退出房间 |
| AddToBlackList | targetId | void | 添加黑名单 |
| RemoveFromBlackList | targetId | void | 移除黑名单 |
| GetUnreadCount | - | int | 获取未读消息数 |
| ClearUnreadCount | roomId | void | 清除未读消息 |

---

### 1.2 聊天信息类

#### ChatChannelInfo - 聊天频道数据
**路径**: `ClientProtocol/Common/ChatObj/ChatChannelInfo.cs`
...
| unreadCount | int | 未读消息数 |
| maxMsgCount | int | 最大消息数 |
| cdTime | int | 发言冷却时间 |
| isInRoom | bool | 是否在房间内 |

#### ChatMessage - 聊天消息
**路径**: `ClientProtocol/Common/ChatObj/ChatMessage.cs`
...
#### PrivateChatInfo - 私聊信息
**路径**: `ClientProtocol/Common/ChatObj/PrivateChatInfo.cs`

| 属性名 | 类型 | 说明 |
|--------|------|------|
| targetId | long | 对方玩家ID |
| targetName | string | 对方玩家名称 |
| targetIcon | long | 对方玩家头像 |
| messageList | List\\<PrivateMessage\\> | 消息列表 |
| lastMsgTime | long | 最后消息时间 |
| unreadCount | int | 未读消息数 |
| isOnline | bool | 对方是否在线 |

#### PrivateMessage - 私聊消息
**路径**: `ClientProtocol/Common/ChatObj/PrivateMessage.cs`
