...
| 字段名 | 类型 | 说明 | 必填 | 校验规则 |
|--------|------|------|------|----------|
| roomId | long | 房间ID | 是 | 必须大于0 |
| msgType | int | 消息类型 | 是 | 1-10 |
| content | string | 消息内容 | 是 | 1-200字符 |
| bubbleId | long | 气泡ID | 否 | 大于0 |
| extraData | object | 额外数据 | 否 | JSON格式 |

**请求示例**:
```csharp
GC2GS_022_006_ReqPlayerSendRoomMsgV2 req = new GC2GS_022_006_ReqPlayerSendRoomMsgV2();
req.setRoomId(100001);
req.setMsgType(1); // 文本消息
req.setContent("大家好，今天副本有人组队吗？");
req.setBubbleId(30001);
// 发送请求
```
...
| 错误码 | 常量名 | 说明 | 触发场景 |
|--------|--------|------|----------|
| 22001 | NOT_LOGGED_IN | 未登录聊天服务器 | 发送消息时 |
| 22002 | NOT_IN_ROOM | 未加入该聊天室 | 发送房间消息时 |
| 22003 | COOLDOWN_ACTIVE | 发言冷却中 | 发送消息时 |
| 22004 | MESSAGE_TOO_LONG | 消息过长 | 发送消息时 |
| 22005 | PLAYER_MUTED | 玩家被禁言 | 发送消息时 |
| 22006 | TARGET_NOT_FOUND | 目标玩家不存在 | 发送私聊时 |
| 22007 | BLOCKED_BY_TARGET | 被目标玩家屏蔽 | 发送私聊时 |
| 22008 | SENSITIVE_WORD | 包含敏感词 | 发送消息时 |
| 22009 | ROOM_NOT_FOUND | 频道不存在 | 加入频道时 |
| 22010 | BLACKLIST_FULL | 黑名单已满 | 添加黑名单时 |
| 22011 | INVALID_TOKEN | 无效的Token | 聊天登录时 |
| 22012 | PERMISSION_DENIED | 无权限 | 加入VIP频道时 |
| 22013 | ROOM_FULL | 房间已满 | 加入房间时 |
| 22014 | ALREADY_IN_ROOM | 已在房间内 | 加入房间时 |
| 22015 | INVALID_MESSAGE_TYPE | 无效的消息类型 | 发送消息时 |
...
flowchart TD
    ...
    D -->|成功| F{状态检查}
    F -->|禁言中| G[返回 PLAYER_MUTED]
    F -->|正常| H{冷却检查}
    H -->|冷却中| I[返回 COOLDOWN_ACTIVE]
    H -->|可发言| J{内容检查}
    J -->|超长| K[返回 MESSAGE_TOO_LONG]
    J -->|正常| L{敏感词检查}
    L -->|违规| M[拦截并记录]
    L -->|正常| N[发送成功]
```
