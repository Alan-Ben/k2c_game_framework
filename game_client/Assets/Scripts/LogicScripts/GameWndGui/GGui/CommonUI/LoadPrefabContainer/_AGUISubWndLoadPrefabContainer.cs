...
namespace GOE
{
    public abstract class _AGUISubWndLoadPrefabContainer<_T_ITEM_MONO, _T_CONTAINER_MONO, _T_ITEM_WND> : _ATALBasicUISubWnd<_T_CONTAINER_MONO>
        where _T_ITEM_MONO : _AALBasicUIWndMono
        where _T_CONTAINER_MONO : _AGUIMonoLoadPrefabContainer<_T_ITEM_MONO>
        where _T_ITEM_WND : _ATALBasicLoadPrefabSubUIWnd<_T_ITEM_MONO>
    {
        /** 存储子窗口对象的队列 */
        ...
        protected _AGUISubWndLoadPrefabContainer(_T_CONTAINER_MONO _containerMono)
            : base(_containerMono)
        {
        ...
        protected void _setItemCount(int _count)
        {
            clearAll();
            _T_ITEM_WND tempItemWnd = null;
            for (int i = 0; i < _count; ++i)
            {
                tempItemWnd = addItemWnd(_getItemUIPathId(i), i);
            }
        ...
        /// 获取对应item的资源id
        /// </summary>
        /// <param name="_itemDataIdx"></param>
        /// <returns></returns>
        protected abstract long _getItemUIPathId(int _itemDataIdx);
         ...
         * 添加一个子窗口到容器中
         **/
        public _T_ITEM_WND addItemWnd(long _uiPathId, int _itemIdx)
        {
            return addItemWnd(UIResPathAssistant.getAssetPath(_uiPathId), UIResPathAssistant.getObjName(_uiPathId), _itemIdx);
        }
        
        public _T_ITEM_WND addItemWnd(string assetPath, string objName, int _itemIdx)
        {
            //判断对应数据是否有效
            if(null == wnd || null == wnd.itemContainer)
                return null;

            //创建一个子窗口管理对象
            _T_ITEM_WND itemWnd = _createItemWnd(assetPath, objName, wnd.itemContainer.transform, _itemIdx);
            if(null == itemWnd)
            {
                return null;
            }
            ...
            itemWnd.load(() =>
            {
                ...
                //将子窗口添加到容器中
                itemWnd.wnd.transform.SetParent(wnd.itemContainer.transform);
                ...
                //在添加了窗口的时候调用的对应事件函数
                _onAddItemWnd(itemWnd, _itemIdx);
        ...
        * */
         ...
         * 创建本对象的脚本对象实例
         **/
        protected virtual _T_ITEM_MONO _instantiateItemMono (_T_ITEM_MONO _template) 
        {
            if(null == _template)
                return null;

            //实例化一个子窗口对象
            return GameObject.Instantiate(_template) as _T_ITEM_MONO;
        }

        /***************
         * 创建本对象的脚本对象实例
         **/
        protected virtual void _releaseItemMono (_T_ITEM_MONO _mono) 
        {
            if(null == _mono)
                return ;
        ...
        /*************
         * 根据带入的UI路径信息创建一个对应子窗口的管理对象
         **/
        protected abstract _T_ITEM_WND _createItemWnd(string assetPath, string objName, Transform _parent, int _itemIdx);

        /// <summary>
        /// 在添加了一个子窗口的时候调用的事件函数
        /// </summary>
        /// <param name="_itemWnd"></param>
        protected abstract void _onAddItemWnd(_T_ITEM_WND _itemWnd, int _itemDataIdx);
    }
}
