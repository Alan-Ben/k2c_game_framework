...
namespace ALPackage
{
    ...
    public class ALHttpSingleDownloadDealer
    {
        ...
        protected void _startDealDownlLoad()
        {
            ...
            {
                //开启下载线程
                ALThread downloadThread = new ALThread(_dealDownload);
        ...
        protected void _dealDownload()
        {
            ...
            if(isSuccess)
            {
                //读取对应的文件
                //处理下载成功操作
                ALCommonActionMonoTask.addMonoTask(_dealSuc);
            }
            else
            {
...
#endif
                    //再次开启加载
                    ALCommonActionMonoTask.addMonoTask(_retry);
                }
                else
                {
                    //不可重试则输出错误，并调用失败处理
...
#endif
                    ALCommonActionMonoTask.addMonoTask(() => { _dealFail(errStatus); });
                }
            }
        }
