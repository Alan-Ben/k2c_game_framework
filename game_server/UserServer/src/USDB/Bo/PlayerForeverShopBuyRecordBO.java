...
@RefBo(isIdAuto= true)
public class PlayerForeverShopBuyRecordBO extends BaseBO {
    ...
    @DataBaseField(type = "int(11)", fieldname = "buy_count", comment = "终身购买次数")
    private int buy_count;

    public PlayerForeverShopBuyRecordBO() {
        id = 0;
        cid = 0L;
        shop_ref_id = 0L;
        shop_item_ref_id = 0L;
    ...
    public PlayerForeverShopBuyRecordBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new PlayerForeverShopBuyRecordBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `shop_ref_id`, `shop_item_ref_id`, `buy_count`";
    }

    @Override
    public String getTableName() {
        return "`player_forever_shop_buy_record`";
    }
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_forever_shop_buy_record` ("
                ...
                + "`buy_count` int(11) NOT NULL DEFAULT '0' COMMENT '终身购买次数',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='玩家终身限购商品购买记录' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
