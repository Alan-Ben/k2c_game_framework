...
public class MjArtifactRecycleLogBO extends MJEventLogBo {
    ...
    public static final int FIELD_recycle_num =7;
    @DataBaseField(type = "int(11)", fieldname = "recycle_num", comment = "本次重塑的藏品数量")
    private int recycle_num;

    public static final int FIELD_recycle_dict =8;
    @DataBaseField(type = "text", fieldname = "recycle_dict", comment = "重塑明细JSON")
    private String recycle_dict;

    public static final int FIELD_timestamp =9;
    @DataBaseField(type = "int(11)", fieldname = "timestamp", comment = "事件发生时间戳(10位)")
    private int timestamp;

    public MjArtifactRecycleLogBO() {
        id = 0;
        cid = 0L;
        uid = "";
        vip_lv = 0;
    ...
    public MjArtifactRecycleLogBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        uid = rs.getString(3);
        vip_lv = rs.getInt(4);
        ...
        recycle_num = rs.getInt(9);
        recycle_dict = rs.getString(10);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new MjArtifactRecycleLogBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `uid`, `vip_lv`, `server_id`, `platform`, `region`, `create_time`, `recycle_num`, `recycle_dict`, `timestamp`";
    }

    @Override
    public String getTableName() {
        return "`mj_artifact_recycle_log`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(recycle_num).append("', ");
        strBuf.append("'").append(recycle_dict == null ? null : recycle_dict.replace("'","''").replace("\\","\\\\")).append("', ");
   ...
   @Override
    ...
    // 本次重塑的藏品数量
    public int getRecycleNum() { return this.recycle_num; }
    public void setRecycleNum(BM _bm, int recycle_num) {
        if(recycle_num==this.recycle_num) 
            return;
        this.recycle_num = recycle_num; 
        markField(_bm, FIELD_recycle_num); 
    }
    public void saveRecycleNum(BM _bm, int recycle_num) {
        if(recycle_num==this.recycle_num) 
            return;
        this.recycle_num = recycle_num;
        saveField(_bm, "recycle_num", recycle_num);
    }

    // 重塑明细JSON
    public String getRecycleDict() { return this.recycle_dict; }
    public void setRecycleDict(BM _bm, String recycle_dict) {
        if(recycle_dict.equals(this.recycle_dict)) 
            return;
        this.recycle_dict = recycle_dict; 
        markField(_bm, FIELD_recycle_dict); 
    }
    public void saveRecycleDict(BM _bm, String recycle_dict) {
        if(recycle_dict.equals(this.recycle_dict)) 
            return;
        this.recycle_dict = recycle_dict;
        saveField(_bm, "recycle_dict", recycle_dict);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `recycle_num` = '").append(recycle_num).append("',");
        sBuilder.append(" `recycle_dict` = '").append(recycle_dict == null ? null : recycle_dict.replace("'","''").replace("\\","\\\\")).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_recycle_num)) sBuilder.append(" `recycle_num` = '").append(recycle_num).append("',");
        if(isFieldMarked(FIELD_recycle_dict)) sBuilder.append(" `recycle_dict` = '").append(recycle_dict == null ? null : recycle_dict.replace("'","''").replace("\\","\\\\")).append("',");
        if(isFieldMarked(FIELD_timestamp)) sBuilder.append(" `timestamp` = '").append(timestamp).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `mj_artifact_recycle_log` ("
                ...
                + "`recycle_num` int(11) NOT NULL DEFAULT '0' COMMENT '本次重塑的藏品数量',"
                + "`recycle_dict` text NULL COMMENT '重塑明细JSON',"
                + "`timestamp` int(11) NOT NULL DEFAULT '0' COMMENT '事件发生时间戳(10位)',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='梦加日志-藏品重塑记录' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
    ...
    private int getBufferSize()
    {
        ...
        _size+=4;//create_time
        _size+=4;//recycle_num
        _size += ALBasicProtocolPack.ALProtocolCommon.GetStringBufSize(recycle_dict);//recycle_dict
        _size+=4;//timestamp
         return _size;
    }
    ...
    public ByteBuffer toByteBuffer()
    {
        ...
        buff.putInt(recycle_num);
        ALBasicProtocolPack.ALProtocolCommon.PutStringIntoBuf(buff, recycle_dict);
    ...
    protected void readFromByteBuffer(ByteBuffer buff)
    {
        ...
        recycle_num=buff.getInt();
        recycle_dict=ALBasicProtocolPack.ALProtocolCommon.GetStringFromBuf(buff);
