...
/// <summary>
/// 一个以传入的坐标为原点，世界坐标下 XY 方向为 xy 方向的 2d 平面
/// </summary>
public class LogicPlane2DPosGetterXY : _ALogicPlane2DPosGetter
{
    private readonly Vector3 _m_originPos;
    
    public LogicPlane2DPosGetterXY(Vector3 _originPos)
    {
    ...
    {
        return new Vector3(_logicPos.x + _m_originPos.x, _logicPos.y + _m_originPos.y, _m_originPos.z);
    }
    
    public override bool intersectionWorldPointWithWorldRay(Ray _ray, out Vector3 _intersectionPos)
    {
        return NPGameUtility.rayToPanelCollision(_ray.origin, _ray.direction, _m_originPos, new Vector3(0, 0, 1), out _intersectionPos, out float t) && t >= 0;
    }
}
