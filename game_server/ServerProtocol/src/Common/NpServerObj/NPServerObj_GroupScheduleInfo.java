 ...
 * 分组排期信息
 **/
public class NPServerObj_GroupScheduleInfo implements ALBasicProtocolPack._IALProtocolStructure {
/** 排期id */
private long scheduleId;
/** 涉及服务器跨服分组id列表 */
private java.util.ArrayList<Long> crossGroupIdList;
/** 排期预推送时间,开始推送到US的时间 */
...
public NPServerObj_GroupScheduleInfo() {
	scheduleId = (long)0;
	crossGroupIdList = new java.util.ArrayList<Long>();
...
public NPServerObj_GroupScheduleInfo(
	 long _scheduleId
	, java.util.ArrayList<Long> _crossGroupIdList
	, long _prePushTimeMs
	, long _startTimeMs
	, long _stopTimeMs
	...
	crossGroupIdList = _crossGroupIdList;
...
/** 涉及服务器跨服分组id列表 */
public java.util.ArrayList<Long> getCrossGroupIdList() { return crossGroupIdList; }
/** 涉及服务器跨服分组id列表 */
public void addCrossGroupIdList(long _crossGroupIdList) { crossGroupIdList.add(_crossGroupIdList); }
/** 排期预推送时间,开始推送到US的时间 */
...
public final int GetBufSize() {
	int _size = 32;
	_size += 2 + (crossGroupIdList.size() * 8);
...
public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	...
	short _crossGroupIdListCount = _buf.getShort();
	for(int _i = 0; _i < _crossGroupIdListCount; _i++) { 
		long _crossGroupIdList = (long)0;
		if(_buf.remaining() > 0) _crossGroupIdList = _buf.getLong();
		crossGroupIdList.add(_crossGroupIdList);
	}
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
...
public final void PutUnzipBuf(ByteBuffer _buf) {
	...
	_buf.putShort((short)crossGroupIdList.size());
	for(int _i = 0; _i < crossGroupIdList.size(); _i++) { 
		_buf.putLong(crossGroupIdList.get(_i));
	}
