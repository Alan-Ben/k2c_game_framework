using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using ALBasicProtocolPack;


namespace GS2GC.p034_InnOp
{

/// <summary>
/// 旅店菜品信息变更
/// </summary>
public class GS2GC_034_052_OnInnDishChg : ALBasicProtocolPack._IALProtocolStructure {
/// <summary>
/// 菜品信息
/// </summary>
private Common.InnObj.Inn_DishInfo dishInfo;


public GS2GC_034_052_OnInnDishChg() {
	dishInfo = new Common.InnObj.Inn_DishInfo();
}

public GS2GC_034_052_OnInnDishChg(
	Common.InnObj.Inn_DishInfo _dishInfo
) {	dishInfo = _dishInfo;
}

public byte getMainOrder() { return (byte)34; }

public byte getSubOrder() { return (byte)52; }

/// <summary>
/// 菜品信息
/// </summary>
public Common.InnObj.Inn_DishInfo getDishInfo() { return dishInfo; }
/// <summary>
/// 菜品信息
/// </summary>
public void setDishInfo(Common.InnObj.Inn_DishInfo _dishInfo) { dishInfo = _dishInfo; }


public int GetBufSize() {
	int _size = 34;

	return _size;
}

public int GetFullPackBufSize() {
	int _size = 36;

	return _size;
}



public void ReadUnzipBuf(ALProtocolBuf _buf, int _finalPos) {
	 if(_finalPos > 0 && _buf.getCurPos() >= _finalPos) return ;
	int _dishInfoCustLen = _buf.getInt();
	int _dishInfoCurPos = _buf.getCurPos();
	dishInfo.ReadUnzipBuf(_buf, _dishInfoCurPos + _dishInfoCustLen);
	_buf.setPosition(_dishInfoCurPos + _dishInfoCustLen);

}

public void PutUnzipBuf(ALProtocolBuf _buf) {
	_buf.putInt(dishInfo.GetBufSize());
	dishInfo.PutUnzipBuf(_buf);
}

public byte[] makeFullPackage() {
	int _bufSize = GetBufSize() + 2;
	ALProtocolBuf _buf = ALProtocolBuf.allocate(_bufSize);
	_buf.put((byte)34);
	_buf.put((byte)52);
	PutUnzipBuf(_buf);
	return _buf.getBuf();
}
public void makeFullPackage(ALProtocolBuf _recBuf) {
	if(null == _recBuf)
		return ;
	_recBuf.put((byte)34);
	_recBuf.put((byte)52);
	PutUnzipBuf(_recBuf);
}
public byte[] makePackage() {
	int _bufSize = GetBufSize();
	ALProtocolBuf _buf = ALProtocolBuf.allocate(_bufSize);
	PutUnzipBuf(_buf);
	return _buf.getBuf();
}
public void readPackage(byte[] _buf) {
	ALProtocolBuf _bufObj = new ALProtocolBuf(_buf);
	ReadUnzipBuf(_bufObj, -1);
}
public void readPackage(ALProtocolBuf _buf) {
	ReadUnzipBuf(_buf, -1);
}
public override string ToString() {
	System.Text.StringBuilder builder = new System.Text.StringBuilder();

	builder.Append("{");
	builder.Append("dishInfo").Append(":").Append(dishInfo == null ? "null" : dishInfo.ToString()).Append(", ");
	builder.Append("}");
	return builder.ToString();
}

}

}

