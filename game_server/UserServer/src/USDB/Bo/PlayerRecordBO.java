...
@RefBo(isIdAuto= true)
public class PlayerRecordBO extends BaseBO {
    ...
    @DataBaseField(type = "int(11)", fieldname = "type", comment = "记录类型")
    private int type;

    public static final int FIELD_count =2;
    @DataBaseField(type = "bigint(20)", fieldname = "count", comment = "数据计数")
    private long count;
    ...
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `player_record` ("
                + "`id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '自增主键',"
                + "`cid` bigint(20) NOT NULL DEFAULT '0' COMMENT '玩家账号ID',"
                + "`type` int(11) NOT NULL DEFAULT '0' COMMENT '记录类型',"
                + "`count` bigint(20) NOT NULL DEFAULT '0' COMMENT '数据计数',"
                + "KEY `cid` (`cid`),"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='Record 玩家记录数据' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
