...
public class NPLS2GC_001_002_EnterGameRes implements ALBasicProtocolPack._IALProtocolStructure {
private boolean res;
...
/** 是否白名单 */
public void setIsWhite(boolean _isWhite) { isWhite = _isWhite; }
...
public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	...
	if(_buf.remaining() > 0) checkCode = ALBasicProtocolPack.ALProtocolCommon.GetStringFromBuf(_buf);
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
	if(_buf.remaining() > 0) isWhite = (_buf.get() != 0);
}
