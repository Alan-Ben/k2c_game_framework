...
/// </summary>
public class WCGAITriggerChgBattleValue extends _AWCGBasicAITrigger
{
    ...
    {
        return EWCGAITriggerType.CHG_BATTLE_V;
    }

    public static WCGAITriggerChgBattleValue read(String _infoStr)
    {
        WCGAITriggerChgBattleValue obj = new WCGAITriggerChgBattleValue();
        String[] strs = CommonFunc.charSplit(_infoStr, ':');
        if (strs.length < 2)
        {
            CommLog.error("ai CHG_BATTLE_V配置错误 正确配置: 枚举:下标索引:值");
            return null;
        }
        try
        {
        ...
        {
            CommLog.error("ai CHG_BATTLE_V配置错误 正确配置: 枚举:下标索引:值", e);
            return null;
        }
    }
}