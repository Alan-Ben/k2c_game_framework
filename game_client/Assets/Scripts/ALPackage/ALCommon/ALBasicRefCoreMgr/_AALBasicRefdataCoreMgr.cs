...
namespace ALPackage
{
    public interface _IALInitRefObj
    {
        void init(System.Action _call, System.Action<Type, _IALInitRefObj> _onFail);
        //获取最终调用函数
        Action finalDelegate { get; }
    }

    public abstract class _AALBasicRefdataCoreMgr
    {
        private List<_IALInitRefObj> _m_lLoadList = new List<_IALInitRefObj>();

        public _AALBasicRefdataCoreMgr()
        {
            _m_lLoadList = new List<_IALInitRefObj>();
        }
...
//                Debug.Log($"{Time.frameCount} init {i}");
        ...
        public void InitAllRefCoreAsync(Action _completeCall)
        {
            ...
            ALCoroutineDealerMgr.instance.addCoroutine(new ALCoroutineWrapper(InitAllRefCoreAsyncCoroutine(_completeCall)));
        }
        ...
        IEnumerator InitAllRefCoreAsyncCoroutine(Action _completeCall)
        {
            ...
            //记录时间，用于避免过长加载
            long timeStampMs = ALCommon.getNowTimeMill();
#if UNITY_EDITOR
            ...
            {
                //                Debug.Log($"{Time.frameCount} init {i}");
                long startInitTimeMs = ALCommon.getNowTimeMill();
                //加载操作
                _m_lLoadList[i].init(stepCounter.addDoneStepCount, _onRefInitFail);

                //计算耗时
                long endInitTimeMs = ALCommon.getNowTimeMill();
#if UNITY_EDITOR
                thisFrameInitCount++;
#endif
                ...
                if (_AALMonoMain.instance.showDebugOutput)
                {
                    ...
                    if(loadUseTime > 20)
                        UnityEngine.Debug.LogWarning($"【Init Use Time {loadUseTime} - {_m_lLoadList[i].ToString()}】 ");
                    if(loadUseTime > 10)
                        UnityEngine.Debug.Log($"【Init Use Time {loadUseTime} - {_m_lLoadList[i].ToString()}】 ");
#if !UNITY_EDITOR
                }
#endif
                ...
                {
                    timeStampMs = ALCommon.getNowTimeMill();
#if UNITY_EDITOR
                    UnityEngine.Debug.LogWarning($"本轮加载Refdata 加载数据集数量【{thisFrameInitCount}】 ");
        ...
        /** 加载失败处理 */
        protected abstract void _onRefInitFail(Type _class, _IALInitRefObj _obj);

        //初始化加载队列
        protected abstract void _initLoadList(List<_IALInitRefObj> _list);
