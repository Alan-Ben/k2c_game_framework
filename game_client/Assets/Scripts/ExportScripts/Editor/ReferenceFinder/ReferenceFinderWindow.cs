...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    private bool needRepaintAssetTree = false;
    //工具栏按钮样式
    ...
    private int helpBoxHeight = 65;

    private string findDepthStr = "20";//查找深度
    private int findDepth = 20;//查找深度
    [SerializeField]
    private TreeViewState m_TreeViewState;
    
    //查找资源引用信息
    [MenuItem("Assets/Find References Pro")]
    static void FindRef()
    {
        InitDataIfNeeded();
        OpenWindow();
    ...
    public List<string> getReference(string _guid)
    {
//        if (data == null)
        {
            InitDataIfNeeded();
        ...
        return data.getReference(_guid, true);
    }
    ...
    private void OnGUI()
    {
        ...
        {
            //绘制Treeview
            m_AssetTreeView.OnGUI(new Rect(0, toolbarGUIStyle.fixedHeight+helpBoxHeight, position.width, position.height - toolbarGUIStyle.fixedHeight-helpBoxHeight));
        }        
    }
    ...
    public void DrawOptionBar()
    {
        ...
        //扩展
        if (GUILayout.Button("GUID2Path", toolbarButtonGUIStyle))
        {
            Debug.Log($"GUID:{GUIUtility.systemCopyBuffer},Path:"+ AssetDatabase.GUIDToAssetPath(GUIUtility.systemCopyBuffer));
        }
        if (GUILayout.Button("Path2GUID", toolbarButtonGUIStyle))
        {
            Debug.Log($"Path:{GUIUtility.systemCopyBuffer},GUID:"+AssetDatabase.AssetPathToGUID(GUIUtility.systemCopyBuffer));
        }
        GUILayout.Label("查找深度:");
        findDepthStr = GUILayout.TextField(findDepthStr, 3);
        if (!int.TryParse(findDepthStr, out findDepth))
        {
            Debug.LogError("查找深度应输入一个整型值(int)");
        }
        //扩展
        ...
        EditorGUILayout.HelpBox("说明：Refrence Finder窗口有两个模式Depend和Reference\n" +
                                   "(1)Model(Depend)模式：显示这个文件依赖哪些文件（用了哪些东西）\n" +
                                   "(2)Model(Reference)模式：显示这个文件引用了哪些文件（用在哪里）\n" +
                                   "注：如果有新增文件或者删除文件，需要手动刷新（Refresh Data）不然结果可能有错，因为插件检索后缓存下来，所以速度上也会比遍历的方式快。\n" +
                                   "ps:支持同时选择多个文件" 
            , MessageType.None);
        EditorGUILayout.EndHorizontal();
    }
    ...
    private  AssetViewItem CreateTree(string guid, ref int elementCount, int _depth)
    {
        ...
        if (_depth > findDepth)
        {
            if (_depth > 50)
            {
                Debug.LogError($"树深度超过50，大概率是相互引用导致的，直接不显示了:{elementCount},{AssetDatabase.GUIDToAssetPath(guid)}");
            }
            return root;
        }
