...
@Service
@Slf4j
public class DungeonEventService {

    @Resource
    private RedisTemplate<String, Object> redisTemplate;

    private static final String EVENT_KEY = "dungeon:event:";

    /**
     * 获取活动加成
     */
    public DungeonEventBonus getEventBonus(int dungeonId) {
        String key = EVENT_KEY + dungeonId;
        Object bonus = redisTemplate.opsForValue().get(key);

        if (bonus != null) {
            return (DungeonEventBonus) bonus;
        }

        // 无活动加成
        return DungeonEventBonus.empty();
    }

    /**
     * 计算伤害加成
     */
    public long applyDamageBonus(long baseDamage, int dungeonId) {
        DungeonEventBonus bonus = getEventBonus(dungeonId);

        if (bonus.getDamageMultiplier() > 1.0f) {
            long bonusDamage = (long) (baseDamage * (bonus.getDamageMultiplier() - 1.0f));
            log.info("活动伤害加成: baseDamage={}, bonusDamage={}, multiplier={}",
                baseDamage, bonusDamage, bonus.getDamageMultiplier());
     ...
     * 计算掉落加成
     */
    public List<ItemInfo> applyDropBonus(List<ItemInfo> baseRewards, int dungeonId) {
        DungeonEventBonus bonus = getEventBonus(dungeonId);

        if (bonus.getDropMultiplier() > 1.0f) {
            List<ItemInfo> bonusRewards = new ArrayList<>();

            for (ItemInfo item : baseRewards) {
                // 额外掉落
                int bonusCount = (int) (item.getCount() * (bonus.getDropMultiplier() - 1.0f));
                if (bonusCount > 0) {
                    bonusRewards.add(new ItemInfo(item.getType(), item.getId(), bonusCount));
                }
            }

            log.info("活动掉落加成: bonusRewards={}", bonusRewards.size());
            return bonusRewards;
        }
     ...
     * 获取额外挑战次数
     */
    public int getExtraAttackCount(int dungeonId) {
        DungeonEventBonus bonus = getEventBonus(dungeonId);
        return bonus.getExtraAttackCount();
    }
}
 ...
 */
@Data
public class DungeonEventBonus {
    private float damageMultiplier = 1.0f;
    private float dropMultiplier = 1.0f;
    private int extraAttackCount = 0;
    private String eventName = "";
    private long startTime;
    private long endTime;

    public static DungeonEventBonus empty() {
        return new DungeonEventBonus();
    }
}
```
 ...
 */
@Service
public class LimitedDungeonEventService {
     ...
     * 开启限时副本活动
     */
    public void startLimitedEvent(LimitedDungeonEvent event) {
        log.info("开启限时副本活动: eventId={}, name={}", event.getEventId(), event.getName());

        // 1. 创建特殊Boss
        createEventBoss(event);
     ...
     * 结束限时副本活动
     */
    public void endLimitedEvent(LimitedDungeonEvent event) {
        log.info("结束限时副本活动: eventId={}, name={}", event.getEventId(), event.getName());

        // 1. 结算排行奖励
        settleEventRankRewards(event);

        // 2. 推送活动结束通知
        broadcastEventEnd(event);

        // 3. 清理活动缓存
        clearEventCache(event);
    }

    private void createEventBoss(LimitedDungeonEvent event) {
        // 创建活动专属Boss
        EveningDungeonBoss boss = new EveningDungeonBoss();
        boss.setBossId(event.getEventBossId());
        boss.setBossName(event.getEventBossName());
        boss.setHp(event.getEventBossHp());
        boss.setCurrentHp(event.getEventBossHp());

        dungeonService.createEventBoss(boss);
    }

    private void settleEventRankRewards(LimitedDungeonEvent event) {
        // 获取活动排行榜
        List<EventRankItem> rankList = getEventRankList(event.getEventId());

        for (EventRankItem item : rankList) {
            // 发放活动排行奖励
            List<ItemInfo> rewards = getEventRankRewards(event, item.getRank());
            mailService.sendMail(
                item.getPlayerId(),
                event.getName() + " 排行奖励",
                String.format("恭喜您在%s中获得第%d名！", event.getName(), item.getRank()),
                rewards
            );
        }
    }
