...
{
#if AL_ILRUNTIME
    public class _IALMonoTaskAdapter : CrossBindingAdaptor
    {
        public override Type BaseCLRType
        {
            get
            {
                return typeof(_IALBaseMonoTask);//这是你想继承的那个类
            }
        }
        ...
        public class Adapter : _IALBaseMonoTask, CrossBindingAdaptorType
        {
            ...
            //    if (mTestVirtual != null && !isVirtualInvoking)
            //    {
            //        isVirtualInvoking = true;
            //        param1[0] = str;
            //        appdomain.Invoke(mTestVirtual, instance, param1);
            //        isVirtualInvoking = false;
            //    }
            //    else
            ...
            bool _m_bIsDealGot;
            IMethod _m_iDealMethod;
            public void deal()
            {
                if(!_m_bIsDealGot)
                {
                    _m_iDealMethod = instance.Type.GetMethod("deal", 0);
                    _m_bIsDealGot = true;
                }
                if(_m_iDealMethod != null)
                {
                    appdomain.Invoke(_m_iDealMethod, instance, param0);//没有参数建议显式传递null为参数列表，否则会自动new object[0]导致GC Alloc
                }
            }
