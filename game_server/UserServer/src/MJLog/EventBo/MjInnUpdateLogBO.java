...
public class MjInnUpdateLogBO extends MJEventLogBo {
    ...
    public static final int FIELD_device_id =7;
    @DataBaseField(type = "bigint(20)", fieldname = "device_id", comment = "货舱/清单/礼物/勋章id")
    private long device_id;

    public static final int FIELD_before_lv =8;
    @DataBaseField(type = "int(11)", fieldname = "before_lv", comment = "升级前等级")
    private int before_lv;

    public static final int FIELD_after_lv =9;
    @DataBaseField(type = "int(11)", fieldname = "after_lv", comment = "升级后等级")
    ...
    public MjInnUpdateLogBO(ResultSet rs) throws Exception {
        id = rs.getLong(1);
        cid = rs.getLong(2);
        uid = rs.getString(3);
        vip_lv = rs.getInt(4);
        ...
        device_id = rs.getLong(9);
        before_lv = rs.getInt(10);
        after_lv = rs.getInt(11);
    ...
    public void getFromResultSet(ResultSet rs, List list) throws Exception {
        list.add(new MjInnUpdateLogBO(rs));
    }

    @Override
    public String getItemsName() {
        return "`id`, `cid`, `uid`, `vip_lv`, `server_id`, `platform`, `region`, `create_time`, `device_id`, `before_lv`, `after_lv`, `timestamp`";
    }

    @Override
    public String getTableName() {
        return "`mj_inn_update_log`";
    }
    ...
    public String getItemsValue() {
        ...
        strBuf.append("'").append(device_id).append("', ");
        strBuf.append("'").append(before_lv).append("', ");
        strBuf.append("'").append(after_lv).append("', ");
   ...
   @Override
    ...
    public void setDeviceId(BM _bm, long device_id) {
        if(device_id==this.device_id) 
            return;
        this.device_id = device_id; 
        markField(_bm, FIELD_device_id); 
    }
    public void saveDeviceId(BM _bm, long device_id) {
        if(device_id==this.device_id) 
            return;
        this.device_id = device_id;
        saveField(_bm, "device_id", device_id);
    }

    // 升级前等级
    public int getBeforeLv() { return this.before_lv; }
    public void setBeforeLv(BM _bm, int before_lv) {
        if(before_lv==this.before_lv) 
            return;
        this.before_lv = before_lv; 
        markField(_bm, FIELD_before_lv); 
    }
    public void saveBeforeLv(BM _bm, int before_lv) {
        if(before_lv==this.before_lv) 
            return;
        this.before_lv = before_lv;
        saveField(_bm, "before_lv", before_lv);
    }

    // 升级后等级
    public int getAfterLv() { return this.after_lv; }
    public void setAfterLv(BM _bm, int after_lv) {
        if(after_lv==this.after_lv) 
            return;
        this.after_lv = after_lv; 
        markField(_bm, FIELD_after_lv); 
    }
    public void saveAfterLv(BM _bm, int after_lv) {
        if(after_lv==this.after_lv) 
            return;
        this.after_lv = after_lv;
        saveField(_bm, "after_lv", after_lv);
    }
    ...
    protected String getUpdateKeyValue() {
        ...
        sBuilder.append(" `device_id` = '").append(device_id).append("',");
        sBuilder.append(" `before_lv` = '").append(before_lv).append("',");
        sBuilder.append(" `after_lv` = '").append(after_lv).append("',");
    ...
    public String getMarkedUpdateKeyValue() {
        ...
        if(isFieldMarked(FIELD_device_id)) sBuilder.append(" `device_id` = '").append(device_id).append("',");
        if(isFieldMarked(FIELD_before_lv)) sBuilder.append(" `before_lv` = '").append(before_lv).append("',");
        if(isFieldMarked(FIELD_after_lv)) sBuilder.append(" `after_lv` = '").append(after_lv).append("',");
        if(isFieldMarked(FIELD_timestamp)) sBuilder.append(" `timestamp` = '").append(timestamp).append("',");
        sBuilder.deleteCharAt(sBuilder.length() - 1);
        return sBuilder.toString();
    }
    public static String getSql_TableCreate() {
        String sql = "CREATE TABLE IF NOT EXISTS `mj_inn_update_log` ("
                ...
                + "`device_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '货舱/清单/礼物/勋章id',"
                + "`before_lv` int(11) NOT NULL DEFAULT '0' COMMENT '升级前等级',"
                + "`after_lv` int(11) NOT NULL DEFAULT '0' COMMENT '升级后等级',"
                + "`timestamp` int(11) NOT NULL DEFAULT '0' COMMENT '事件发生时间戳(10位)',"
                + "PRIMARY KEY (`id`)"
                + ") COMMENT='梦加日志-旅店货舱/清单/礼物/勋章升级记录' engine=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci AUTO_INCREMENT=1";
        return sql;
    }
    ...
    public ByteBuffer toByteBuffer()
    {
        ...
        buff.putLong(device_id);
        buff.putInt(before_lv);
        buff.putInt(after_lv);
    ...
    protected void readFromByteBuffer(ByteBuffer buff)
    {
        ...
        device_id=buff.getLong();
        before_lv=buff.getInt();
        after_lv=buff.getInt();
