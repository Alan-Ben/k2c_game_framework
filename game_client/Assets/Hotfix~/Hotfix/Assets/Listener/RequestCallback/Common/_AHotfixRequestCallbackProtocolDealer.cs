...
namespace Hotfix
{
    ...
    /// </summary>
    public abstract class _AHotfixRequestCallbackProtocolDealer<T> : _INPRequestCallbackDealer where T : _IALProtocolStructure
    {
        //是否在处理回调前默认调用协议处理
        ...
        public _AHotfixRequestCallbackProtocolDealer()
        {
            _m_bPreDealGCProtocol = false;
            _m_bNeedprintProtocol = true;
        }
        public _AHotfixRequestCallbackProtocolDealer(bool _preDealGCProtocol, bool _needPrintProtocol)
        {
        ...
        public void dealSuc(byte[] _sucMsg)
        {
            ...
            if(Game.instance.mainCamera.gameSetting.printProtocol && _m_bNeedprintProtocol)
            {
                ...
                {
                    GCommon.NetRecv(string.Format("[Hotfix]<color=red>S -> C: {0} ; </color> {1}", protocol.GetType().Name, HotfixGCommon.GetInfoPropertys(protocol)));
                }
                else
                {
        ...
        public void dealFail(int _errCode, byte[] _sucMsg)
        {
            ...
            if(Game.instance.mainCamera.gameSetting.printProtocol)
            {
                if (protocol.GetFullPackBufSize() <= Game.instance.mainCamera.gameSetting.protocolPrintMinSize)
                {
                    GCommon.NetRecv(string.Format("[Hotfix]<color=red>S -> C: {0} ; </color> errorCode:{1} msg:{2}", protocol.GetType().Name, _errCode, ProtocolErrorCodeResult.instance.getResultMsg(_errCode)));
                }
                else
                {
                    GCommon.NetWaring($"协议{protocol.GetType().Name}过大，大小：{protocol.GetFullPackBufSize() / 1024}kb。不打印，注意检查");
