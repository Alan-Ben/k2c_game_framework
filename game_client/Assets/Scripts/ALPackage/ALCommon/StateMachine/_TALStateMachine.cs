...
namespace ALPackage
{
    ...
    /// author: Coda
    /// </summary>
    public class _TALStateMachine<STATE_T, T> where T : Enum where STATE_T : _ATALStateBase<T>
    {
        //状态工厂对象
        private _ATALStateFactory<STATE_T, T> _m_stateFactory;
        //当前状态对应类
        private Type _m_tType;
        // 当前所处的状态
        [NotNull] private STATE_T _m_curState;

        public _TALStateMachine(_ATALStateFactory<STATE_T, T> _factory)
        {
        ...
        public _TALStateMachine(_ATALStateFactory<STATE_T, T> _factory, STATE_T _defaultState)
        {
            _m_stateFactory = _factory;
            // 构造一个空状态
            _m_tType = null;
