...
namespace IFix.Editor
{
    ...
    public class IFixEditor
    {
        ...
        [MenuItem("Hotfix/InjectFix/Inject", false, 1)]
        ...
        {
            UnityEngine.Debug.Log("[InjectFix] AutoInjectAssemblys");
        ...
        [MenuItem("Hotfix/InjectFix/Fix", false, 2)]
        public static void Patch()
        {
            EditorUtility.DisplayProgressBar("Generate Patch for Edtior", "patching...", 0);
            try
            {
                if (!Directory.Exists("InjectFix"))
                {
                    Directory.CreateDirectory("InjectFix");
                }
                if (!Directory.Exists("InjectFix/pc"))
                {
                    Directory.CreateDirectory("InjectFix/pc");
                }
                ...
                foreach (var assembly in injectAssemblys)
                {
                    ...
                    GenPatch(assembly, assembly_path, "./Assets/Plugins/IFix.Core.dll",
                        string.Format("{0}{1}.patch.bytes", "InjectFix/pc/", assembly));
                }
...
#if UNITY_2018_3_OR_NEWER
        [MenuItem("Hotfix/InjectFix/Fix(Android)", false, 3)]
        public static void CompileToAndroid()
        {
            ...
            try
            {
                ...
                if (!Directory.Exists("InjectFix/android"))
                {
                    Directory.CreateDirectory("InjectFix/android");
                }
                
                GenPlatformPatch(Platform.android, "InjectFix/android/");
            }
            catch(Exception e)
            {
                UnityEngine.Debug.LogError(e);
        ...
        [MenuItem("Hotfix/InjectFix/Fix(IOS)", false, 4)]
        public static void CompileToIOS()
        {
            ...
            try
            {
                ...
                if (!Directory.Exists("InjectFix/ios"))
                {
                    Directory.CreateDirectory("InjectFix/ios");
                }
                
                GenPlatformPatch(Platform.ios, "InjectFix/ios/");
            }
            catch(Exception e)
            {
                UnityEngine.Debug.LogError(e);
        ...
        //补丁数
        private static int _m_curPatchCount;

        /// <summary>
        /// 输出补丁数
        /// </summary>
        private static void _debugPatchCount()
        {
            if(_m_curPatchCount == 0)
                UnityEngine.Debug.Log($"【IFix】需要Patch的方法数量为0");
        }

        /// <summary>
        /// 计算补丁数
        /// </summary>
        private static void _calPatchCount()
        {
            int count = 0;
            foreach (var assembly in injectAssemblys)
            {
                var patchMethods = Configure.GetTagMethods(typeof(PatchAttribute), assembly).ToList();
                count += patchMethods.Count;
            }
            _m_curPatchCount = count;
        }
    }
}
