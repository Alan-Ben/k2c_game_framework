...
namespace UnityEditor.Rendering.Universal
{
    ...
    internal class ShaderPreprocessor : IPreprocessShaders
    {
        ...
        void InitializeLocalShaderKeywords(Shader shader)
        {
            ...
            m_Dithering = TryGetLocalKeyword(shader, ShaderKeywordStrings.Dithering);
        }
        ...
        bool StripUnusedPass(ShaderFeatures features, ShaderSnippetData snippetData)
        {
            ...
            //补充剔除内置管线passName
            if (snippetData.passName == "FORWARD_DELTA")
                return true;
        ...
        bool StripUnusedFeatures(ShaderFeatures features, Shader shader, ShaderSnippetData snippetData, ShaderCompilerData compilerData)
        {
            ...
            //组合变体剔除
            bool universalRenderPipelineCustomVariantCombineSetting = (globalSettings != null && globalSettings.universalRenderPipelineCustomVariantCombineSetting != null);
            if (universalRenderPipelineCustomVariantCombineSetting && StripCustomVariantCombine(globalSettings.universalRenderPipelineCustomVariantCombineSetting,shader,compilerData))
            {
                return true;
            }

            //LightCookies
            bool stripLightCookiesShaders = (globalSettings == null || globalSettings.stripLightCookiesVariants);
            if(stripLightCookiesShaders && compilerData.shaderKeywordSet.IsEnabled(m_LightCookies))
                return true;
            ...
            bool stripMainLightShadowsCascade = (globalSettings == null || globalSettings.stripMainLightShadowsCascade);
            if(stripMainLightShadowsCascade && compilerData.shaderKeywordSet.IsEnabled(m_MainLightShadowsCascades))
                return true;

            bool forceUseSoftShadowVariants = (globalSettings != null && globalSettings.forceUseSoftShadowVariants);
#if UNITY_ANDROID
            if (forceUseSoftShadowVariants && !compilerData.shaderKeywordSet.IsEnabled(m_SoftShadows) &&
                (compilerData.shaderKeywordSet.IsEnabled(m_MainLightShadows) || compilerData.shaderKeywordSet.IsEnabled(m_MainLightShadowsCascades))
                && !compilerData.shaderKeywordSet.IsEnabled(m_MainLightShadowsScreen))
                return true;
           
#else
            //  有主阴影并且不是屏幕空间阴影的情况下，强制使用软阴影
            if (forceUseSoftShadowVariants && !compilerData.shaderKeywordSet.IsEnabled(m_SoftShadows) 
                                           && compilerData.shaderKeywordSet.IsEnabled(m_MainLightShadowsCascades)
                                           && !compilerData.shaderKeywordSet.IsEnabled(m_MainLightShadowsScreen))
                return true;
#endif
        ...
        bool StripCustomVariantCombine(
            UniversalRenderPipelineCustomVariantCombineSetting universalRenderPipelineCustomVariantCombineSetting,
            Shader shader, ShaderCompilerData compilerData)
        {
            //共生变体 剔除
            foreach (var symbiosisShader in universalRenderPipelineCustomVariantCombineSetting.symbiosisShaders)
            {
                if (shader.name != symbiosisShader.shader.name)
                    continue;

                foreach (var symbiosisVariantCombine in symbiosisShader.symbiosisVariantCombines)
                {
                    bool hasVariantName1 = compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(shader,
                        symbiosisVariantCombine.variantName1));
                    bool hasVariantName2 = compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(shader,
                        symbiosisVariantCombine.variantName2));

                    if (hasVariantName1 ^ hasVariantName2) //异或代表仅开启其中一个
                    {
                        string errorVariantName = hasVariantName1 ? symbiosisVariantCombine.variantName1 : symbiosisVariantCombine.variantName2;

                        //如果没有其他共生关系 则剔除
                        if (!HaveOtherSymbiosis(symbiosisShader, symbiosisVariantCombine, errorVariantName, shader,
                            compilerData))
                            return true;
                    }
                }
            ...
            //寄生变体 剔除
            foreach (var parasitismVariantShader in universalRenderPipelineCustomVariantCombineSetting.parasitismShaders)
            {
                if (shader.name != parasitismVariantShader.shader.name)
                    continue;
                
                foreach (var parasitismVariantCombine in parasitismVariantShader.parasitismVariantCombines)
                {
                    bool hasVariantName1 = compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(shader,
                        parasitismVariantCombine.variantName1));

                    bool hasParasitismVariantName2 = compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(
                        shader,
                        parasitismVariantCombine.parasitismVariantName2));

                    //如果
                    if (!hasVariantName1 && hasParasitismVariantName2)
                    {
                        //如果没有其他寄生关系 则剔除
                        if (!HaveOtherParasitism(parasitismVariantShader, parasitismVariantCombine, shader,
                            compilerData))
                            return true;
                    }
            ...
            //竞争变体 剔除
            foreach (var competitionShader in universalRenderPipelineCustomVariantCombineSetting.competitionShaders)
            {
                if (shader.name != competitionShader.shader.name)
                    continue;

                foreach (var competitionVariantCombine in competitionShader.competitionVariantCombines)
                {
                    bool hasVariantName1 = compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(shader,
                        competitionVariantCombine.variantName1));

                    bool hasVariantName2 = compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(
                        shader,
                        competitionVariantCombine.variantName2));
                    
                    //同时存在的变体剔除
                    if (hasVariantName1 && hasVariantName2)
                    {
                        return true;
                    }

                    bool hasVariantName2Protect = false;
                    foreach (var variantName2Protect in competitionVariantCombine.variantName2Protects)
                    {
                        if (compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(
                            shader, variantName2Protect)))
                            hasVariantName2Protect = true;
                    }

                    //有保镖的剔除变体1
                    if (hasVariantName2Protect && hasVariantName1)
                        return true;
                    
                    //没有保镖剔除变体2
                    if (!hasVariantName2Protect && hasVariantName2)
                        return true;
                }
            }
            return false;
        ...
        //是否有其他共生关系
        bool HaveOtherSymbiosis(SymbiosisShader symbiosisShader,SymbiosisVariantCombine errorSymbiosisVariantCombine,string errorVariantName,Shader shader, ShaderCompilerData compilerData)
        {
            //判断是否有其他共生关系
            foreach (var internalVariantCombine in symbiosisShader.symbiosisVariantCombines)
            {
                if(internalVariantCombine.Equals(errorSymbiosisVariantCombine))
                    continue;

                                   
                if (internalVariantCombine.variantName1 == errorVariantName ||
                    internalVariantCombine.variantName2 == errorVariantName)
                {
                    string otherSymbiosisVariantName = internalVariantCombine.variantName1 == errorVariantName ? internalVariantCombine.variantName2 : internalVariantCombine.variantName1;

                    if (compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(shader,
                        otherSymbiosisVariantName)))
                    {
                        return true;
                    }
                }
        ...
        //是否有其他寄生关系
        bool HaveOtherParasitism(ParasitismShader parasitismShader,ParasitismVariantCombine errorParasitismVariantCombine,Shader shader, ShaderCompilerData compilerData)
        {
            //判断是否有其他寄生关系
            foreach (var internalVariantCombine in parasitismShader.parasitismVariantCombines)
            {
                if(internalVariantCombine.Equals(errorParasitismVariantCombine))
                    continue;

                                   
                if (internalVariantCombine.parasitismVariantName2 == errorParasitismVariantCombine.parasitismVariantName2)
                {
                    if (compilerData.shaderKeywordSet.IsEnabled(TryGetLocalKeyword(shader,
                        internalVariantCombine.variantName1)))
                    {
                        return true;
                    }
                }
        ...
        bool StripVolumeFeatures(VolumeFeatures features, Shader shader, ShaderSnippetData snippetData, ShaderCompilerData compilerData)
        {
            ...
            if (true)
            {
                if (compilerData.shaderKeywordSet.IsEnabled(m_BloomLQDirt) // �������ķ�����۹�
                    || compilerData.shaderKeywordSet.IsEnabled(m_BloomHQDirt) // �������ķ�����۹�
                    || compilerData.shaderKeywordSet.IsEnabled(m_BloomHQ)  // �������ķ���
                    || compilerData.shaderKeywordSet.IsEnabled(m_ToneMapNeutral)
                    || compilerData.shaderKeywordSet.IsEnabled(m_UseDrawProcedural) // XR �õ�
                    || compilerData.shaderKeywordSet.IsEnabled(m_Dithering) // �����ɫתΪ8λ�����ڴ��õ�
                    )
                    return true;
            }
        ...
        void LogShaderVariants(Shader shader, ShaderSnippetData snippetData, ShaderVariantLogLevel logLevel, int prevVariantsCount, int currVariantsCount, double stripTimeMs, IList<ShaderCompilerData> compilerDataList)
        {
            if (logLevel == ShaderVariantLogLevel.AllShaders || shader.name.Contains("Universal Render Pipeline"))
            {
                ...
                Debug.Log(result);
                // if (shader.name == "URPEx/Skin")
                {
                    string info = "";
                    foreach (var scData in compilerDataList)
                    {
                        string prevShaderKeywords = "";
                        var keywords = scData.shaderKeywordSet.GetShaderKeywords();
                        foreach (var keyword in keywords)
                        {
                            prevShaderKeywords += $"{keyword},";
                        }
                        info += $"{prevShaderKeywords}\n";
                    }
                    Debug.Log($"BenKeywords:{shader.name}:{info}");
                }
                
            }
        }

        private string getCompilerDataKeywords(Shader shader,IList<ShaderCompilerData> compilerDataList)
        {
            string prevShaderKeywords = "";
            if (shader.name != "URPEx/Fabric")
                return prevShaderKeywords;
            foreach (var data in compilerDataList)
            {
                prevShaderKeywords = "";
                ...
                Debug.Log($"{shader.name}:{prevShaderKeywords}");
            }
        ...
        public void OnProcessShader(Shader shader, ShaderSnippetData snippetData, IList<ShaderCompilerData> compilerDataList)
        {
            ...
            if (urpAsset.shaderVariantLogLevel != ShaderVariantLogLevel.Disabled)
            {
                ...
                // string currShaderKeywords = getCompilerDataKeywords(shader, compilerDataList);

                LogShaderVariants(shader, snippetData, urpAsset.shaderVariantLogLevel, prevVariantCount, compilerDataList.Count, stripTimeMs, compilerDataList);
            }
