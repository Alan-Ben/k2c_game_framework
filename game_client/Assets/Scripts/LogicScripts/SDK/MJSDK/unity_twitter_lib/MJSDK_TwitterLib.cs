...
namespace MJSDK_Package
{
    ...
    /// twitter组件
    /// </summary>
    public class MJSDK_TwitterLib
    {
        //组件标识符  — 库名
        private static string _g_MJSDK_LibName = "MJSDK_Twitter";
        //组件版本信息
...
#if UNITY_IOS
            //组件库依赖iOS平台库 版本判断
            _g_isInit = g_Version.judgePlatformDependenceVersionIsEnough(_platform_ComponentInfo.version, 0, 10, 4, 0);
#elif UNITY_ANDROID
        ...
        /// <summary>
        /// twitter-userInfo：twitter账号信息
        /// </summary>
        /// <param name="_sucDelegate"></param>
        /// <param name="_failDelegate"></param>
        [UnityEngine.Scripting.Preserve]
        public static void twitter_userInfo(Action<MJSDK_PhpApiCommon_Twitter_UserInfo> _sucDelegate, Action<int, string> _failDelegate)
        {
            if (!MJSDK_Tool_Lib.checkInitState(_g_MJSDK_LibName, _g_isInit, _failDelegate))
            {
                return;
            }
            MJSDK.sendMsgToPhonePlatform("twitter", "userInfo", "", new MJSDK_CallbackDealer_Model<MJSDK_PhpApiCommon_Twitter_UserInfo>(_sucDelegate, _failDelegate));
        }
    }
}
