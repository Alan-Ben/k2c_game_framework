...
namespace GOE
{
    ...
    /// 跑马灯item对象池
    /// </summary>
    public class GMarqueeItemCacheMgr
    {
        private static GMarqueeItemCacheMgr _g_instance;
        [NotNull]
        public static GMarqueeItemCacheMgr instance
        {
            get
            {
                if (_g_instance == null)
                    _g_instance = new GMarqueeItemCacheMgr();
                return _g_instance;
            }
        }
        ...
        //不同资源缓存池
        [NotNull] private Dictionary<long, GMarqueeItemCache> _m_dCacheDic;
        //根节点
        [NotNull] private readonly GameObject _m_cacheRoot;

        public GMarqueeItemCacheMgr()
        {
            _m_cacheRoot = new GameObject("marquee_item_cache_root");
            _m_cacheRoot.SetActive(false);
            _m_cacheRoot.transform.localPosition = Vector3.up * -10000;
            Object.DontDestroyOnLoad(_m_cacheRoot);
            _m_dCacheDic = new Dictionary<long, GMarqueeItemCache>();
        }
        ...
        public void popItem(long _uiResId,Transform _itemParent,Action<GGUIWndSubMarqueeItem>  _onPopAction)
        {
            ...
            void onCacheLoaded()
            {
                ...
                GGUIWndSubMarqueeItem wnd = cache.popItem();
            ...
            {
                // 如果没有获取cache，就构建一个新的
                cache = new GMarqueeItemCache(_uiResId, _m_cacheRoot.transform, _m_iMinCacheCount, _m_iMaxCacheCount);
                // 添加到字典中
        ...
        public void pushBackCacheItem(long _uiResId, GGUIWndSubMarqueeItem _item)
        {
            ...
            // 尝试找到对应的缓存池
            if (_m_dCacheDic.TryGetValue(_uiResId, out GMarqueeItemCache cache) && cache != null)
            {
                // 放回对应的缓存池
            ...
            {
                // 如果没有找到缓存池，弹出错误提示
                ALLog.Error($"GMarqueeItemCacheMgr 出现了未知的异常，Cache回收资源时，未找到对应缓存池");
        ...
        public void discard()
        {
            foreach (GMarqueeItemCache cache in _m_dCacheDic.Values)
            {
                cache?.forceDiscard();
            }
            _m_dCacheDic.Clear();
