...
namespace GOE
{
    ...
    /// 现金礼包子窗口
    /// </summary>
    public class GGUISubWndCashGiftPack : _ATALBasicUISubWnd<GGUISubMonoCashGiftPack>
    {
        //礼包信息
        private GiftPackRefObj _m_giftPackRef;
        private List<NPCommonCostItem> _m_lTmpNormalRewardItemList;
        ...
        public GGUISubWndCashGiftPack(GGUISubMonoCashGiftPack _mono) : base(_mono)
        {
            initWnd();
        }
        ...
        protected override void _onDiscard()
        {
            ...
            _m_giftPackRef = null;
            _m_lTmpNormalRewardItemList?.Clear();
            _m_lTmpNormalRewardItemList = null;
        }
        ...
        private void _refreshWnd()
        {
            ...
            //礼包名称
            string giftPackName = TextTranslate.instance.getLanguage(_m_giftPackRef.name, _m_giftPackRef.name_args);
            ALUGUICommon.setLabelTxt(wnd.txtGiftPackName, giftPackName);
            ALUGUICommon.setLabelTxt(wnd.tmpGiftPackName, giftPackName);
            
            // 礼包描述
            string giftPackDesc = TextTranslate.instance.getLanguage(_m_giftPackRef.desc, _m_giftPackRef.desc_args_list);
            ALUGUICommon.setLabelTxt(wnd.txtGiftPackDesc, giftPackDesc);
            
            //礼包价值百分比
            string profitPerStr = TextTranslate.instance.getLanguage(TransKeyConst.common_percentage_num, _m_giftPackRef.profit_per / 100);
            if(!string.IsNullOrEmpty(wnd.profitPerKey))
                profitPerStr = TextTranslate.instance.getLanguage(wnd.profitPerKey, profitPerStr);
            ALUGUICommon.setLabelTxt(wnd.txtProfitPer, profitPerStr);
            ALUGUICommon.setLabelTxt(wnd.tmpProfitPer, profitPerStr);
            ...
            else
            {
                ...
                {
                    if (wnd.specialItemNeedShowAsSingle)
                    {
                        NPCommonCostItem specialItem = _m_giftPackRef.specialShowRewardItem;

                        if (_m_lTmpNormalRewardItemList == null)
                            _m_lTmpNormalRewardItemList = new List<NPCommonCostItem>();
                        _m_giftPackRef.getNormalShowRewardItemList(_m_lTmpNormalRewardItemList);

                        //展示特殊道具
                        if (_m_wSpecialItem != null)
                        {
                        ...
                        //展示奖励列表
                        _m_wItemContainer?.showWnd();
                        _m_wItemContainer?.showItemList(_m_lTmpNormalRewardItemList);
                    }
                    else
                    {
                        //展示奖励列表
