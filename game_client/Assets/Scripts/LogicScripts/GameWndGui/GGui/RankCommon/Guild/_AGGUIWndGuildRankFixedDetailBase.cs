...
namespace GOE
{
    ...
    /// 常驻联盟排行榜界面基类
    /// </summary>
    public abstract class _AGGUIWndGuildRankFixedDetailBase<T> : _ANPGGUIBasicResBarWnd<T> where T : GGUIMonoGuildRankFixedDetail
    {
        //排行榜实例id列表
        ...
        //排行榜数据对象
        private GuildRankShowData _m_rdRankData;
        //当前处理的排行数据序列号
        private long _m_lCurDealRankSerialize;
        //排名列表
        private GGUIWndGuildRankFixedDetailGrid _m_wRankGrid;
        //自己的排名item
        private GGUIWndGuildRankFixedTopGuildInfo _m_wSelfRankItem;
        //排行榜详情前几名信息附加窗口
        private List<GGUIWndGuildRankFixedTopGuildInfo> _m_lTopPlayerInfoList;

        protected _AGGUIWndGuildRankFixedDetailBase(EALUIWndLayer _layer) : base(_layer)
        {
            _m_rdRankData = new GuildRankShowData();
            _m_lCurDealRankSerialize = 0;
        }

        private long _m_lUIResId = UIResPathConst.C_COMMON_RANK_GUILD_NORMAL_RES_ID;//资源id
       ...
       **/
        ...
        protected override void _onHideWnd()
        {
            ...
            {
                foreach (GGUIWndGuildRankFixedTopGuildInfo subTopPlayerInfo in _m_lTopPlayerInfoList)
                {
        ...
        protected override void _onWndInitDone()
        {
            ...
            if (wnd.monoRankListGrid != null)
                _m_wRankGrid = new GGUIWndGuildRankFixedDetailGrid(wnd.monoRankListGrid);

            if (wnd.monoSelfRankItem != null)
                _m_wSelfRankItem = new GGUIWndGuildRankFixedTopGuildInfo(wnd.monoSelfRankItem);

            _m_lTopPlayerInfoList = new List<GGUIWndGuildRankFixedTopGuildInfo>();
            if (wnd.monoTopPlayerList != null)
            {
                for (int i = 0; i < wnd.monoTopPlayerList.Count; i++)
                {
                    _m_lTopPlayerInfoList.Add(new GGUIWndGuildRankFixedTopGuildInfo(wnd.monoTopPlayerList[i]));
                }
            }
        ...
        private void _setRankFixedId(long _rankFixedId)
        {
            ...
            if (!_m_rdRankData.isDone && _m_rdRankData.rankFixedId != _rankFixedId && _m_lTopPlayerInfoList != null)
            {
                for (int i = 0; i < _m_lTopPlayerInfoList.Count; i++)
                {
                    ...
                    _m_lTopPlayerInfoList[i]?.setInfo(null, 0);
                }
            }
            _m_wSelfRankItem?.showWnd();
            _m_wSelfRankItem?.setInfo(_m_rdRankData.selfData, 0);
            wnd?.switchRankAnim?.resetAni();
            ALUGUICommon.setGameObjEnable(wnd?.goCanLikeShowList, false);
            ALUGUICommon.setGameObjEnable(wnd?.goCanNotLikeShowList, false);

            //刷新排行榜描述
            _refreshRankDesc(_rankFixedId);

            //请求数据刷新界面
            _m_rdRankData.setRanFixedkId(_rankFixedId
                , (_serialize) =>
                {
        ...
        private void _refreshWnd()
        {
            ...
            //获取排名列表
            List<GuildRankInfo> rankShowInfoList = new List<GuildRankInfo>();
            if(_m_rdRankData.rankListGridDataList != null)
                rankShowInfoList.AddRange(_m_rdRankData.rankListGridDataList);
            rankShowInfoList.Sort((_a,_b)=>_a.rankSortOrder.CompareTo(_b.rankSortOrder));
            //设置前几名的特殊展示
            for (int i = 0; i < _m_lTopPlayerInfoList.Count; i++)
            {
                _m_lTopPlayerInfoList[i]?.showWnd();
                if (rankShowInfoList.Count > 0)
                {
                    _m_lTopPlayerInfoList[i]?.setInfo(rankShowInfoList[0], 0);
                    //移除已设置的数据
                    rankShowInfoList.RemoveAt(0);
                }
                else
                    _m_lTopPlayerInfoList[i]?.setInfo(null, 0);
            }
            ...
            _m_wRankGrid?.setShowData(rankShowInfoList);
            //设置自己的排名
            _m_wSelfRankItem?.showWnd();
            _m_wSelfRankItem?.setInfo(_m_rdRankData.selfData, 0);
            //刷新点赞
            _refreshLike(_m_rdRankData.rankListGridDataList != null ? _m_rdRankData.rankListGridDataList.Count : 0);
        }

        //刷新点赞状态
        private void _refreshLike(long _rankListCount)
        {
            ...
            ALUGUICommon.setGameObjEnable(wnd.goCanLikeShowList, needLike && count > 0 && _rankListCount > 0);
            ALUGUICommon.setGameObjEnable(wnd.goCanNotLikeShowList, needLike && count <= 0 && _rankListCount > 0);
        }
        ...
        private void _switchToTargetRank(int _index)
        {
            ...
            else
            {
                ...
                int curIndex = _m_curIdx;

                //关闭当前窗口切换到个人排行榜窗口
                _onClickClose(null);
                QueueMgr.instance.AddNode(new GNodeRank(rankFixedIdList, curIndex));
            }
        }
