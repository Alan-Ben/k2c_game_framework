 ...
 * ugui中的事件监听对象
 **/
public class ALUGUIEventDragListener : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IMoveHandler
{
    /** 回调函数对象 */
     ...
     * 获取事件监听对象
     **/
    public static ALUGUIEventDragListener Get(GameObject _go)
    {
        if (null == _go)
            return null;

        ALUGUIEventDragListener listener = _go.GetComponent<ALUGUIEventDragListener>();
        if (null == listener)
            listener = _go.AddComponent<ALUGUIEventDragListener>();
