...
import NPGameRes.GameObjs.PlayerCondition.ConditionObj.NPPlayerCondition_CS_CONSORT_RES_COUNT;
...
public class NPPlayerConditionDealer_CS_CONSORT_RES_COUNT extends _ANPPlayerConditionDealer
{
    @Override
    ...
    {
        NPPlayerCondition_CS_CONSORT_RES_COUNT cond = (NPPlayerCondition_CS_CONSORT_RES_COUNT) _cond;

        ConsortInfo consort = _userData.getConsortComponent().lookup(cond.getConsortId());
        if(null == consort)
        	return false;
        
        long count = consort.getConsortRes(cond.getConsortResType());
