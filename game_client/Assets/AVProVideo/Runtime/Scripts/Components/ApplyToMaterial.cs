...
namespace RenderHeads.Media.AVProVideo
{
	...
	public sealed class ApplyToMaterial : ApplyToBase
	{
		...
		{
			get { return _propTexture.Id; }
			set { _propTexture.Id = value; }
		}
		...
		public override void Apply()
		{
			...
			if (_media != null && _media.TextureProducer != null)
			{
				...
				if (texture != null)
				{
					...
					{
#if UNITY_EDITOR
						if (IsLikelyBlackFrame(texture))
						{
							Debug.LogError($"视频黑帧,请确认视频源:{_media.MediaPath.Path}", gameObject);
						}
#endif
		...
		// 简易 GPU 黑帧检测（采样中心像素）
		private bool IsLikelyBlackFrame(Texture tex)
		{
			if (!(tex is Texture2D) && !(tex is RenderTexture)) return false;

			// 获取一个 1x1 小采样（GPU→CPU）
			RenderTexture rt = RenderTexture.GetTemporary(16, 16, 0);
			Graphics.Blit(tex, rt);
			RenderTexture.active = rt;
			Texture2D temp = new Texture2D(16, 16, TextureFormat.RGB24, false);
			temp.ReadPixels(new Rect(0, 0, 16, 16), 0, 0);
			temp.Apply();
			...
			avg = avg /pixels.Length / 3f;
			// float avg = (temp.GetPixel(0, 0).r + temp.GetPixel(0, 0).g + temp.GetPixel(0, 0).b) / 3f;
			Object.Destroy(temp);
			bool isBlack = avg <= 0.01f;
			return isBlack;  // 亮度阈值
		}
	}
}