 ...
 * 联盟职位变更
 **/
public class GuildLog_PositionChange implements ALBasicProtocolPack._IALProtocolStructure {
/** 操作者信息 */
...
public GuildLog_PositionChange() {
	...
	targetPlayerName = "";
	targetPosition = Common.GuildEnum.EGuildPositionType.values()[0];
}
...
public final void ReadUnzipBuf(ByteBuffer _buf, int _finalPos) {
	...
	if(_buf.remaining() > 0) targetPlayerName = ALBasicProtocolPack.ALProtocolCommon.GetStringFromBuf(_buf);
	 if(_finalPos > 0 && _buf.position() >= _finalPos) return ;
	if(_buf.remaining() > 0) targetPosition = Common.GuildEnum.EGuildPositionType.EGuildPositionType_FromInt(_buf.getInt());
}
...
public final void PutUnzipBuf(ByteBuffer _buf) {
	...
	_buf.putInt(targetPosition.ordinal());
