...
namespace GOE
{
    ...
    /// 充值返利页签item容器
    /// </summary>
    public class GGUIWndRechargeRebatePageTabContainer : _ATNPGGUIWndShowAnimContainer<GGUIMonoRechargeRebatePageTabContainerItem, GGUIMonoRechargeRebatePageTabContainer, GGUIWndRechargeRebatePageTabContainerItem>
    {
        //item列表
        protected List<GGUIWndRechargeRebatePageTabContainerItem> _m_lItemList;
        //当前选中的item
        private GGUIWndRechargeRebatePageTabContainerItem _m_wCurSelectItem;
        //点击item事件
        ...
        /// 点击item事件
        /// </summary>
        public Action<GGUIWndRechargeRebatePageTabContainerItem, bool> onClickItem { get { return _m_aOnClickItem; } set { _m_aOnClickItem = value; } }

        public GGUIWndRechargeRebatePageTabContainer(GGUIMonoRechargeRebatePageTabContainer _mono) : base(_mono)
        {
            initWnd();
        }

        protected override GGUIWndRechargeRebatePageTabContainerItem _createItemWnd(GGUIMonoRechargeRebatePageTabContainerItem _itemMono)
        {
            GGUIWndRechargeRebatePageTabContainerItem item = new GGUIWndRechargeRebatePageTabContainerItem(_itemMono);
        ...
        protected override void _onWndInitDone()
        {
            ...
            _m_lItemList = new List<GGUIWndRechargeRebatePageTabContainerItem>();
        ...
        /// 设置信息
        /// </summary>
        public void showItemList(List<RechargeRebateInfo> _list)
        {
            if (_list == null || _m_lItemList == null)
                return;

            RechargeRebateInfo lastSelectRef = _m_wCurSelectItem?.rechargeRebateInfo;
            _m_wCurSelectItem?.setSelect(false);
            _m_wCurSelectItem = null;
            GGUIWndRechargeRebatePageTabContainerItem itemWnd = null;
            int count = 0;
            //遍历玩家数据
            for (int i = 0; i < _list.Count; i++)
            {
                ...
                itemWnd.showWnd();
                itemWnd.setInfo(_list[i], count == 0);
                count++;

                //判断当前选中的item是否在列表中，有则选中
                if (lastSelectRef != null && lastSelectRef.groupId == _list[i].groupId && lastSelectRef.activityInstanceId == _list[i].activityInstanceId)
            ...
            //列表为空显隐
            ALUGUICommon.setGameObjEnable(wnd.goEmptyShowList, _list.Count <= 0);
            ALUGUICommon.setGameObjEnable(wnd.goEmptyHideList, _list.Count > 0);
        ...
        //点击选中item
        private void _onClickItem(GGUIWndRechargeRebatePageTabContainerItem _item, bool _moveToTop)
        {
            if (wnd == null || _item == null)
                return;

            if (_m_wCurSelectItem == null || (_m_wCurSelectItem.rechargeRebateInfo?.groupId != _item.rechargeRebateInfo?.groupId || _m_wCurSelectItem.rechargeRebateInfo?.activityInstanceId != _item.rechargeRebateInfo?.activityInstanceId))
            {
                _m_wCurSelectItem?.setSelect(false);
