...
namespace ALPackage
{
    ...
    public class ALHttpCommon
    {
        ...
        /// <returns></returns>
        public static string GetContentSync(string _url, IDictionary<string, string> _parameters, bool _isApplicationJsonMode)
        {
            ...
            {
                ALLog.Error($"GetContentSync URL[{_url}] Get Exception: {e}");
                return null;
            }
            finally
            {
        ...
        /// <returns></returns>
        static HttpWebResponse _getHttpWebResponse(string _url, IDictionary<string, string> _parameters, bool _isApplicationJsonMode)
        {
            ...
            if(null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                ALLog.Error($"_getHttpWebResponse url:{url}");
            ...
            {
                ALLog.Error($"_getHttpWebResponse URL[{_url}] Get Exception: {e}");
                return null;
            }
        }
        ...
        public static void GetContentAsync(string _url, IDictionary<string, string> _parameters, bool _isApplicationJsonMode, Action<string> _onSuc, Action<string, int> _onFail)
        {
            if(string.IsNullOrEmpty(_url))
                return;
            ...
            {
                if (null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                    ALLog.Error($"GetAsync new Uri URL[{_url}] Get Exception: {e}");

                if(_onFail != null)
                {
                    ALCommonTaskController.CommonActionAddMonoTask(()=> { _onFail($"GetAsync url Error：{url}\n Exception: {e}", 0); }); //转回主线程调用
                }
            }
            ...
            try
            {
                ...
                request.BeginGetResponse(_result =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                                ALLog.Debug($"GetAsync URL[{_url}] response is null");
                        ...
                        {
                            if (null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                                ALLog.Debug($"GetAsync URL[{_url}] status == {status}");
                        ...
                        if(_onSuc != null)
                        {
                            ALCommonTaskController.CommonActionAddMonoTask(() => { _onSuc(retString); }); //转回主线程调用
                        }
                    }
                    catch (Exception e)
                    {
                        if (null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                            ALLog.Error($"GetAsync URL[{_url}] GetResponse Error: {e}");
            ...
            {
                if (null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                    ALLog.Error($"GetAsync URL[{_url}] General Error: {e}");
        ...
        public static void PostSync(string _url, IDictionary<string, string> _parameters, Action<int, string> _onSucceed, Action<string, int> _onFailed)
        {
            HttpWebResponse response = null;
            Stream myResponseStream = null;
            StreamReader myStreamReader = null;
        ...
        private static HttpWebResponse _createPostHttpResponse(string url, IDictionary<string, string> parameters, int? timeout, CookieCollection cookies)
        {
            if (null != _AALMonoMain.instance && _AALMonoMain.instance.showDebugOutput)
                ALLog.Debug($"Http Post Request url:[{url}]");
            ...
            try
            {
                ...
                if (url.StartsWith("https", StringComparison.OrdinalIgnoreCase))
                {
                    ServicePointManager.ServerCertificateValidationCallback = new RemoteCertificateValidationCallback(CheckValidationResult);
                    request = WebRequest.Create(myUri) as HttpWebRequest;
                    request.ProtocolVersion = HttpVersion.Version10;//来自 https://blog.51cto.com/zhoufoxcn/561934  但是感觉不太合理- -
            ...
            {
                ALLog.Error($"HttpPost 请求{url}失败 e:{e}");
                return null;
            }
        }
