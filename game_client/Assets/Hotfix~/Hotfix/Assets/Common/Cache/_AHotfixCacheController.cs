...
namespace Hotfix
{
     ...
     * 缓存池模板对象
     **/
    public abstract class _AHotfixCacheController<T, TEMP>
    {
        ...
        protected _AHotfixCacheController(int _minCount, int _maxCount)
        {
        ...
        protected _AHotfixCacheController(int _minCount, int _maxCount, int _addUnit)
        {
            _m_tTemplateObj = default(TEMP);
        ...
        public T popItem()
        {
            if (_m_lEnableCacheList.Count <= 0)
            {
                ...
                if (_m_lTotalCacheList.Count > _m_iIsWarningCount)
                {
                    ...
                    //判断是否超过需要报错的数量1000
                    if (_m_lTotalCacheList.Count > 1000)
                    {
