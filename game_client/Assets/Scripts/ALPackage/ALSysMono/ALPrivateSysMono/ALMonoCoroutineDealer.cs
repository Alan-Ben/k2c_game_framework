...
namespace ALPackage
{
    public class ALMonoCoroutineDealer : MonoBehaviour
    {
        ...
        void Update()
        {
            try
            {
                ...
                _IALCoroutineDealer coroutineDealer = ALCoroutineDealerMgr.instance.popCoroutineObj();
                while (null != coroutineDealer)
                {
                    StartCoroutine(coroutineDealer.dealCoroutine());

                    //取出下一个需要执行的Coroutine
                    coroutineDealer = ALCoroutineDealerMgr.instance.popCoroutineObj();
                }
            }
            catch (Exception e)
            {
                UnityEngine.Debug.LogError("ALMonoCoroutineDealer Update has Exception:\n" + e);
