...
namespace GOE
{
	...
	public static class VectorExtension {
	    ...
	    public static void DecomposeYawPitch(Transform rOwner, Vector3 rFrom, Vector3 rTo, ref float rYaw, ref float rPitch) {
	        ...
	        rPitch = (-Mathf.Atan2(lDelta.y, Mathf.Sqrt((lDelta.x * lDelta.x) + (lDelta.z * lDelta.z))) * Mathf.Rad2Deg) +
    ...
    /// When using trig functions:
	    ...
	    /// <summary>
	    /// Returns the slop angle of the plane
	    /// </summary>
	    /// <param name="_normal"></param>
	    /// <returns></returns>
	    public static float GetPlaneAngle(Vector3 _normal) {
	        if (_normal == Vector3.zero) {
	            //法线不存在，则设置坡度90
	            return 90;
	        }
	        else {
	            float slopAngle = Vector3.Angle(_normal, Vector3.up);
	            //                Debug.DrawRay(CachedTransform.position, _normal, Color.red, 2f);
	            if (slopAngle > 90) {
